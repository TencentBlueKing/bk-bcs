...
bcsTokenNotifyTitle="${bcsTokenNotifyTitle:-TKEx(蓝鲸容器平台) API Key 续期提醒}" \
bcsTokenNotifyContent="${bcsTokenNotifyContent:-你好，{{ .Username \}\}:<br>您的 API Key 过期时间为: {{ .ExpiredAt \}\}，如有需要请前往 API Key 页面及时续期。}" \
