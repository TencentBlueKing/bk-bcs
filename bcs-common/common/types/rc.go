...
type EnvVarSource struct {
	...
	// +optional
	ResourceFieldRef *ResourceFieldSelector `json:"resourceFieldRef,omitempty" protobuf:"bytes,2,opt,name=resourceFieldRef"` // nolint
}
...
type Uri struct {
	...
	Pwd       string // nolint package registry password, example for curl -u 'user:pwd' -X GET "http://xxx.registry.xxx.com/xxx/v1/pack.tar.gz"
	OutputDir string
}
