...
func rabbitmqBroker(q *QueueOptions) (broker.Broker, error) {
	...
	brokerOpts = append(brokerOpts, broker.Addrs(q.Address), rabbitmq.ExchangeName(q.Exchange.Name),
...
func natstreamingBroker(q *QueueOptions) (broker.Broker, error) {
	var brokerOpts []broker.Option
	brokerOpts = append(brokerOpts, stan.ClusterID(q.NatsOptions.ClusterID), broker.Addrs(q.Address))
	// exchange durable feature
...
func NewQueueBroker(options *QueueOptions) (broker.Broker, error) {
	...
	switch options.QueueKind {
	case RABBITMQ:
		broker, err = rabbitmqBroker(options)
	case NATSTREAMING:
		broker, err = natstreamingBroker(options)
	default:
		return nil, errors.New("unSupported queue kind")
	}
