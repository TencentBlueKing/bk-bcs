...
// ResourceToQueue pub/sub resourceToQueue
func ResourceToQueue(resourceQueue map[string]string) QueueOption {
	return func(q *QueueOptions) {
		if len(resourceQueue) == 0 {
			q.ResourceToQueue = DefaultResourceToQueue
		} else {
			q.ResourceToQueue = resourceQueue
		}
	}
}
...
func validateNatsMqOptions(n *QueueOptions) error {
	if !(len(n.Address) > 0 && regexp.MustCompile("^nat(s)?://.*").MatchString(n.Address)) {
...
func validateRabbitMqOptions(r *QueueOptions) error {
	if !(len(r.Address) > 0 && regexp.MustCompile("^amqp(s)?://.*").MatchString(r.Address)) {
