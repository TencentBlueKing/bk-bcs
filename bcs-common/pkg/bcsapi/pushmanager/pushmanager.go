...
// SetClientConfig set push manager client config
...
func GetClient(innerClientName string) (PushManagerClient, func(), error) {
	if clientConfig == nil {
		return nil, nil, bcsapi.ErrNotInited
	}
	var addr string
	if discovery.UseServiceDiscovery() {
		addr = fmt.Sprintf("%s:%d", discovery.PushManagerServiceName, discovery.ServiceGrpcPort)
	} else {
		if clientConfig.Discovery == nil {
			return nil, nil, fmt.Errorf("push manager module not enable discovery")
		}
	...
	klog.Infof("get push manager client with address: %s", addr)
	conf := &bcsapi.Config{
		Hosts:           []string{addr},
		TLSConfig:       clientConfig.TLSConfig,
		InnerClientName: innerClientName,
	}
	cli, closeCon := NewPushManager(conf)

	return cli, closeCon, nil
}

// NewPushManager create PushManager SDK implementation
func NewPushManager(config *bcsapi.Config) (PushManagerClient, func()) {
	...
	for i := 0; i < maxTries; i++ {
		...
		if err != nil {
			klog.Errorf("Create push manager grpc client with %s error: %s", addr, err.Error())
			continue
		}
		if conn != nil {
			break
	...
	if conn == nil {
		klog.Errorf("create no push manager client after all instance tries")
		return nil, nil
	}

	// init push manager client
	// nolint
	return NewPushManagerClient(conn), func() { conn.Close() }
}
