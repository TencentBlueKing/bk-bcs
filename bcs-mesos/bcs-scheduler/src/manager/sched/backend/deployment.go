...
func (b *backend) DeleteDeployment(ns string, name string, enforce bool) (int, error) {
	...
	if deployment.Application != nil {
		...
		if err != nil {
			blog.Warn("delete application(%s.%s) error %s", ns, deployment.Application.ApplicationName, err.Error())
		}
	...
	if deployment.ApplicationExt != nil {
		...
		if err != nil {
			blog.Warn("delete application(%s.%s) error %s", ns, deployment.ApplicationExt.ApplicationName, err.Error())
		}
