...
func NewRoleController(localIP string, zkAddr string, watchPath string) (RoleInterface, error) {
	if len(localIP) == 0 {
	...
	go func() {
		for event := range eventChan {
			if event.Err != nil {
				...
				statistic.Set(statusid, fmt.Errorf("received error from zk discover chan, err: %v", err))
				continue
			}
			...
			if err := json.Unmarshal([]byte(event.Server[0]), &info); nil != err {
				...
				statistic.Set(statusid, fmt.Errorf("unmarshal zk info failed, skip role handler, err: %v", err))
				continue
			}
			statistic.Reset(statusid)

			if info.IP == r.localIP {
