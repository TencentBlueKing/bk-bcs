...
func (cpq *scaleUpProjectQuota) buildScaleUpHostQuotaSteps() []*types.Step {
	...
	content := fmt.Sprintf("user(%s) scale up project(%s) CA "+
		...
		cpq.resource.GetZoneResources().GetQuotaNum()+cpq.projectQuota.Quota.HostResources.QuotaNum)
...
func (cpq *scaleUpProjectQuota) buildScaleUpSharedQuotaSteps() []*types.Step {
	...
	content := fmt.Sprintf("user %s scale up shared cluster %s "+
...
func (cpq *scaleUpProjectQuota) buildScaleUpFederationQuotaSteps() []*types.Step {
	...
	content := fmt.Sprintf("user(%s) scale up federation cluster(%s) "+
