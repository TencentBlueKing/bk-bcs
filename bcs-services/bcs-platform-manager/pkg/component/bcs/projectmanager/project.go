...
func GetProject(ctx context.Context, projectIDOrCode string) (*bcsproject.Project, error) {
	...
	p, err := cli.GetProject(cli.ctx, &bcsproject.GetProjectRequest{ProjectIDOrCode: projectIDOrCode})
	if err != nil {
...
func ListProject(ctx context.Context) ([]*bcsproject.Project, error) {
	cli, err := New(ctx)
	if err != nil {
		return nil, err
	}
	...
	p, err := cli.ListProjects(cli.ctx, &bcsproject.ListProjectsRequest{All: true})
	if err != nil {
