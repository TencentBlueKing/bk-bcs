...
func newDenylistListerWatcher(namespaces map[string]struct{}, next cache.ListerWatcher) cache.ListerWatcher {
	if len(namespaces) == 0 {
		return next
	}
...
func (w *denylistListerWatcher) List(options metav1.ListOptions) (runtime.Object, error) {
	...
	if err != nil {
		glog.Error("msg", "error listing", "err", err)
		return nil, err
	}

	objs, err := meta.ExtractList(list)
	if err != nil {
		glog.Error("msg", "error extracting list", "err", err)
		return nil, err
	}

	metaObj, err := meta.ListAccessor(list)
	if err != nil {
		glog.Error("msg", "error getting list accessor", "err", err)
		return nil, err
	}
	...
	for _, obj := range objs {
		...
		if err != nil {
			glog.Error("msg", "error getting meta accessor accessor", "obj", fmt.Sprintf("%v", obj), "err", err)
			return nil, err
		}
		...
		for {
			...
			case event, ok := <-next.ResultChan():
				...
				if err != nil {
					...
					glog.Infof("msg", fmt.Sprintf("unexpected object type in event (%T): %v", event.Object, event.Object))
					continue
				}
