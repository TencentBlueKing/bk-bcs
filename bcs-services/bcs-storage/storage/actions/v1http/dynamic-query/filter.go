...
func qGenerate(q qFilter, timeLayout string) *operator.Condition {
	...
	for i := 0; i < n; i++ {
		...
		sub := operator.BaseCondition
		// extra filter
		switch op {
		case "timeL":
			if t, err := getTime(value, timeLayout); err == nil {
				sub = sub.AddOp(operator.Gt, tag, t)
			} else {
				continue
			}
		case "timeR":
			if t, err := getTime(value, timeLayout); err == nil {
				sub = sub.AddOp(operator.Lt, tag, t)
			} else {
				continue
			}
		case "int64":
			if v, err := strconv.ParseInt(value, 10, 0); err == nil {
				sub = sub.AddOp(operator.Eq, tag, v)
			}
		case "int":
		...
		case "bool":
			...
			sub = sub.AddOp(operator.Eq, tag, v)
		default:
			sub = sub.AddOp(operator.In, tag, strings.Split(value, ","))
		}

		if allowNoExists {
			sub = sub.Or(operator.BaseCondition.AddOp(operator.Ext, tag, false))
		}
		r = r.And(sub)
	}
	return r
