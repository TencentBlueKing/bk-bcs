...
var rollbackV1CMD = &cobra.Command{
	Use:   "rollbackv1",
	Short: "rollbackv1",
	Long:  "rollbackv1 chart release",
	Run:   RollbackV1,
}

// Rollback provide the actions to do rollbackCMD
func RollbackV1(cmd *cobra.Command, args []string) {
	req := &helmmanager.RollbackReleaseV1Req{}

	if len(args) < 2 {
		fmt.Printf("rollback v1 args need at least 2, install [name] [revision]\n")
		os.Exit(1)
	}
	revision, err := strconv.Atoi(args[1])
	if err != nil {
		fmt.Printf("rollback v1 get invalid revision %s, %s\n", args[1], err.Error())
		os.Exit(1)
	}
	if revision <= 0 {
		fmt.Printf("rollback v1 get invalid revision %s, revision should be positive\n", args[1])
		os.Exit(1)
	}
	...
	if err := c.Release().RollbackV1(cmd.Context(), req); err != nil {
...
func init() {
	rollbackV1CMD.PersistentFlags().StringVarP(
