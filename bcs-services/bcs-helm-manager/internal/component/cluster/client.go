...
func genClusterInfoCacheKey(clusterID string) string {
	return cacheClusterIDKeyPrefix + "-" + clusterID
}
...
func NewCMClient() (*CMClient, error) {
	cli := CMClient{cache: cache.New(defaultExpiration, cache.NoExpiration)}
...
func (c *CMClient) fetchClusterInfoWithCache(ctx context.Context, clusterID string) (*Cluster, error) {
	...
	log.Info("cluster %s info not in cache, start call cluster manager", clusterID)
	...
	if err = c.cache.Add(cacheKey, clusterInfo, cache.NoExpiration); err != nil {
		log.Warn("set cluster info to cache failed: %v", err)
	}
...
func (c *CMClient) fetchClusterInfo(ctx context.Context, clusterID string) (*Cluster, error) {
	url := fmt.Sprintf("%s/bcsapi/v4/clustermanager/v1/cluster/%s", config.GlobalOptions.BCSAPIGW.Host, clusterID)
