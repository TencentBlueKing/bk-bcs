...
func (g *GetAddonsDetailAction) Handle(ctx context.Context,
	...
	// get current status
	rl, err := g.model.GetRelease(ctx, g.req.GetClusterID(), addons.Namespace, addons.AssociationName(addons.Name))
	...
	if rl != nil {
		...
		clusterAddons.ReleaseName = &rl.ReleaseName
		clusterAddons.DisplayName = &rl.DisplayName
		if len(rl.Values) > 0 {
	...
	clusterRelease, err := g.releaseHandler.Cluster(g.req.GetClusterID()).
		Get(ctx, release.GetOption{Namespace: addons.Namespace, Name: addons.AssociationName(addons.ReleaseName)})
	if err != nil && !errors.Is(err, driver.ErrReleaseNotFound) {
		blog.Warnf("releaseHandler get release detail failed, %s, clusterID: %s namespace: %s, name: %s",
			err.Error(), g.req.GetClusterID(), addons.Namespace, addons.AssociationName(addons.ReleaseName))
		g.setResp(common.ErrHelmManagerGetActionFailed, err.Error(), nil)
		return nil
	}
