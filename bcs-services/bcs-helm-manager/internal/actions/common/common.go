/*
 * Tencent is pleased to support the open source community by making Blueking Container Service available.
 * Copyright (C) 2019 THL A29 Limited, a Tencent company. All rights reserved.
 * Licensed under the MIT License (the "License"); you may not use this file except
 * in compliance with the License. You may obtain a copy of the License at
 * http://opensource.org/licenses/MIT
 * Unless required by applicable law or agreed to in writing, software distributed under,
 * the License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND,
 * either express or implied. See the License for the specific language governing permissions and
 * limitations under the License.
 */

package common

import (
	"context"

	"github.com/Tencent/bk-bcs/bcs-common/common/blog"
	"github.com/Tencent/bk-bcs/bcs-services/bcs-helm-manager/internal/common"
	helmmanager "github.com/Tencent/bk-bcs/bcs-services/bcs-helm-manager/proto/bcs-helm-manager"
)

// AvailableAction provide the instance to handle available process
type AvailableAction struct {
	ctx context.Context

	req  *helmmanager.AvailableReq
	resp *helmmanager.AvailableResp
}

// NewAvailableAction return a new AvailableAction
func NewAvailableAction() *AvailableAction {
	return &AvailableAction{}
}

// Handle the available check process
func (aa *AvailableAction) Handle(ctx context.Context,
	req *helmmanager.AvailableReq, resp *helmmanager.AvailableResp) error {
	if req == nil || resp == nil {
		blog.Errorf("action/common/available: check available failed, req or resp is empty")
		return common.ErrHelmManagerReqOrRespEmpty.GenError()
	}
	aa.ctx = ctx
	aa.req = req
	aa.resp = resp

	aa.setResp(common.ErrHelmManagerSuccess, "ok")
	blog.Infof("action/common/available: check available success")
	return nil
}

func (aa *AvailableAction) setResp(err common.HelmManagerError, message string) {
	code := err.Int32()
	msg := err.ErrorMessage(message)
	aa.resp.Code = &code
	aa.resp.Message = &msg
	aa.resp.Result = err.OK()
}
