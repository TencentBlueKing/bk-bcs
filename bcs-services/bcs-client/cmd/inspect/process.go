...
func inspectProcess(c *utils.ClientContext) error {
	...
	single, err := storage.InspectProcess(c.ClusterID(), c.Namespace(), c.String(utils.OptionName))
	if err != nil {
		return fmt.Errorf("failed to inspect process: %v", err)
	}
