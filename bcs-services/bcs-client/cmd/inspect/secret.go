...
func inspectSecret(c *utils.ClientContext) error {
	...
	single, err := storage.InspectSecret(c.ClusterID(), c.Namespace(), c.String(utils.OptionName))
	if err != nil {
		return fmt.Errorf("failed to inspect secret: %v", err)
	}
