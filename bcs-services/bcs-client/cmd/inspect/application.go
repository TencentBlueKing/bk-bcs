...
func inspectApplication(c *utils.ClientContext) error {
	...
	single, err := storage.InspectApplication(c.ClusterID(), c.Namespace(), c.String(utils.OptionName))
	if err != nil {
		return fmt.Errorf("failed to inspect application: %v", err)
	}
