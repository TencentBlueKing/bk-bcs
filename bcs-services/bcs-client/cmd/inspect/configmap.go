...
func inspectConfigMap(c *utils.ClientContext) error {
	...
	single, err := storage.InspectConfigMap(c.ClusterID(), c.Namespace(), c.String(utils.OptionName))
	if err != nil {
		return fmt.Errorf("failed to inspect configmap: %v", err)
	}
