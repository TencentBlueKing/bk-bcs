...
func inspectDeployment(c *utils.ClientContext) error {
	...
	single, err := storage.InspectDeployment(c.ClusterID(), c.Namespace(), c.String(utils.OptionName))
	if err != nil {
		return fmt.Errorf("failed to inspect deployment: %v", err)
	}
