...
func refreshUsertoken(c *utils.ClientContext) error {
	...
	user, err := userManager.RefreshUsertoken(c.String(utils.OptionUserType), c.String(utils.OptionUserName))
	if err != nil {
		return fmt.Errorf("failed to refresh usertoken: %v", err)
	}
