...
type Option struct {
	Sys *cc.SysOption
	// Name current feed proxy instance name，is the only one of all feed proxy.
	Name string
}

// InitOptions init feed proxy's options from command flags.
func InitOptions() (*Option, error) {
	...
	fs.StringVarP(&opt.Name, "name", "n", "", "feed proxy instance name, that is the only one of all feed proxy. "+
...
// minor used to generate the default feed proxy instance name, which generates an 8-bit string
func minor() string {
