...
func (s *Service) UnDeleteKv(ctx context.Context, req *pbds.UnDeleteKvReq) (*pbbase.EmptyResp, error) {
	...
	// 只有删除的才能恢复
	kv, err := s.dao.Kv().GetByKvState(kt, req.Attachment.BizId, req.Attachment.AppId, req.Spec.Key, []string{
	...
	// 看该key是否有存在新增
	addKv, err := s.dao.Kv().GetByKvState(kt, req.Attachment.BizId, req.Attachment.AppId, req.Spec.Key, []string{
		string(table.KvStateAdd),
	})
	...
	toUpdate, err := s.getKvUpdateData(kt, req.Attachment.BizId, req.Attachment.AppId, kv)
	if err != nil {
		return nil, err
	}
...
func (s *Service) UndoKv(ctx context.Context, req *pbds.UndoKvReq) (*pbbase.EmptyResp, error) {
	...
	if err = s.dao.Kv().Update(kt, toUpdate); err != nil {
		logs.Errorf("undo kv (%d) failed, err: %v, rid: %s", req.Spec.Key, err, kt.Rid)
		return nil, err
	}
...
func (s *Service) getKvUpdateData(kt *kit.Kit, bizID, appID uint32, kv *table.Kv) (*table.Kv, error) {
