...
func newGateway(st serviced.State, dao dao.Set) (*gateway, error) {
	mux, err := newDataServiceMux()
	if err != nil {
		return nil, err
	}
...
// newDataServiceMux new data service mux.
func newDataServiceMux() (*runtime.ServeMux, error) {
	...
	if err = pbds.RegisterDataHandler(context.Background(), mux, conn); err != nil {
		logs.Errorf("register data service handler client failed, err: %v", err)
		return nil, err
	}
