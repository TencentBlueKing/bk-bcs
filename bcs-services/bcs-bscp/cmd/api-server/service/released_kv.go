...
func (m *kvService) Export(w http.ResponseWriter, r *http.Request) {
	...
	case "xlsx":
		exporter = &XLSXExporter{OutData: rkvsToOutData(rkvs.Details)}
	...
	case "xml":
		exporter = &XMLExporter{OutData: rkvsToOutData(rkvs.Details)}
		w.Header().Set("Content-Type", "application/xml")
		w.Header().Set("Content-Disposition", "attachment; filename=output.xml")
...
func rkvsToOutData(rkvs []*pbrkv.ReleasedKv) []RkvOutData {

	var d []RkvOutData

	for _, rkv := range rkvs {
		d = append(d, RkvOutData{
			Key:    rkv.Spec.Key,
