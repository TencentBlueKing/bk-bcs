...
type ClientEvent interface {
	...
	List(kit *kit.Kit, bizID, appID, clientID uint32, startTime, endTime time.Time, searchValue string,
		order *pbds.ListClientMetricEventsReq_Order, opt *types.BasePage) ([]*table.ClientEvent, int64, error)
}
...
func (dao *clientEventDao) List(kit *kit.Kit, bizID, appID, clientID uint32, startTime, endTime time.Time,
	searchValue string, order *pbds.ListClientMetricEventsReq_Order, opt *types.BasePage) (
...
// 处理排序
func (dao *clientEventDao) handleOrder(order *pbds.ListClientMetricEventsReq_Order) []field.Expr {
	var exprs []field.Expr
	m := dao.genQ.ClientEvent
