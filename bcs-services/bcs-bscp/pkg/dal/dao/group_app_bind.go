...
func (dao *groupAppDao) Get(kit *kit.Kit, groupID, appID, bizID uint32) (*table.GroupAppBind, error) {
	...
	err := dao.orm.Do(dao.sd.ShardingOne(bizID).DB()).Get(kit.Ctx, item, sqlBuf.String(), groupID, appID, bizID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, errf.New(errf.NotFound, "group app bind not found")
		}
		return nil, err
	}
	return item, nil
