...
func (dao *appDao) Create(kit *kit.Kit, g *table.App) (uint32, error) {
	...
	createTx := func(tx *gen.Query) error {
		...
		if err = q.Create(g); err != nil {
			return errf.Errorf(errf.DBOpFailed, i18n.T(kit, "create data failed, %s", err))
		}

		if err = ad.Do(tx); err != nil {
			return errf.Errorf(errf.DBOpFailed, i18n.T(kit, "execution of transactions failed, %s", err))
		}
		...
		if err = eDecorator.Fire(one); err != nil {
			...
			return errf.Errorf(errf.DBOpFailed, i18n.T(kit, "fire event failed, %s", err.Error()))
		}
	...
	if err != nil {
		return 0, errf.Errorf(errf.DBOpFailed, i18n.T(kit, "transaction processing failed %s", err))
	}
...
func (dao *appDao) Update(kit *kit.Kit, g *table.App) error {
	...
	if err != nil {
		return errf.Errorf(errf.DBOpFailed, i18n.T(kit, "get update app failed, err: %v", err))
	}
	...
	updateTx := func(tx *gen.Query) error {
		...
		if err = eDecorator.Fire(one); err != nil {
			...
			return errf.Errorf(errf.DBOpFailed, i18n.T(kit, "fire event failed, err: %s", err))
		}
		return nil
	}
