...
type Client interface {
	...
	List(kit *kit.Kit, bizID, appID uint32, heartbeatTime int64, search *pbclient.ClientMetricQueryCondition,
		order *pbds.ListClientMetricsReq_Order, opt *types.BasePage) ([]*table.Client, int64, error)
	// ListClientGroupByCurrentReleaseID 通过当前版本ID统计数量
	ListClientGroupByCurrentReleaseID(kit *kit.Kit, bizID, appID uint32) ([]types.ClientConfigVersionCharts, error)
}
...
// ListClientGroupByCurrentReleaseID 通过当前版本ID统计数量
func (dao *clientDao) ListClientGroupByCurrentReleaseID(kit *kit.Kit, bizID uint32, appID uint32) (
	[]types.ClientConfigVersionCharts, error) {
	m := dao.genQ.Client
	q := dao.genQ.Client.WithContext(kit.Ctx).Where(m.BizID.Eq(bizID), m.AppID.Eq(appID))
	var items []types.ClientConfigVersionCharts
	err := q.Select(m.CurrentReleaseID, m.ID.Count().As("total")).Group(m.CurrentReleaseID).Scan(&items)
	if err != nil {
		return nil, err
	}
	return items, nil
...
func (dao *clientDao) List(kit *kit.Kit, bizID, appID uint32, heartbeatTime int64,
	search *pbclient.ClientMetricQueryCondition, order *pbds.ListClientMetricsReq_Order,
...
// 处理搜索
func (dao *clientDao) handleSearch(kit *kit.Kit, bizID, appID uint32, search *pbclient.ClientMetricQueryCondition) (
	...
	if len(search.GetIp()) > 0 {
		ip := field.NewString(m.TableName(), "ip")
		conds = append(conds, q.Where(ip.Like("%"+search.GetIp()+"%")))
	}
	if len(search.GetUid()) > 0 {
		uid := field.NewString(m.TableName(), "uid")
		conds = append(conds, q.Where(uid.Like("%"+search.GetUid()+"%")))
	}
	...
	if len(search.GetCurrentReleaseName()) > 0 {
		...
		if len(item) > 0 {
			...
			conds = append(conds, q.Or(m.CurrentReleaseID.In(releaseID...)))
		}
	}
	...
	if len(search.GetTargetReleaseName()) > 0 {
		...
		if len(item) > 0 {
			...
			if len(clientEvent) > 0 {
				cid := []uint32{}
				for _, v := range clientEvent {
					cid = append(cid, v.ClientID)
				}
				conds = append(conds, q.Or(m.ID.In(cid...)))
			}
		}
	}
...
// 处理排序
func (dao *clientDao) handleOrder(order *pbds.ListClientMetricsReq_Order) []field.Expr {
	var exprs []field.Expr
	m := dao.genQ.Client
