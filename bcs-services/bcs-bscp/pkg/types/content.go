...
// ListContentsOption defines options to list content.
type ListContentsOption struct {
...
// Validate the list content options
func (opt *ListContentsOption) Validate(po *PageOption) error {
	if opt.BizID <= 0 {
	...
	exprOpt := &filter.ExprOption{
		...
		RuleFields: table.ContentColumns.WithoutColumn("biz_id", "app_id"),
	}
...
// ListContentDetails defines the response details of requested ListContentsOption.
type ListContentDetails struct {
	Count   uint32           `json:"count"`
	Details []*table.Content `json:"details"`
}
