# 错误国际化
## 国际化使用方式
1. 在源码中有调用golang.org/x/text/message包中的`func (p *Printer) 
   Sprintf/Fprintf/Printf`三个方法中的任意一个
2. 首次部署项目或没有gotext命令行工具时执行`make init`，该操作会安装gotext命令行工具（该工具能自动从源码中提取要翻译的message以及合并message，并生成对应的目录文件）
3. 执行`make i18n`，从源码中提取要翻译的message以及合并message并更新到pkg/i18n目录下的对应文件中
4. 将pkg/i18n/translations/locales/zh/messages.gotext.json文件中未翻译的内容进行补充，补充完后再次执行`make i18n`，这时所有message均有了对应翻译，out.
   gotext.json和catalog.go文件也都被更新，重新编译项目后生效
5. http请求的header中，通过X-Bkapi-Language指定对应的语言，当前可以是zh（中文）或en（英文），不指定默认使用zh，从而获取对应语言的错误信息
...
### 3. 执行`make i18n`，从源码中提取要翻译的message以及合并message并更新到pkg/i18n目录下的对应文件中
以zh目录下的国际化文件为例，该操作会进行如下操作：
- 从源码中提取到要翻译的message，源码中任何调用了golang.org/x/text/message包中的`func (p *Printer)
  Sprintf/Fprintf/Printf`三个方法中任意一个的message均会被提取
- 更新所有语言的翻译内容映射关系到pkg/i18n/translations/locales/catalog.go文件
- **说明：后面的操作以zh目录下的文件为例，其他语言目录也是一样，因为源码语言用的en，翻译后的message和源码相同，所以en目录不需要做任何调整**
- 更新pkg/i18n/translations/locales/en/out.gotext.json文件
- 复制pkg/i18n/translations/locales/en/out.gotext.json文件到pkg/i18n/translations/locales/zh/messages.gotext.json
- **说明：重复执行，已翻译过的内容会依然存在** 
...
i18n:
	...
	@cp ./pkg/i18n/translations/locales/en/out.gotext.json ./pkg/i18n/translations/locales/en/messages.gotext.json
...
* -lang 指定要翻译的目标语言，这里是英语en和中文zh，这里将源码使用语言en也带上便于集中查看比较，多个语言之间以逗号分隔
...
执行操作后的示例, 输出有提示哪些message缺少对应的翻译：
```shell
$ make i18n
...
```
查看./pkg/i18n/translations/locales/zh/messages.gotext.json文件示例（out.gotext.json文件也一样），能看到translation字段为空，等待去填充翻译：
```json
{
    "language": "zh",
    "messages": [
...
### 4. 将pkg/i18n/translations/locales/zh/messages.gotext.json文件中未翻译的内容进行补充，补充完后再次执行`make i18n`，这时所有message均有了对应翻译，out.gotext.json和catalog.go文件也都被更新，重新编译项目后生效
