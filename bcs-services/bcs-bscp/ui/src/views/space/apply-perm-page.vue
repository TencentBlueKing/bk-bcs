<script setup lang="ts">
  ...
  const noSpaceViewPerm = computed(() => {
    return applyPermResource.value.findIndex(item => item.action === 'find_business_resource') > -1
  })

  const permTitle = computed(() => {
    if (applyPermResource.value.length > 0) { // 暂时只展示最上层权限名称
      return `无${applyPermResource.value[0].action_name}权限`
    }
    return '无访问权限'
  })
      ...
      <bk-exception type="403" scene="part">
        <div class="apply-description">
          ...
          <template v-if="noSpaceViewPerm">
            <p class="tips">你没有相应业务的访问权限，请前往申请相关业务权限</p>
          </template>
          <template>
          </template>
          <bk-button text theme="primary" @click="handleApply">申请权限</bk-button>
        </div>
      </bk-exception>
    </div>
</template>
