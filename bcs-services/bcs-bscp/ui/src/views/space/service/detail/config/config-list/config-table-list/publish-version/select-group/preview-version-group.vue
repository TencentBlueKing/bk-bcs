...
<template>
  <div class="version-callapse-item" :key="previewGroup.id">
    <div class="version-header" @click="fold = !fold">
      ...
      <div :class="['version-type-marking', previewGroup.type]">【{{ TYPE_MAP[previewGroup.type as keyof typeof TYPE_MAP] }}】</div>
      <span v-if="previewGroup.type === 'modify'" class="name"> - {{ previewGroup.name }}</span>
      <span class="group-count-wrapper">共 <span class="count">{{ previewGroup.children.length }}</span> 个分组</span>
      <bk-button
