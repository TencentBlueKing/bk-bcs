...
const isParentNodeChecked = (node: ITreeParentNodeData) =>
  node.children.length > 0 && node.children.every((group) => group.checked);
...
const handleClearAll = () => {
  const hasCheckedGroups = props.groupList.filter(
    (group) => props.disabled.includes(group.id) && props.value.findIndex((item) => item.id === group.id) > -1
  );
...
// 节点搜索
const handleSearch = (val: string, itemValue: string) => itemValue.toLowerCase().includes(val.toLowerCase());
