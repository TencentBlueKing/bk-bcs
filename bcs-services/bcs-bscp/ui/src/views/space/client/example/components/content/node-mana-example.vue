<template>
  <div class="node-mana-container">
    <div class="top-tip">
      节点管理插件客户端需要在<span class="em" @click="linkTo(linkUrl.nodeMana)">节点管理平台<share /></span>部署“bkbscp
      (bscp 服务配置分发和热更新)”插件，部署详情请参考产品白皮书：
      <span class="em" @click="linkTo(linkUrl.clientNode)">
        《客户端配置》-“节点管理插件客户端拉取配置”章节<share />
      </span>
    </div>
    <div class="preview-content">
      <bk-form label-width="145">
          ...
          <div class="service-content">
            <div class="service-item">
              ...
              <span class="bk-form-content">
                (通常用于按标签进行灰度发布，支持设置多个标签；如果不需要按标签灰度，可不设置)
              </span>
            </div>
            <div class="service-item">
        ...
        <bk-form-item label="业务ID：">
          <span class="content-em" @click="copyText(bizId)"> {{ bizId }} <copy-shape class="icon-shape" /> </span>
        </bk-form-item>
        <bk-form-item label="临时目录：">
          <span class="content-em" @click="copyText('/data/bscp')">/data/bscp <copy-shape class="icon-shape" /></span>
        </bk-form-item>
        <bk-form-item label="全局标签：">
          (全局标签与服务标签参数一样，常用于按标签进行灰度发布；不同的是全局标签可供多个服务共用)
        </bk-form-item>
        <bk-form-item label="服务密钥："> (即客户端密钥，需填写与此服务配置关联过的实际客户端密钥) </bk-form-item>
      </bk-form>
    </div>
  </div>
</template>
