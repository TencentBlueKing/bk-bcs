export default {
  ...
  文件大小不能超过100M: '文件大小不能超过100M',
  重置: '重置',
  configTips: {
    createConfig: `配置文件内支持引用全局变量与定义新的BSCP变量，变量规则如下
  ...
  '服务：': '服务：',
  '(通常用于按标签进行灰度发布，支持设置多个标签；如果不需要按标签灰度，可不设置)': '(通常用于按标签进行灰度发布，支持设置多个标签；如果不需要按标签灰度，可不设置)',
  '业务ID：': '业务ID：',
  '临时目录：': '临时目录：',
  '全局标签：': '全局标签：',
  '服务密钥：': '服务密钥：',
  ...
  '下载地址：': '下载地址：',
  '创建命令配置文件，配置文件为 YAML 格式，命名为：bscp.yaml': '创建命令配置文件，配置文件为 YAML 格式，命名为：bscp.yaml',
