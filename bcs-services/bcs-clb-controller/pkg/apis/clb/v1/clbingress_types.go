...
func (tls *ClbTls) Validate() error {
	if tls.Mode == cloudlbType.ClbListenerTLSModeUniDirectional {
		if len(tls.CertID) == 0 {
			if len(tls.CertServerKey) == 0 || len(tls.CertServerContent) == 0 || len(tls.CertServerName) == 0 {
				return fmt.Errorf("in unidirectional mode, need (certId) or (certServerName, certServerKey, certServerContent)")
			}
		}
		return nil
	} else if tls.Mode == cloudlbType.ClbListenerTLSModeMutual {
		if len(tls.CertID) == 0 {
			if len(tls.CertServerKey) == 0 || len(tls.CertServerContent) == 0 || len(tls.CertServerName) == 0 {
	...
	return fmt.Errorf("tls mode invalid: must be [%s, %s]", cloudlbType.ClbListenerTLSModeUniDirectional, cloudlbType.ClbListenerTLSModeMutual)
}
...
func (clbRule *ClbRule) Validate() error {
	...
	if clbRule.LbPolicy != nil {
		if clbRule.LbPolicy.Strategy != cloudlbType.ClbLBPolicyWRR &&
			clbRule.LbPolicy.Strategy != cloudlbType.ClbLBPolicyLeastConn &&
			clbRule.LbPolicy.Strategy != cloudlbType.ClbLBPolicyIPHash {
			return fmt.Errorf("invalid lb policy strategy %s", clbRule.LbPolicy.Strategy)
		}
	} else {
		clbRule.LbPolicy = &ClbLoadBalance{
			Strategy: cloudlbType.ClbLBPolicyWRR,
		}
	}
