/*
 * Tencent is pleased to support the open source community by making Blueking Container Service available.
 * Copyright (C) 2019 THL A29 Limited, a Tencent company. All rights reserved.
 * Licensed under the MIT License (the "License"); you may not use this file except
 * in compliance with the License. You may obtain a copy of the License at
 * http://opensource.org/licenses/MIT
 * Unless required by applicable law or agreed to in writing, software distributed under
 * the License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND,
 * either express or implied. See the License for the specific language governing permissions and
 * limitations under the License.
 *
 */
// Code generated by client-gen. DO NOT EDIT.

package v1

import (
	v1 "github.com/Tencent/bk-bcs/bcs-services/bcs-clb-controller/pkg/apis/network/v1"
	scheme "github.com/Tencent/bk-bcs/bcs-services/bcs-clb-controller/pkg/client/internalclientset/scheme"
	meta_v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	rest "k8s.io/client-go/rest"
)

// CloudListenersGetter has a method to return a CloudListenerInterface.
// A group's client should implement this interface.
type CloudListenersGetter interface {
	CloudListeners(namespace string) CloudListenerInterface
}

// CloudListenerInterface has methods to work with CloudListener resources.
type CloudListenerInterface interface {
	Create(*v1.CloudListener) (*v1.CloudListener, error)
	Update(*v1.CloudListener) (*v1.CloudListener, error)
	UpdateStatus(*v1.CloudListener) (*v1.CloudListener, error)
	Delete(name string, options *meta_v1.DeleteOptions) error
	DeleteCollection(options *meta_v1.DeleteOptions, listOptions meta_v1.ListOptions) error
	Get(name string, options meta_v1.GetOptions) (*v1.CloudListener, error)
	List(opts meta_v1.ListOptions) (*v1.CloudListenerList, error)
	Watch(opts meta_v1.ListOptions) (watch.Interface, error)
	Patch(name string, pt types.PatchType, data []byte, subresources ...string) (result *v1.CloudListener, err error)
	CloudListenerExpansion
}

// cloudListeners implements CloudListenerInterface
type cloudListeners struct {
	client rest.Interface
	ns     string
}

// newCloudListeners returns a CloudListeners
func newCloudListeners(c *NetworkV1Client, namespace string) *cloudListeners {
	return &cloudListeners{
		client: c.RESTClient(),
		ns:     namespace,
	}
}

// Get takes name of the cloudListener, and returns the corresponding cloudListener object, and an error if there is any.
func (c *cloudListeners) Get(name string, options meta_v1.GetOptions) (result *v1.CloudListener, err error) {
	result = &v1.CloudListener{}
	err = c.client.Get().
		Namespace(c.ns).
		Resource("cloudlisteners").
		Name(name).
		VersionedParams(&options, scheme.ParameterCodec).
		Do().
		Into(result)
	return
}

// List takes label and field selectors, and returns the list of CloudListeners that match those selectors.
func (c *cloudListeners) List(opts meta_v1.ListOptions) (result *v1.CloudListenerList, err error) {
	result = &v1.CloudListenerList{}
	err = c.client.Get().
		Namespace(c.ns).
		Resource("cloudlisteners").
		VersionedParams(&opts, scheme.ParameterCodec).
		Do().
		Into(result)
	return
}

// Watch returns a watch.Interface that watches the requested cloudListeners.
func (c *cloudListeners) Watch(opts meta_v1.ListOptions) (watch.Interface, error) {
	opts.Watch = true
	return c.client.Get().
		Namespace(c.ns).
		Resource("cloudlisteners").
		VersionedParams(&opts, scheme.ParameterCodec).
		Watch()
}

// Create takes the representation of a cloudListener and creates it.  Returns the server's representation of the cloudListener, and an error, if there is any.
func (c *cloudListeners) Create(cloudListener *v1.CloudListener) (result *v1.CloudListener, err error) {
	result = &v1.CloudListener{}
	err = c.client.Post().
		Namespace(c.ns).
		Resource("cloudlisteners").
		Body(cloudListener).
		Do().
		Into(result)
	return
}

// Update takes the representation of a cloudListener and updates it. Returns the server's representation of the cloudListener, and an error, if there is any.
func (c *cloudListeners) Update(cloudListener *v1.CloudListener) (result *v1.CloudListener, err error) {
	result = &v1.CloudListener{}
	err = c.client.Put().
		Namespace(c.ns).
		Resource("cloudlisteners").
		Name(cloudListener.Name).
		Body(cloudListener).
		Do().
		Into(result)
	return
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().

func (c *cloudListeners) UpdateStatus(cloudListener *v1.CloudListener) (result *v1.CloudListener, err error) {
	result = &v1.CloudListener{}
	err = c.client.Put().
		Namespace(c.ns).
		Resource("cloudlisteners").
		Name(cloudListener.Name).
		SubResource("status").
		Body(cloudListener).
		Do().
		Into(result)
	return
}

// Delete takes name of the cloudListener and deletes it. Returns an error if one occurs.
func (c *cloudListeners) Delete(name string, options *meta_v1.DeleteOptions) error {
	return c.client.Delete().
		Namespace(c.ns).
		Resource("cloudlisteners").
		Name(name).
		Body(options).
		Do().
		Error()
}

// DeleteCollection deletes a collection of objects.
func (c *cloudListeners) DeleteCollection(options *meta_v1.DeleteOptions, listOptions meta_v1.ListOptions) error {
	return c.client.Delete().
		Namespace(c.ns).
		Resource("cloudlisteners").
		VersionedParams(&listOptions, scheme.ParameterCodec).
		Body(options).
		Do().
		Error()
}

// Patch applies the patch and returns the patched cloudListener.
func (c *cloudListeners) Patch(name string, pt types.PatchType, data []byte, subresources ...string) (result *v1.CloudListener, err error) {
	result = &v1.CloudListener{}
	err = c.client.Patch(pt).
		Namespace(c.ns).
		Resource("cloudlisteners").
		SubResource(subresources...).
		Name(name).
		Body(data).
		Do().
		Into(result)
	return
}
