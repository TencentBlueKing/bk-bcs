...
var (
	// ExampleStepName step name for create cluster
	ExampleStepName = fedsteps.StepNames{
		Alias: "example step alias name",
		Name:  "EXAMPLE",
	}
)

// NewExampleStep sum step
func NewExampleStep() *ExampleStep {
	return &ExampleStep{}
}

// ExampleStep sum step
type ExampleStep struct{}

// Alias step name
func (s ExampleStep) Alias() string {
	return ExampleStepName.Alias
}

// GetName step name
func (s ExampleStep) GetName() string {
	return ExampleStepName.Name
}

// DoWork for worker exec task
func (s ExampleStep) DoWork(t *types.Task) error {
...
func (s ExampleStep) BuildStep(kvs []task.KeyValue, opts ...types.StepOption) *types.Step {
