...
func GetItems(obj map[string]interface{}, paths interface{}) (interface{}, error) {
	...
	default:
		return nil, errorx.New(0, "paths's type must one of (string, []string), get %v", t)
	}
}

func getItems(obj map[string]interface{}, paths []string) (interface{}, error) {
	if len(paths) == 0 {
		return nil, errorx.New(0, "paths is empty list")
	}
	ret, exists := obj[paths[0]]
	if !exists {
		return nil, errorx.New(0, "key %s not exist", paths[0])
	}
	if len(paths) == 1 {
		return ret, nil
	...
	return nil, errorx.New(0, "key %s, val not map[string]interface{} type", paths[0])
}
...
func SetItems(obj map[string]interface{}, paths interface{}, val interface{}) error {
	...
	default:
		return errorx.New(0, "paths's type must one of (string, []string), get %v", t)
	}
	return nil
}
...
func setItems(obj map[string]interface{}, paths []string, val interface{}) error {
	if len(paths) == 0 {
		return errorx.New(0, "paths is empty list")
	}
	if len(paths) == 1 {
		obj[paths[0]] = val
	} else if subMap, ok := obj[paths[0]].(map[string]interface{}); ok {
		...
		return errorx.New(0, "key %s not exists or obj[key] not map[string]interface{} type", paths[0])
	}
	return nil
}
