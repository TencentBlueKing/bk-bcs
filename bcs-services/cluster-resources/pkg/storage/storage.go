...
var (
	// ClusteredKind kind of clustered resource
	ClusteredKind = []string{"PersistentVolume", "StorageClass", "CustomResourceDefinition"}
)
...
func ListMultiClusterResources(ctx context.Context, req ListMultiClusterResourcesReq) ([]*Resource, int, error) {
	...
	// if kind is clustered resource, set namespaces to empty
	if slice.StringInSlice(req.Kind, ClusteredKind) {
		for i := range req.ClusteredNamespaces {
			req.ClusteredNamespaces[i].Namespaces = []string{}
		}
	}
