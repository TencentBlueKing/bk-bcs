          ...
          ui:component:
            ...
            props:
              ...
              remoteConfig:
                params:
                  format: selectItems
                url: "{{`{{`}} `${$context.baseUrl}/projects/${$context.projectID}/clusters/${$context.clusterID}/namespaces/${$self.getValue('data.saToken.namespace')}/rbac/service_accounts` {{`}}`}}"
          ui:reactions:
            - lifetime: init
              # 命名空间已经选中（有值）才加载 SA 数据
              if: "{{`{{`}} $self.getValue('data.saToken.namespace') {{`}}`}}"
              then:
                state:
                  disabled: false
                actions:
                  - "{{`{{`}} $loadDataSource {{`}}`}}"
            - source: "data.saToken.namespace"
              # 命名空间已经选中（有值）才加载 SA 数据
              if: "{{`{{`}} $self.getValue('data.saToken.namespace') {{`}}`}}"
              then:
                state:
