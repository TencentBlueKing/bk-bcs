    ...
    resName:
      ...
      ui:component:
        ...
        props:
          clearable: false
          remoteConfig:
            url: "{{`{{`}} `${$context.baseUrl}/projects/${$context.projectID}/clusters/${$context.clusterID}/res_select_items?namespace=${$self.getValue('metadata.namespace')}&kind=${$self.getValue('spec.ref.kind')}` {{`}}`}}"
      ui:reactions:
        - lifetime: init
          then:
            actions:
