...
func TestDS(t *testing.T) {
	...
	manifest, _ := example.LoadDemoManifest(ctx, "workload/simple_daemonset", "", "", resCsts.DS)
	...
	assert.Equal(t, "DaemonSetList", mapx.GetStr(respData, "manifest.kind"))
	...
	assert.Equal(t, "DaemonSet", mapx.GetStr(respData, "manifest.kind"))
	assert.Equal(t, "tVal", mapx.GetStr(respData, "manifest.spec.template.metadata.labels.tKey"))
