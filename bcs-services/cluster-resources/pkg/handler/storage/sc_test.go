...
func TestSC(t *testing.T) {
	...
	manifest, _ := example.LoadDemoManifest(ctx, "storage/simple_storage_class", "", "", resCsts.SC)
	...
	assert.Equal(t, "StorageClassList", mapx.GetStr(respData, "manifest.kind"))
	...
	assert.Equal(t, "StorageClass", mapx.GetStr(respData, "manifest.kind"))
