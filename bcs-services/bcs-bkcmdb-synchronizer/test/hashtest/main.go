...
func main() {
	...
	tmpMap["9.99.243.22"] = make([]string, 0)
	tmpMap["2.2.2.2"] = make([]string, 0)
	tmpMap["9.0.10.127"] = make([]string, 0)
	c := consistent.New()
	c.Add("9.99.243.22")
	c.Add("2.2.2.2")
	c.Add("9.0.10.127")
	...
	for _, u := range users {
		...
		fmt.Printf("%s => %s\n", u, server)
		tmpMap[server] = append(tmpMap[server], u)
	}
	for key, arr := range tmpMap {
		fmt.Printf("server %s, len %#v\n", key, arr)
	}
}
