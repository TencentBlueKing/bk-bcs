...
func (ca *CreateVirtualClusterAction) validate() error {
	...
	if ca.req.Ns.Annotations == nil {
		ca.req.Ns.Annotations = map[string]string{
			options.GetGlobalCMOptions().CommonConfig.AnnoKeyProjCode: ca.req.ProjectCode,
			utils.NamespaceCreator: ca.req.Creator,
		}
	} else {
		ca.req.Ns.Annotations[options.GetGlobalCMOptions().CommonConfig.AnnoKeyProjCode] = ca.req.ProjectCode
