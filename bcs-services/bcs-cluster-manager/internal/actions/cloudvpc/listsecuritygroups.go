...
type ListSecurityGroupsAction struct {
	...
	req            *cmproto.ListSecurityGroupsRequest
	resp           *cmproto.ListSecurityGroupsResponse
...
func (la *ListSecurityGroupsAction) getClusterBasicInfo() error {
	...
	if err != nil {
		blog.Errorf("get Cluster %s provider %s and cloud failed, %s",
...
func (la *ListSecurityGroupsAction) Handle(
	ctx context.Context, req *cmproto.ListSecurityGroupsRequest, resp *cmproto.ListSecurityGroupsResponse) {
	if req == nil || resp == nil {
	...
	if err != nil {
		blog.Errorf("get cloudprovider %s VPCManager for list security groups in cluster %s failed, %s",
	...
	if err != nil {
		blog.Errorf("get credential for cloudprovider %s/%s list security groups in cluster %s failed, %s",
	...
	sg, err := vpcMgr.ListSecurityGroups(cmOption)
	if err != nil {
		la.setResp(common.BcsErrClusterManagerCloudProviderErr, err.Error())
		return
	}
	la.securityGroups = sg
