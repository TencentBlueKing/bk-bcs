...
func (ua *UpdateKubeconfigAction) updateClusterCredential() error {
	...
	if err != nil {
		blog.Errorf("get cluster %s relative cloud provider %s failed, %s",
			ua.req.ClusterID, ua.cloud.CloudProvider, err.Error())
		return err
	}
	...
	cmOption.Region = ua.cluster.GetRegion()

	err = provider.UpdateCloudKubeConfig(ua.req.KubeConfig,
...
func (ua *UpdateKubeconfigAction) Handle(ctx context.Context, req *cmproto.UpdateClusterKubeConfigReq, // nolint
	...
	err := ua.updateClusterCredential()
	if err != nil {
