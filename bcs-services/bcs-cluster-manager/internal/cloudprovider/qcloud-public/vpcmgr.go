...
func (c *VPCManager) ListVpcsByPage(opt *cloudprovider.ListNetworksOption) (int64, []*proto.CloudVpcs, error) {
	...
	for _, v := range vpcs.VpcSet {
		underlayNums, err :=
			getIpNumsAndCidr(&opt.CommonOption, utils.StringPtrToString(v.VpcId), 0)
		if err != nil {
			return 0, nil, err
		}
		overlayNums, err :=
			getIpNumsAndCidr(&opt.CommonOption, utils.StringPtrToString(v.VpcId), 1)
		if err != nil {
			return 0, nil, err
		}
		result = append(result, &proto.CloudVpcs{
			...
			AvailableOverlayIpNum:  overlayNums.AvailableIpAddressCount,
			AvailableOverlayCidr:   overlayNums.AvailableCidrBlock,
			TotalOverlayIpNum:      overlayNums.TotalIpAddressCount,
			UnderlayCidr:           underlayNums.CidrBlock,
			AvailableUnderlayIpNum: underlayNums.AvailableIpAddressCount,
			AvailableUnderlayCidr:  underlayNums.AvailableCidrBlock,
			TotalUnderlayIpNum:     underlayNums.TotalIpAddressCount,
...
func getIpNumsAndCidr(
	...
	case 1:
		// 获取可用网段、ip总数量、总网段
		ipInfo, err := business.GetOverlayIPNetsAndNums(opt, vpcId)
		if err != nil {
			return nil, err
		}
