...
func ImportClusterNodesTask(taskID string, stepName string) error {
	...
	if err != nil {
		cloudprovider.GetStorageModel().CreateTaskStepLogError(context.Background(), taskID, stepName, fmt.Sprintf("import cluster instances failed [%s]", err))
	...
	if err != nil {
		cloudprovider.GetStorageModel().CreateTaskStepLogError(context.Background(), taskID, stepName, fmt.Sprintf("update cluster master nodes info failed [%s]", err))
		return err
	}
...
func RegisterClusterKubeConfigTask(taskID string, stepName string) error {
	...
	if err != nil {
		cloudprovider.GetStorageModel().CreateTaskStepLogError(context.Background(), taskID, stepName, fmt.Sprintf("get tke external cluster endpoint failed [%s]", err))
	...
	cloudprovider.GetStorageModel().CreateTaskStepLogInfo(context.Background(), taskID, stepName, "get tke external cluster endpoint successful")
