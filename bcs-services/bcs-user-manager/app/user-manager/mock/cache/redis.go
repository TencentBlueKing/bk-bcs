...
func (m *MockCache) Set(_ context.Context, key string, value interface{}, expiration time.Duration) (string, error) {
	args := m.Called(key, value, expiration)
	return args.String(0), args.Error(1)
}

// SetNX mock sets a key-value pair in the cache when the key does not exist
func (m *MockCache) SetNX(_ context.Context, key string, value interface{}, expiration time.Duration) (bool, error) {
	args := m.Called(key, value, expiration)
	return args.Bool(0), args.Error(1)
}

// SetEX mock sets a key-value pair in the cache when the key is exist
func (m *MockCache) SetEX(_ context.Context, key string, value interface{}, expiration time.Duration) (string, error) {
	args := m.Called(key, value, expiration)
	return args.String(0), args.Error(1)
}
...
func (m *MockCache) Del(_ context.Context, key string) (uint64, error) {
	args := m.Called(key)
	return args.Get(0).(uint64), args.Error(1)
}

// Expire mock set a key's expiration time
func (m *MockCache) Expire(_ context.Context, key string, expiration time.Duration) (bool, error) {
