...
type UpgradeHelper interface {
	...
	HelperName() string
	HttpRequest(method, url string, payload []byte) ([]byte, error)
	ClusterManagerRequest(method, url string, payload []byte) ([]byte, error)
	SetSsmToken() error
}
...
type Helper struct {
	DB     drivers.DB
	config options.HttpCliRequestConfig
...
func NewUpgradeHelper(opt *HelperOpt) *Helper {
	...
	clusterManagerCli.SetTlsVerityConfig(tlsConf)
