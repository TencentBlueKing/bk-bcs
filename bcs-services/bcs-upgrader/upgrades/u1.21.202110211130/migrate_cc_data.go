...
func migrateCCData(ctx context.Context, helper upgrader.UpgradeHelper) error {
	...
	allProject, err := getAllProjects(h)
	if err != nil {
...
func migrateProjectData(data ccProject, helper *upgrader.Helper) error {

	var project *bcsProject
	var err error

	project, err = findProject(data.ProjectID, helper)
	if err != nil {
	...
	updateProjectData, err := getUpdateProjectData(data, *project)
	if err != nil {
		blog.Errorf("diff project data failed, err: %v", err)
	}
	if updateProjectData == nil {
		return nil
	}
...
func migrateClusterData(projectID, ccAppID string, helper *upgrader.Helper) error {
	...
	for _, clusterData := range allClusterData {
		if clusterData.ProjectID != projectID {
			continue
		}
		for _, list := range clusterData.ClusterList {

			cluster, err := genCluster(projectID, list.ClusterID, ccAppID, helper)
			if err != nil {
				blog.Errorf("gen cluster(%s) data failed, err: %v", list.ClusterID, err)
				continue
			}

			bcsCluster, err := findCluster(list.ClusterID, helper)
			if err != nil {
				blog.Errorf("get bcs cluster(%s) data failed, err: %v", list.ClusterID, err)
				...
				// 对比cluster数据
				upData := getUpdateClusterData(*cluster, *bcsCluster)
				if upData == nil {
					continue
				}
				if err = updateCluster(*upData, helper); err != nil {
					blog.Errorf("migrate cluster(%s) data failed, err: %v", list.ClusterID, err)
					continue
				}
			}

			if err = migrateNodeData(projectID, list.ClusterID, helper); err != nil {
				blog.Errorf("migrate cluster(%s) node failed, err: %v", list.ClusterID, err)
			}
		}
	}
...
func migrateNodeData(projectID, clusterID string, helper *upgrader.Helper) error {
	...
	if createNodeData != nil {
		...
		if err != nil {
			blog.Errorf("create cluster(%s) node data failed, clusterID, err: %v", clusterID, err)
		}
	}
	if deleteNodeData != nil {
		err = deleteNode(*deleteNodeData, helper)
		if err != nil {
			blog.Errorf("delete cluster(%s) node data failed, clusterID, err: %v", clusterID, err)
		}
	}
...
// getUpdateClusterData :通过比较bcs cc和bcs中的数据，得到待更新的cluster数据
func getUpdateClusterData(ccData bcsReqCreateCluster, bcsData bcsRespFindCluster) *bcsReqUpdateCluster {

	upData := &bcsReqUpdateCluster{
...
func genCluster(projectID, clusterID, ccAppID string, helper *upgrader.Helper) (*bcsReqCreateCluster, error) {
...
// getUpdateProjectData :通过对比bcs cc和bcs中project数据，得到待更新的project数据
func getUpdateProjectData(ccData ccProject, bcsData bcsProject) (*bcsProject, error) {
	...
	case 2:
		...
		break
	default:
		return nil, fmt.Errorf("kind(%d) failed", ccData.Kind)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("deployType(%s) failed", ccData.DeployType)
	}
...
// diffNode :通过对比bcs cc和bcs中节点IP，得到待创建和待删除的几点
