...
func request2metrics(req *restful.Request, uri string) (respBody []byte, err error) {
	...
	serv, err := rd.GetModuleServers(types.BCS_MODULE_METRICSERVICE)
	if err != nil {
		blog.Error("get servers %s error %s", types.BCS_MODULE_METRICSERVICE, err.Error())
		err1 := bhttp.InternalError(common.BcsErrApiGetMetricsFail, common.BcsErrApiGetMetricsFailStr)
		return nil, err1
	}

	ser, ok := serv.(*types.MetricServiceInfo)
	if !ok {
		blog.Errorf("servers convert to MetricService")
		err1 := bhttp.InternalError(common.BcsErrApiGetMetricsFail, common.BcsErrApiGetMetricsFailStr)
		return nil, err1
	}

	host := fmt.Sprintf("%s://%s:%d", ser.Scheme, ser.IP, ser.Port)
	url := fmt.Sprintf("%s/api/v1/metric/%s", host, uri)
	...
	blog.Info("request metric, url: %s, method: %s", url, req.Request.Method)
	if len(errs) != 0 {
...
func init() {
	actions.RegisterAction(actions.Action{"POST", "/bcsapi/v4/metric/{uri:*}", nil, handlerActions})
	actions.RegisterAction(actions.Action{"PUT", "/bcsapi/v4/metric/{uri:*}", nil, handlerActions})
	actions.RegisterAction(actions.Action{"GET", "/bcsapi/v4/metric/{uri:*}", nil, handlerActions})
	actions.RegisterAction(actions.Action{"DELETE", "/bcsapi/v4/metric/{uri:*}", nil, handlerActions})
}
