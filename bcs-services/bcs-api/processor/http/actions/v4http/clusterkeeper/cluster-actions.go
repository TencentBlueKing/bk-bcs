...
func request2clusterkeeper(req *restful.Request, uri string) (respBody []byte, err error) {
	...
	serv, err := rd.GetModuleServers(types.BCS_MODULE_CLUSTERKEEPER)
	if err != nil {
		blog.Error("get servers %s error %s", types.BCS_MODULE_CLUSTERKEEPER, err.Error())
		err1 := bhttp.InternalError(common.BcsErrApiGetNetserviceFail, "clusterkeeper not found")
		return nil, err1
	}

	ser, ok := serv.(*types.ClusterKeeperServInfo)
	if !ok {
		blog.Errorf("servers convert to ClusterKeeperServInfo")
		err1 := bhttp.InternalError(common.BcsErrApiGetNetserviceFail, "clusterkeeper not found")
		return nil, err1
	}

	host := fmt.Sprintf("%s://%s:%d", ser.Scheme, ser.IP, ser.Port)
	url := fmt.Sprintf("%s/bcsclusterkeeper/v1/%s", host, uri)
