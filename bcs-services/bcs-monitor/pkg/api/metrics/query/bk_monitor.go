...
func (h BKMonitorHandler) handleBKMonitorClusterMetric(c *rest.Context, promql string) (promclient.ResultData, error) {
	...
	ipMatch, nodeNameMatch, err := GetMasterNodeMatch(c.Request.Context(), c.ClusterId)
	if err != nil {
		return promclient.ResultData{}, err
	}
...
func (h BKMonitorHandler) ClusterDiskUsage(c *rest.Context) (promclient.ResultData, error) {
	promql := `(sum(avg_over_time(bkmonitor:node_filesystem_size_bytes{bcs_cluster_id="%<clusterID>s", bk_instance!~"%<instance>s", fstype=~"%<fstype>s", mountpoint=~"%<mountpoint>s"}[2m])) - ` +
		`sum(avg_over_time(bkmonitor:node_filesystem_free_bytes{bcs_cluster_id="%<clusterID>s", bk_instance!~"%<instance>s", fstype=~"%<fstype>s", mountpoint=~"%<mountpoint>s"}[2m]))) / ` +
		`sum(avg_over_time(bkmonitor:node_filesystem_size_bytes{bcs_cluster_id="%<clusterID>s", bk_instance!~"%<instance>s", fstype=~"%<fstype>s", mountpoint=~"%<mountpoint>s"}[2m])) * 100`
...
func (h BKMonitorHandler) ClusterDiskioUsage(c *rest.Context) (promclient.ResultData, error) {
	promql := `sum(max by(bk_instance) (rate(bkmonitor:node_disk_io_time_seconds_total{bcs_cluster_id="%<clusterID>s", bk_instance!~"%<instance>s"}[2m]))) / ` +
		`count(max by(bk_instance) (rate(bkmonitor:node_disk_io_time_seconds_total{bcs_cluster_id="%<clusterID>s", bk_instance!~"%<instance>s"}[2m])))`
