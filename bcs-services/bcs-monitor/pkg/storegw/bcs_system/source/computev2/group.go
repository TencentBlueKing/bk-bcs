...
func (m *ComputeV2) handleGroupMetric(ctx context.Context, projectID, clusterID, group string,
...
func (m *ComputeV2) GetClusterGroupNodeNum(ctx context.Context, projectID, clusterID, group string, start,
	...
	promql :=
		`%<prefix>sbkbcs_clustermanager_cluster_group_nodeNum{cluster_id="%<clusterID>s", group="%<group>s", ` +
			`%<provider>s}`
...
func (m *ComputeV2) GetClusterGroupMaxNodeNum(ctx context.Context, projectID, clusterID, group string, start,
	...
	promql :=
		`%<prefix>sbkbcs_clustermanager_cluster_group_maxNodeNum{cluster_id="%<clusterID>s", group="%<group>s", ` +
			`%<provider>s}[2m]`
	return m.handleGroupMetric(ctx, projectID, clusterID, group, promql, start, end, step)
}
