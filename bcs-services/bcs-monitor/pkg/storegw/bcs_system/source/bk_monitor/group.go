...
// handleGroupMetric xxx
func (m *BKMonitor) handleGroupMetric(ctx context.Context, projectID, clusterID, group string, promql string, start,
...
func (m *BKMonitor) GetClusterGroupNodeNum(ctx context.Context, projectID, clusterID, group string, start,
	end time.Time, step time.Duration) ([]*prompb.TimeSeries, error) {
	promql :=
...
func (m *BKMonitor) GetClusterGroupMaxNodeNum(ctx context.Context, projectID, clusterID, group string, start,
	end time.Time, step time.Duration) ([]*prompb.TimeSeries, error) {
	promql :=
