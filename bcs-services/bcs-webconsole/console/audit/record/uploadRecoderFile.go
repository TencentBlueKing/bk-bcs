	...
	for {
		...
		case <-timer.C:
			...
			for path, localFiles := range pf {
				remoteFiles, err := storage.ListFile(ctx, path)
				if err != nil {
...
func getRecordEnd(f string) (bool, error) {
	...
	for {
		...
		if err != nil {
			return false, fmt.Errorf("roll back file pointer err: %v\n", err)
		}
...
func writeRecordEnd(f string) error {
	...
	if err != nil {
		return fmt.Errorf("move to file end err:%v\n", err)
	}
	_, err = file.Write(lastLine)
	if err != nil {
		return fmt.Errorf("write last line err:%v\n", err)
	}
	return nil
}

func uploadRemain(ctx context.Context, data_dir string, remainFiles map[string][]string, storage repository.Provider) error {
	for path, files := range remainFiles {
		for _, file := range files {
			localPath := filepath.Join(data_dir, path, file)
			end, err := getRecordEnd(localPath)
			if err != nil {
				return fmt.Errorf("read file last line err: %v\n", err)
			}
			if end {
