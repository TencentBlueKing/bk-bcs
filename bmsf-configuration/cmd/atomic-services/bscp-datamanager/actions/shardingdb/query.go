...
// QueryAction is shardingdb query action object.
...
func (act *QueryAction) queryShardingDB() (pbcommon.ErrCode, string) {
	db, err := act.smgr.QueryShardingDB(act.req.DbId)
	if err == dbsharding.RECORDNOTFOUND {
		return pbcommon.ErrCode_E_DM_NOT_FOUND, "sharding database non-exist."
	}
	if err != nil {
...
func (act *QueryAction) Do() error {
	// query shardingdb.
	if errCode, errMsg := act.queryShardingDB(); errCode != pbcommon.ErrCode_E_OK {
