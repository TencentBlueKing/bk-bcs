...
func NewAction(ctx context.Context, viper *viper.Viper, smgr *dbsharding.ShardingManager,
	req *pb.HealthzReq, resp *pb.HealthzResp) *Action {
	action := &Action{ctx: ctx, viper: viper, smgr: smgr, req: req, resp: resp}
...
func (act *Action) healthzDB() {
	...
	if err := sd.DB().Exec("SELECT 1;").Error; err != nil {
		info.IsHealthy = false
		info.Message = err.Error()
		return
	}
