...
// ListAction is content list action object.
...
func (act *ListAction) Output() error {
	contentList := []*pbcommon.Content{}
	for _, st := range act.contentList {
		content := &pbcommon.Content{
			...
			ContentSize:  uint32(st.ContentSize),
			Creator:      st.Creator,
	...
	act.resp.Data = &pb.QueryConfigContentListResp_RespData{TotalCount: uint32(act.totalCount), Info: contentList}
	return nil
}
...
func (act *ListAction) queryConfigContentCount() (pbcommon.ErrCode, string) {
...
func (act *ListAction) queryConfigContentList() (pbcommon.ErrCode, string) {
	err := act.sd.DB().
...
func (act *ListAction) Do() error {
	...
	// query config content count.
	if errCode, errMsg := act.queryConfigContentCount(); errCode != pbcommon.ErrCode_E_OK {
		return act.Err(errCode, errMsg)
	}

	// query config content list.
	if errCode, errMsg := act.queryConfigContentList(); errCode != pbcommon.ErrCode_E_OK {
		return act.Err(errCode, errMsg)
	}
	return nil
}
