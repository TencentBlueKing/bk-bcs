...
// ListAction list target variable group objects.
...
func (act *ListAction) Output() error {
	act.resp.Data = &pb.QueryVariableGroupListResp_RespData{TotalCount: act.totalCount, Info: act.variableGroups}
	return nil
}
...
func (act *ListAction) queryVariableGroupList() (pbcommon.ErrCode, string) {
	r := &pbdatamanager.QueryVariableGroupListReq{
		Seq:   act.kit.Rid,
		BizId: act.req.BizId,
		Page:  act.req.Page,
	}
	...
	logger.V(4).Infof("QueryVariableGroupList[%s]| request to DataManager, %+v", r.Seq, r)

	resp, err := act.dataMgrCli.QueryVariableGroupList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_TPL_SYSTEM_UNKNOWN,
			fmt.Sprintf("request to DataManager QueryVariableGroupList, %+v", err)
	}
	if resp.Code != pbcommon.ErrCode_E_OK {
		return resp.Code, resp.Message
	}
	act.totalCount = resp.Data.TotalCount
	act.variableGroups = resp.Data.Info
