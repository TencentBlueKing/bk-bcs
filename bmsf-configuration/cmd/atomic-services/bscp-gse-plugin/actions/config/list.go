...
func NewListAction(ctx context.Context, viper *safeviper.SafeViper, gseTunnel *tunnel.Tunnel,
	req *pb.PullConfigListReq, resp *pb.PullConfigListResp) *ListAction {
	action := &ListAction{ctx: ctx, viper: viper, gseTunnel: gseTunnel, req: req, resp: resp}
...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	req := &pbtunnelserver.GTCMDPullConfigListReq{
		Seq:   act.req.Seq,
		BizId: act.req.BizId,
		AppId: act.req.AppId,
		Page:  act.req.Page,
	...
	resp, err := act.gseTunnel.PullConfigList(messageID, req)
	if err == types.ErrorTimeout {
		return pbcommon.ErrCode_E_TIMEOUT, "timeout"
	}
	if err != nil {
		return pbcommon.ErrCode_E_CONNS_SYSTEM_UNKNOWN, fmt.Sprintf("request to gse tunnel QueryConfigList, %+v", err)
	}

	if resp.Code != pbcommon.ErrCode_E_OK {
		return resp.Code, resp.Message
	}
	act.resp.Configs = resp.Configs
