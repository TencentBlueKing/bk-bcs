...
func (c *config) check() error {
	...
	c.viper.SetDefault("server.endpoint.port", 59516)
	...
	c.viper.BindEnv("server.gseTunnelTimeout", c.envName("GSE_TUNNEL_TIMEOUT"))
	c.viper.SetDefault("server.gseTunnelTimeout", types.RPCNormalTimeout)

	c.viper.BindEnv("server.gseTunnelProcesserNum", c.envName("GSE_TUNNEL_PROCESSER_NUM"))
	c.viper.SetDefault("server.gseTunnelProcesserNum", 100)

	c.viper.BindEnv("metrics.endpoint", c.envName("METRICS_ENDPOINT"))
	c.viper.SetDefault("metrics.endpoint", common.Endpoint(c.viper.GetString("server.endpoint.ip"), 59517))

	c.viper.BindEnv("metrics.path", c.envName("METRICS_PATH"))
	c.viper.SetDefault("metrics.path", "/metrics")

	// sidecar instance configs.
	...
	// local gse plugin connserver configs, serve for container and local plugin processer.
	c.viper.BindEnv("connserver.hostName", c.envName("CONNSERVER_HOSTNAME"))
	c.viper.SetDefault("connserver.hostName", c.viper.GetString("server.endpoint.ip"))

	c.viper.BindEnv("connserver.port", c.envName("CONNSERVER_PORT"))
	c.viper.SetDefault("connserver.port", c.viper.GetInt("server.endpoint.port"))
	...
	c.viper.SetDefault("appinfo.ip", c.viper.GetString("server.endpoint.ip"))
	...
	c.viper.SetDefault("logger.directory", "./bscp-log")
