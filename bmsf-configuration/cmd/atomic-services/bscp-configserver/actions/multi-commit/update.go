...
func (act *UpdateAction) queryApp() (pbcommon.ErrCode, string) {
	...
	logger.V(4).Infof("UpdateMultiCommit[%s]| request to datamanager, %+v", r.Seq, r)
...
func (act *UpdateAction) update() (pbcommon.ErrCode, string) {
	...
	if err != nil {
		return pbcommon.ErrCode_E_CS_SYSTEM_UNKNOWN, fmt.Sprintf("request to datamanager UpdateMultiCommit, %+v", err)
	}
	...
	audit.Audit(int32(pbcommon.SourceType_ST_MULTI_COMMIT), int32(pbcommon.SourceOpType_SOT_UPDATE),
		act.req.BizId, act.req.MultiCommitId, act.kit.User, act.req.Memo)
