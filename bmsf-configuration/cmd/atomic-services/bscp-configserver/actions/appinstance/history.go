...
func NewHistoryAction(kit kit.Kit, viper *viper.Viper, dataMgrCli pbdatamanager.DataManagerClient,
	req *pb.QueryHistoryAppInstancesReq, resp *pb.QueryHistoryAppInstancesResp) *HistoryAction {
	action := &HistoryAction{kit: kit, viper: viper, dataMgrCli: dataMgrCli, req: req, resp: resp}
...
func (act *HistoryAction) history() (pbcommon.ErrCode, string) {
	...
	if err != nil {
		return pbcommon.ErrCode_E_CS_SYSTEM_UNKNOWN,
			fmt.Sprintf("request to datamanager QueryHistoryAppInstances, %+v", err)
	}
	if resp.Code != pbcommon.ErrCode_E_OK {
		return resp.Code, resp.Message
	}
	act.resp.Data = &pb.QueryHistoryAppInstancesResp_RespData{TotalCount: resp.Data.TotalCount, Info: resp.Data.Info}
