...
// DeleteAction deletes target config object.
...
func NewDeleteAction(kit kit.Kit, viper *viper.Viper,
	...
	req *pb.DeleteConfigReq, resp *pb.DeleteConfigResp) *DeleteAction {
...
func (act *DeleteAction) queryApp() (pbcommon.ErrCode, string) {
	...
	logger.V(4).Infof("DeleteConfig[%s]| request to datamanager, %+v", r.Seq, r)
...
func (act *DeleteAction) delete() (pbcommon.ErrCode, string) {
	r := &pbdatamanager.DeleteConfigReq{
		Seq:      act.kit.Rid,
		BizId:    act.req.BizId,
		CfgId:    act.req.CfgId,
	...
	resp, err := act.dataMgrCli.DeleteConfig(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_CS_SYSTEM_UNKNOWN, fmt.Sprintf("request to datamanager DeleteConfig, %+v", err)
	}
	if resp.Code != pbcommon.ErrCode_E_OK {
		return resp.Code, resp.Message
	}

	// audit here on config deleted.
	audit.Audit(int32(pbcommon.SourceType_ST_CONFIG), int32(pbcommon.SourceOpType_SOT_DELETE),
		act.req.BizId, act.req.CfgId, act.kit.User, "")
