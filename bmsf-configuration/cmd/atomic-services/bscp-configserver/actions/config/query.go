...
func (act *QueryAction) verify() error {
	...
	if len(act.req.CfgId) == 0 && len(act.req.Name) == 0 {
		return errors.New("invalid input data, cfg_id or name/fpath is required")
	}
...
func (act *QueryAction) query() (pbcommon.ErrCode, string) {
	...
	logger.V(4).Infof("QueryConfig[%s]| request to datamanager, %+v", r.Seq, r)
