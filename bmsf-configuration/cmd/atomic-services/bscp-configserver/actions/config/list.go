...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	...
	logger.V(4).Infof("QueryConfigList[%s]| request to datamanager, %+v", r.Seq, r)

	resp, err := act.dataMgrCli.QueryConfigList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_CS_SYSTEM_UNKNOWN, fmt.Sprintf("request to datamanager QueryConfigList, %+v", err)
	}
	if resp.Code != pbcommon.ErrCode_E_OK {
		return resp.Code, resp.Message
	}
	act.resp.Data = &pb.QueryConfigListResp_RespData{TotalCount: resp.Data.TotalCount, Info: resp.Data.Info}
