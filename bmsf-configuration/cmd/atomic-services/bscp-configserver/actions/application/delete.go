...
func (act *DeleteAction) queryApp() (pbcommon.ErrCode, string) {
	...
	logger.V(4).Infof("DeleteApp[%s]| request to datamanager, %+v", r.Seq, r)
...
func (act *DeleteAction) delete() (pbcommon.ErrCode, string) {
	...
	if err != nil {
		return pbcommon.ErrCode_E_CS_SYSTEM_UNKNOWN, fmt.Sprintf("request to datamanager DeleteApp, %+v", err)
	}
	...
	audit.Audit(int32(pbcommon.SourceType_ST_APP), int32(pbcommon.SourceOpType_SOT_DELETE),
		act.req.BizId, act.req.AppId, act.kit.User, "")
