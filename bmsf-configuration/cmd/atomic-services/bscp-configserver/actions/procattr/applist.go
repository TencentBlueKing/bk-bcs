...
func NewAppListAction(kit kit.Kit, viper *viper.Viper, dataMgrCli pbdatamanager.DataManagerClient,
	req *pb.QueryAppProcAttrListReq, resp *pb.QueryAppProcAttrListResp) *AppListAction {
	action := &AppListAction{kit: kit, viper: viper, dataMgrCli: dataMgrCli, req: req, resp: resp}
...
func (act *AppListAction) list() (pbcommon.ErrCode, string) {
	r := &pbdatamanager.QueryAppProcAttrListReq{
		Seq:   act.kit.Rid,
		BizId: act.req.BizId,
		AppId: act.req.AppId,
		Page:  act.req.Page,
	...
	logger.V(4).Infof("QueryAppProcAttrList[%s]| request to datamanager, %+v", r.Seq, r)

	resp, err := act.dataMgrCli.QueryAppProcAttrList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_CS_SYSTEM_UNKNOWN,
			fmt.Sprintf("request to datamanager QueryAppProcAttrList, %+v", err)
	}
	if resp.Code != pbcommon.ErrCode_E_OK {
		return resp.Code, resp.Message
	}
	act.resp.Data = &pb.QueryAppProcAttrListResp_RespData{TotalCount: resp.Data.TotalCount, Info: resp.Data.Info}
