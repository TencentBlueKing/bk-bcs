...
func (act *QueryAction) query() (pbcommon.ErrCode, string) {
	...
	logger.V(4).Infof("QueryHostProcAttr[%s]| request to datamanager, %+v", r.Seq, r)

	resp, err := act.dataMgrCli.QueryHostProcAttr(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_CS_SYSTEM_UNKNOWN, fmt.Sprintf("request to datamanager QueryHostProcAttr, %+v", err)
	}
