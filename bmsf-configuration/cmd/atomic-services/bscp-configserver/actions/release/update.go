...
func (act *UpdateAction) queryApp() (pbcommon.ErrCode, string) {
	...
	logger.V(4).Infof("UpdateRelease[%s]| request to datamanager, %+v", r.Seq, r)
...
func (act *UpdateAction) update() (pbcommon.ErrCode, string) {
	...
	if err != nil {
		return pbcommon.ErrCode_E_CS_SYSTEM_UNKNOWN, fmt.Sprintf("request to datamanager UpdateRelease, %+v", err)
	}
	...
	audit.Audit(int32(pbcommon.SourceType_ST_RELEASE), int32(pbcommon.SourceOpType_SOT_UPDATE),
		act.req.BizId, act.req.ReleaseId, act.kit.User, act.req.Memo)
