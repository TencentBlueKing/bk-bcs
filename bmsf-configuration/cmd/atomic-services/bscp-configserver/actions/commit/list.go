...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	r := &pbdatamanager.QueryHistoryCommitsReq{
		Seq:       act.kit.Rid,
		BizId:     act.req.BizId,
		AppId:     act.req.AppId,
		CfgId:     act.req.CfgId,
	...
	if err != nil {
		return pbcommon.ErrCode_E_CS_SYSTEM_UNKNOWN, fmt.Sprintf("request to datamanager QueryHistoryCommits, %+v", err)
	}
	if resp.Code != pbcommon.ErrCode_E_OK {
		return resp.Code, resp.Message
	}
	act.resp.Data = &pb.QueryHistoryCommitsResp_RespData{TotalCount: resp.Data.TotalCount, Info: resp.Data.Info}
