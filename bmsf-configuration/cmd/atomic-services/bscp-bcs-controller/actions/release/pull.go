...
func (act *PullAction) targetRelease() (pbcommon.ErrCode, string) {
	...
	if release.BizId != act.req.BizId {
		return pbcommon.ErrCode_E_BCS_SYSTEM_UNKNOWN, "can't pull target release, inconsistent bizid"
	}
	if release.AppId != act.req.AppId {
		return pbcommon.ErrCode_E_BCS_SYSTEM_UNKNOWN, "can't pull target release, inconsistent appid"
	}
	if release.CfgId != act.req.CfgId {
		return pbcommon.ErrCode_E_BCS_SYSTEM_UNKNOWN, "can't pull target release, inconsistent cfgid"
	}
	if release.State != int32(pbcommon.ReleaseState_RS_PUBLISHED) {
		return pbcommon.ErrCode_E_BCS_SYSTEM_UNKNOWN, "can't pull target release, not published"
	}
	...
	if err := json.Unmarshal([]byte(release.Strategies), &strategies); err != nil {
		return pbcommon.ErrCode_E_BCS_SYSTEM_UNKNOWN, err.Error()
	}
...
func (act *PullAction) newestRelease() (pbcommon.ErrCode, string) {
	...
	for {
		...
		if err != nil {
			return pbcommon.ErrCode_E_BCS_SYSTEM_UNKNOWN,
		...
		for _, release := range resp.Data.Info {
			if release.BizId != act.req.BizId {
				return pbcommon.ErrCode_E_BCS_SYSTEM_UNKNOWN, "can't pull newest release, inconsistent bizid"
			}
			if release.AppId != act.req.AppId {
				return pbcommon.ErrCode_E_BCS_SYSTEM_UNKNOWN, "can't pull newest release, inconsistent appid"
			}
			if release.CfgId != act.req.CfgId {
				return pbcommon.ErrCode_E_BCS_SYSTEM_UNKNOWN, "can't pull newest release, inconsistent cfgid"
			}
			if release.State != int32(pbcommon.ReleaseState_RS_PUBLISHED) {
				return pbcommon.ErrCode_E_BCS_SYSTEM_UNKNOWN, "can't pull newest release, not published"
			}
