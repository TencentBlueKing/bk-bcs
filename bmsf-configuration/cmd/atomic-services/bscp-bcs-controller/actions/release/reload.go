...
func (act *ReloadAction) queryMultiRelease() (pbcommon.ErrCode, string) {
	...
	if err != nil {
		return pbcommon.ErrCode_E_BCS_SYSTEM_UNKNOWN, fmt.Sprintf("request to datamanager QueryMultiRelease, %+v", err)
	}
...
func (act *ReloadAction) reload() (pbcommon.ErrCode, string) {
	publishing := &pbcommon.Publishing{ReloadSpec: act.req.ReloadSpec}
