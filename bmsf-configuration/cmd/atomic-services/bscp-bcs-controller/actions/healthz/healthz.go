...
func (act *Action) healthzNats() {
	info := &pbcommon.ComponentHealthzInfo{
		Component: healthz.ComponentNameNats,
		IsHealthy: true,
		Message:   healthz.HealthStateMessage,
	}

	isHealthy, err := natsz.Healthz(act.viper.GetStringSlice("natsmqCluster.endpoints")[0],
		act.viper.GetString("natsmqCluster.tls.caFile"),
		act.viper.GetString("natsmqCluster.tls.certFile"),
		act.viper.GetString("natsmqCluster.tls.keyFile"),
		act.viper.GetString("natsmqCluster.tls.certPassword"))
	if err != nil {
	...
	if !isHealthy {
		info.IsHealthy = false
		info.Message = "nats mq cluster is not working"
		return
	}
