...
// CreateAction creates sidecar app instance information.
...
func NewCreateAction(viper *viper.Viper, dataMgrCli pbdatamanager.DataManagerClient,
	instance *pbcommon.AppInstance) *CreateAction {
	action := &CreateAction{viper: viper, dataMgrCli: dataMgrCli, instance: instance}
	return action
}
...
func (act *CreateAction) create() (pbcommon.ErrCode, string) {
	...
	logger.V(4).Infof("CreateAppInstance| request to datamanager CreateAppInstance, %+v", r)

	resp, err := act.dataMgrCli.CreateAppInstance(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_TS_SYSTEM_UNKNOWN, fmt.Sprintf("request to datamanager CreateAppInstance, %+v", err)
	}
