...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	...
	logger.V(4).Infof("PullConfigList[%s]| request to datamanager QueryConfigList, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QueryConfigList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_TS_SYSTEM_UNKNOWN, fmt.Sprintf("request to datamanager QueryConfigList, %+v", err)
	}
