...
func (act *Action) healthzGSE() {
	info := &pbcommon.ComponentHealthzInfo{
		Component: healthz.ComponentNameGSE,
		IsHealthy: true,
		Message:   healthz.HealthStateMessage,
	}

	isHealthy, err := gsez.Healthz(act.viper.GetStringSlice("gseTaskServer.endpoints")[0])
	if err != nil {
	...
	if !isHealthy {
		info.IsHealthy = false
		info.Message = "gse plat service is not working"
		return
	}
