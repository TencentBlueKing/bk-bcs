...
func (act *PublishPreAction) queryRelease() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("PublishReleasePre[%s]| request to datamanager, %+v", act.req.Seq, r)
...
func (act *PublishPreAction) queryCommit() (pbcommon.ErrCode, string) {
	...
	if err != nil {
		return pbcommon.ErrCode_E_GSE_SYSTEM_UNKNOWN, fmt.Sprintf("request to datamanager QueryCommit, %+v", err)
	}
...
func (act *PublishPreAction) Do() error {
	...
	if act.release.State == int32(pbcommon.ReleaseState_RS_ROLLBACKED) {
		return act.Err(pbcommon.ErrCode_E_GSE_ALREADY_ROLLBACKED, "can't publish the release, it's already rollbacked.")
	}

	if act.release.State == int32(pbcommon.ReleaseState_RS_CANCELED) {
		return act.Err(pbcommon.ErrCode_E_GSE_ALREADY_CANCELED, "can't publish the release, it's already canceled.")
	}
	...
	if act.commit.State != int32(pbcommon.CommitState_CS_CONFIRMED) {
		return act.Err(pbcommon.ErrCode_E_GSE_SYSTEM_UNKNOWN, "can't publish the release with a commit not confirmed.")
	}
