...
func NewReportAction(ctx context.Context, viper *viper.Viper, dataMgrCli pbdatamanager.DataManagerClient,
	req *pb.ReportReq, resp *pb.ReportResp) *ReportAction {
	action := &ReportAction{ctx: ctx, viper: viper, dataMgrCli: dataMgrCli, req: req, resp: resp}
...
func (act *ReportAction) report() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("Report[%s]| request to datamanager, %+v", act.req.Seq, r)
