...
func NewPullAction(ctx context.Context, viper *viper.Viper, bcsControllerCli pbbcscontroller.BCSControllerClient,
	dataMgrCli pbdatamanager.DataManagerClient,
	req *pb.PullReleaseReq, resp *pb.PullReleaseResp) *PullAction {

	action := &PullAction{ctx: ctx, viper: viper, bcsControllerCli: bcsControllerCli,
...
func (act *PullAction) pullRelease() (pbcommon.ErrCode, string) {
	...
	if err != nil {
		return pbcommon.ErrCode_E_CONNS_SYSTEM_UNKNOWN, fmt.Sprintf("request to bcs-controller PullRelease, %+v", err)
	}
...
func (act *PullAction) queryReleaseConfigContent() (pbcommon.ErrCode, string) {
	...
	if err := json.Unmarshal([]byte(act.req.Labels), sidecarLabels); err != nil {
		return pbcommon.ErrCode_E_CONNS_SYSTEM_UNKNOWN, err.Error()
	}
	...
	if err != nil {
		return pbcommon.ErrCode_E_CONNS_SYSTEM_UNKNOWN,
