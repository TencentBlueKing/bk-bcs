...
func (act *DeleteAction) deleteTemplate() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("DeleteConfigTemplateReq[%d]| request to datamanager DeleteConfigTemplateReq, %+v", req.Seq, req)

	resp, err := act.dataMgrCli.DeleteConfigTemplate(ctx, req)
	if err != nil {
		return pbcommon.ErrCode_E_TPL_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager DeleteConfigTemplate, %+v", err)
	}
