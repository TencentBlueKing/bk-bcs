...
type DeleteAction struct {
	...
	req  *pb.DeleteTemplateVersionReq
	resp *pb.DeleteTemplateVersionResp
}

// NewDeleteAction create new DeleteAction
func NewDeleteAction(viper *viper.Viper, dataMgrCli pbdatamanager.DataManagerClient,
	req *pb.DeleteTemplateVersionReq, resp *pb.DeleteTemplateVersionResp) *DeleteAction {
...
func (act *DeleteAction) deleteVersion() (pbcommon.ErrCode, string) {
	req := &pbdatamanager.DeleteTemplateVersionReq{
		Seq:       act.req.Seq,
		Bid:       act.req.Bid,
	...
	logger.V(2).Infof("DeleteTemplateVersion[%d]|request to datamanager DeleteTemplateVersion, %+v", act.req.Seq, req)

	resp, err := act.dataMgrCli.DeleteTemplateVersion(ctx, req)
	if err != nil {
		return pbcommon.ErrCode_E_TPL_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager DeleteTemplateVersion, %+v", err)
	}
...
func (act *DeleteAction) Do() error {
	...
	if errCode, errMsg := act.deleteVersion(); errCode != pbcommon.ErrCode_E_OK {
