...
func (act *UpdateAction) verify() error {
	...
	if len(act.req.Memo) > database.BSCPLONGSTRLENLIMIT {
		return errors.New("invalid params, memo is too long")
	}

	return nil
}

func (act *UpdateAction) updateVersion() (pbcommon.ErrCode, string) {
	ctx, cancel := context.WithTimeout(context.Background(), act.viper.GetDuration("datamanager.calltimeout"))
	defer cancel()

	req := &pbdatamanager.UpdateTemplateVersionReq{
		Seq:         act.req.Seq,
		Bid:         act.req.Bid,
	...
	logger.V(2).Infof("UpdateTemplateVersion[%d]| request to datamanager UpdateTemplateVersion, %+v", req.Seq, req)

	resp, err := act.dataMgrCli.UpdateTemplateVersion(ctx, req)
	if err != nil {
		return pbcommon.ErrCode_E_TPL_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager UpdateTemplateVersion, %+v", err)
	}
...
func (act *UpdateAction) Do() error {
	...
	if errCode, errMsg := act.updateVersion(); errCode != pbcommon.ErrCode_E_OK {
