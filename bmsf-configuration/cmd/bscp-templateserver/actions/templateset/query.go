...
// QueryAction query config template set object.
...
func (act *QueryAction) Output() error {
	act.resp.TemplateSet = act.configTemplateSet
	return nil
}
...
func (act *QueryAction) queryTemplateSet() (pbcommon.ErrCode, string) {
	r := &pbdatamanager.QueryConfigTemplateSetReq{
		Seq:   act.req.Seq,
		Bid:   act.req.Bid,
		Setid: act.req.Setid,
	}
	...
	logger.V(2).Infof("QueryConfigTemplateSet[%d]| request to datamanager QueryConfigTemplateSet, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QueryConfigTemplateSet(ctx, r)
	if err != nil {
	...
	act.configTemplateSet = resp.TemplateSet
