...
// ListAction list target template set objects.
...
func (act *ListAction) Output() error {
	act.resp.TemplateSets = act.configTemplateSets
	return nil
}
...
func (act *ListAction) queryTemplateSetList() (pbcommon.ErrCode, string) {
	r := &pbdatamanager.QueryConfigTemplateSetListReq{
		Seq:   act.req.Seq,
		Bid:   act.req.Bid,
		Index: act.req.Index,
		Limit: act.req.Limit,
	...
	logger.V(2).Infof("QueryConfigTemplateSetList[%d]| request to datamanager QueryConfigTemplateSetList, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QueryConfigTemplateSetList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_TPL_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager QueryConfigTemplateSetList, %+v", err)
	}
	if resp.ErrCode != pbcommon.ErrCode_E_OK {
		return resp.ErrCode, resp.ErrMsg
	}

	act.configTemplateSets = resp.TemplateSets
...
func (act *ListAction) Do() error {
	...
	if errCode, errMsg := act.queryTemplateSetList(); errCode != pbcommon.ErrCode_E_OK {
