...
func NewAppListAction(viper *viper.Viper, dataMgrCli pbdatamanager.DataManagerClient,
	req *pb.QueryAppProcAttrListReq, resp *pb.QueryAppProcAttrListResp) *AppListAction {
	action := &AppListAction{viper: viper, dataMgrCli: dataMgrCli, req: req, resp: resp}
...
func (act *AppListAction) list() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("QueryAppProcAttrList[%d]| request to datamanager QueryAppProcAttrList, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QueryAppProcAttrList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager QueryAppProcAttrList, %+v", err)
	}
