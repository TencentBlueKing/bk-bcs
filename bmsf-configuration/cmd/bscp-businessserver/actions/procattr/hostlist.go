...
func NewHostListAction(viper *viper.Viper, dataMgrCli pbdatamanager.DataManagerClient,
	req *pb.QueryHostProcAttrListReq, resp *pb.QueryHostProcAttrListResp) *HostListAction {
	action := &HostListAction{viper: viper, dataMgrCli: dataMgrCli, req: req, resp: resp}
...
func (act *HostListAction) list() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("QueryHostProcAttrList[%d]| request to datamanager QueryHostProcAttrList, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QueryHostProcAttrList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager QueryHostProcAttrList, %+v", err)
	}
