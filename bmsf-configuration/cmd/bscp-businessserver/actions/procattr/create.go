...
// TODO add bid/appid/clusterid/zoneid check.
...
func (act *CreateAction) create() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("CreateProcAttr[%d]| request to datamanager CreateProcAttr, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.CreateProcAttr(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager CreateProcAttr, %+v", err)
	}
	...
	audit.Audit(int32(pbcommon.SourceType_ST_PROC_ATTR), int32(pbcommon.SourceOpType_SOT_CREATE),
		act.req.Bid, act.req.Appid, act.req.Creator, act.req.Memo)
