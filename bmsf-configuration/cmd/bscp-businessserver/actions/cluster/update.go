...
func (act *UpdateAction) update() (pbcommon.ErrCode, string) {
	r := &pbdatamanager.UpdateClusterReq{
		Seq:        act.req.Seq,
		Bid:        act.req.Bid,
	...
	logger.V(2).Infof("UpdateCluster[%d]| request to datamanager UpdateCluster, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.UpdateCluster(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager UpdateCluster, %+v", err)
	}
	if resp.ErrCode != pbcommon.ErrCode_E_OK {
		return resp.ErrCode, resp.ErrMsg
	}

	// audit here on cluster updated.
	audit.Audit(int32(pbcommon.SourceType_ST_CLUSTER), int32(pbcommon.SourceOpType_SOT_UPDATE),
		act.req.Bid, act.req.Clusterid, act.req.Operator, act.req.Memo)
