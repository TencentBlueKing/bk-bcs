...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	r := &pbdatamanager.QueryClusterListReq{
		Seq:     act.req.Seq,
		Bid:     act.req.Bid,
		Appid:   act.req.Appid,
	...
	logger.V(2).Infof("QueryClusterList[%d]| request to datamanager QueryClusterList, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QueryClusterList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager QueryClusterList, %+v", err)
	}
