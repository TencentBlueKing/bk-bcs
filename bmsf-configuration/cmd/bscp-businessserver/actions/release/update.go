...
// UpdateAction updates target release object.
type UpdateAction struct {
	viper      *viper.Viper
	dataMgrCli pbdatamanager.DataManagerClient

	req  *pb.UpdateReleaseReq
	resp *pb.UpdateReleaseResp
}
...
func NewUpdateAction(viper *viper.Viper, dataMgrCli pbdatamanager.DataManagerClient,
	req *pb.UpdateReleaseReq, resp *pb.UpdateReleaseResp) *UpdateAction {
...
func (act *UpdateAction) update() (pbcommon.ErrCode, string) {
	r := &pbdatamanager.UpdateReleaseReq{
		Seq:       act.req.Seq,
		Bid:       act.req.Bid,
	...
	logger.V(2).Infof("UpdateRelease[%d]| request to datamanager UpdateRelease, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.UpdateRelease(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager UpdateRelease, %+v", err)
	}
	...
	audit.Audit(int32(pbcommon.SourceType_ST_RELEASE), int32(pbcommon.SourceOpType_SOT_UPDATE),
		act.req.Bid, act.req.Releaseid, act.req.Operator, act.req.Memo)
