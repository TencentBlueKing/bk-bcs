...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	r := &pbdatamanager.QueryHistoryReleasesReq{
		Seq:       act.req.Seq,
		Bid:       act.req.Bid,
	...
	logger.V(2).Infof("QueryHistoryReleases[%d]| request to datamanager QueryHistoryReleases, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QueryHistoryReleases(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager QueryHistoryReleases, %+v", err)
	}
