...
func (act *UpdateAction) update() (pbcommon.ErrCode, string) {
	r := &pbdatamanager.UpdateShardingDBReq{
		Seq:      act.req.Seq,
		Dbid:     act.req.Dbid,
		Host:     act.req.Host,
		Port:     act.req.Port,
	...
	logger.V(2).Infof("UpdateShardingDB[%d]| request to datamanager UpdateShardingDB, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.UpdateShardingDB(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager UpdateShardingDB, %+v", err)
	}
