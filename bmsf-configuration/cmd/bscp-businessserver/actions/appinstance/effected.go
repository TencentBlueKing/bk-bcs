...
func NewEffectedAction(viper *viper.Viper, dataMgrCli pbdatamanager.DataManagerClient,
	req *pb.QueryEffectedAppInstancesReq, resp *pb.QueryEffectedAppInstancesResp) *EffectedAction {
	action := &EffectedAction{viper: viper, dataMgrCli: dataMgrCli, req: req, resp: resp}
...
func (act *EffectedAction) effected() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("QueryEffectedAppInstances[%d]| request to datamanager QueryEffectedAppInstances, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QueryEffectedAppInstances(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager QueryEffectedAppInstances, %+v", err)
	}
