...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	r := &pbdatamanager.QueryZoneListReq{
		Seq:         act.req.Seq,
		Bid:         act.req.Bid,
		Appid:       act.req.Appid,
	...
	logger.V(2).Infof("QueryZoneList[%d]| request to datamanager QueryZoneList, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QueryZoneList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager QueryZoneList, %+v", err)
	}
