...
func (act *QueryAction) query() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("QueryZone[%d]| request to datamanager QueryZone, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QueryZone(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager QueryZone, %+v", err)
	}
	act.zone = resp.Zone
