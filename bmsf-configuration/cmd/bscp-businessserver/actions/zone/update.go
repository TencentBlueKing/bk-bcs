...
// UpdateAction updates target zone object.
...
func (act *UpdateAction) update() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("UpdateZone[%d]| request to datamanager UpdateZone, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.UpdateZone(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager UpdateZone, %+v", err)
	}
	if resp.ErrCode != pbcommon.ErrCode_E_OK {
		return resp.ErrCode, resp.ErrMsg
	}

	// audit here on zone updated.
	audit.Audit(int32(pbcommon.SourceType_ST_ZONE), int32(pbcommon.SourceOpType_SOT_UPDATE),
		act.req.Bid, act.req.Zoneid, act.req.Operator, act.req.Memo)
