...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("QueryAppList[%d]| request to datamanager QueryAppList, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QueryAppList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager QueryAppList, %+v", err)
	}
	act.apps = resp.Apps
