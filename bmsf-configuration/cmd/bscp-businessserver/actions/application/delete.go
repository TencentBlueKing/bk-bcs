...
// DeleteAction deletes target app object.
...
func (act *DeleteAction) delete() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("DeleteApp[%d]| request to datamanager DeleteApp, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.DeleteApp(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager DeleteApp, %+v", err)
	}
	...
	// audit here on app deleted.
	audit.Audit(int32(pbcommon.SourceType_ST_APP), int32(pbcommon.SourceOpType_SOT_DELETE),
		act.req.Bid, act.req.Appid, act.req.Operator, "")
