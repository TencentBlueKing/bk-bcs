...
func (act *QueryAction) query() (pbcommon.ErrCode, string) {
	r := &pbdatamanager.QueryConfigsReq{
		Seq:       act.req.Seq,
		Bid:       act.req.Bid,
		Appid:     act.req.Appid,
	...
	logger.V(2).Infof("QueryConfigs[%d]| request to datamanager QueryConfigs, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QueryConfigs(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager QueryConfigs, %+v", err)
	}
