...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("QueryConfigsList[%d]| request to datamanager QueryConfigsList, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QueryConfigsList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager QueryConfigsList, %+v", err)
	}
