...
// UpdateAction updates target multi commit object.
...
func (act *UpdateAction) update() (pbcommon.ErrCode, string) {
	r := &pbdatamanager.UpdateMultiCommitReq{
		Seq:           act.req.Seq,
		Bid:           act.req.Bid,
	...
	logger.V(2).Infof("UpdateMultiCommit[%d]| request to datamanager UpdateMultiCommit, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.UpdateMultiCommit(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager UpdateMultiCommit, %+v", err)
	}
	...
	audit.Audit(int32(pbcommon.SourceType_ST_MULTI_COMMIT), int32(pbcommon.SourceOpType_SOT_UPDATE),
		act.req.Bid, act.req.MultiCommitid, act.req.Operator, act.req.Memo)
