...
// ListAction query multi commit list.
...
func (act *ListAction) Output() error {
	act.resp.MultiCommits = act.multiCommits
	return nil
}
...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	r := &pbdatamanager.QueryHistoryMultiCommitsReq{
		Seq:       act.req.Seq,
		Bid:       act.req.Bid,
		Appid:     act.req.Appid,
	...
	logger.V(2).Infof("QueryHistoryMultiCommits[%d]| request to datamanager QueryHistoryMultiCommits, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QueryHistoryMultiCommits(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager QueryHistoryMultiCommits, %+v", err)
	}
	act.multiCommits = resp.MultiCommits
