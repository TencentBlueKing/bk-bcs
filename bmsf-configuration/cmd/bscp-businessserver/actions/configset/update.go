...
import (
	...
	common "bk-bscp/pkg/common"
	"bk-bscp/pkg/logger"
)

// UpdateAction updates target configset object.
type UpdateAction struct {
	viper      *viper.Viper
	dataMgrCli pbdatamanager.DataManagerClient

	req  *pb.UpdateConfigSetReq
	resp *pb.UpdateConfigSetResp
}
...
func NewUpdateAction(viper *viper.Viper, dataMgrCli pbdatamanager.DataManagerClient,
	req *pb.UpdateConfigSetReq, resp *pb.UpdateConfigSetResp) *UpdateAction {
...
func (act *UpdateAction) verify() error {
	...
	length = len(act.req.Fpath)
	if length == 0 {
		return errors.New("invalid params, fpath missing")
	}
...
func (act *UpdateAction) update() (pbcommon.ErrCode, string) {
	r := &pbdatamanager.UpdateConfigSetReq{
		Seq:      act.req.Seq,
		Bid:      act.req.Bid,
	...
	logger.V(2).Infof("UpdateConfigSet[%d]| request to datamanager UpdateConfigSet, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.UpdateConfigSet(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager UpdateConfigSet, %+v", err)
	}
	if resp.ErrCode != pbcommon.ErrCode_E_OK {
		return resp.ErrCode, resp.ErrMsg
	}

	// audit here on configset updated.
	audit.Audit(int32(pbcommon.SourceType_ST_CONFIGSET), int32(pbcommon.SourceOpType_SOT_UPDATE),
