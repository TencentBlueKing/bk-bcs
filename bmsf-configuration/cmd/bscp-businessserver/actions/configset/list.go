...
type ListAction struct {
	...
	req  *pb.QueryConfigSetListReq
	resp *pb.QueryConfigSetListResp
...
func NewListAction(viper *viper.Viper, dataMgrCli pbdatamanager.DataManagerClient,
	req *pb.QueryConfigSetListReq, resp *pb.QueryConfigSetListResp) *ListAction {
...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("QueryConfigSetList[%d]| request to datamanager QueryConfigSetList, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QueryConfigSetList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager QueryConfigSetList, %+v", err)
	}
	act.configSets = resp.ConfigSets
