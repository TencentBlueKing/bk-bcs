...
func (act *DeleteAction) delete() (pbcommon.ErrCode, string) {
	r := &pbdatamanager.DeleteConfigSetReq{
		Seq:      act.req.Seq,
		Bid:      act.req.Bid,
	...
	logger.V(2).Infof("DeleteConfigSet[%d]| request to datamanager DeleteConfigSet, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.DeleteConfigSet(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager DeleteConfigSet, %+v", err)
	}
	if resp.ErrCode != pbcommon.ErrCode_E_OK {
		return resp.ErrCode, resp.ErrMsg
	}

	// audit here on configset deleted.
	audit.Audit(int32(pbcommon.SourceType_ST_CONFIGSET), int32(pbcommon.SourceOpType_SOT_DELETE),
