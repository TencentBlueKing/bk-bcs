...
func (act *QueryAction) query() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("QuerySharding[%d]| request to datamanager QuerySharding, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QuerySharding(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager QuerySharding, %+v", err)
	}
