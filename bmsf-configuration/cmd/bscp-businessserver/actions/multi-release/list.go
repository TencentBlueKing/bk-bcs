...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("QueryHistoryMultiReleases[%d]| request to datamanager QueryHistoryMultiReleases, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QueryHistoryMultiReleases(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager QueryHistoryMultiReleases, %+v", err)
	}
