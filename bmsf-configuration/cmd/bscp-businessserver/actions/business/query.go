...
func (act *QueryAction) query() (pbcommon.ErrCode, string) {
	r := &pbdatamanager.QueryBusinessReq{
		Seq:  act.req.Seq,
		Bid:  act.req.Bid,
		Name: act.req.Name,
	}
	...
	logger.V(2).Infof("QueryBusiness[%d]| request to datamanager QueryBusiness, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QueryBusiness(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager QueryBusiness, %+v", err)
	}
