...
func (act *UpdateAction) update() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("UpdateCommit[%d]| request to datamanager UpdateCommit, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.UpdateCommit(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager UpdateCommit, %+v", err)
	}
	...
	audit.Audit(int32(pbcommon.SourceType_ST_COMMIT), int32(pbcommon.SourceOpType_SOT_UPDATE),
		act.req.Bid, act.req.Commitid, act.req.Operator, act.req.Memo)
