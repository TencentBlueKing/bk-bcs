...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("QueryHistoryCommits[%d]| request to datamanager QueryHistoryCommits, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.QueryHistoryCommits(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager QueryHistoryCommits, %+v", err)
	}
