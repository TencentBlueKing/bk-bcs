...
func (act *DeleteAction) delete() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("DeleteStrategy[%d]| request to datamanager DeleteStrategy, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.DeleteStrategy(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager DeleteStrategy, %+v", err)
	}
	...
	audit.Audit(int32(pbcommon.SourceType_ST_STRATEGY), int32(pbcommon.SourceOpType_SOT_DELETE),
		act.req.Bid, act.req.Strategyid, act.req.Operator, "")
