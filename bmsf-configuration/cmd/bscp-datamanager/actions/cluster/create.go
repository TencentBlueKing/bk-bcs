...
// CreateAction is cluster create action object.
...
func (act *CreateAction) reCreateCluster() (pbcommon.ErrCode, string) {
	act.sd.AutoMigrate(&database.Cluster{})

	ups := map[string]interface{}{
		"Bid":          act.req.Bid,
		"Clusterid":    act.req.Clusterid,
		"State":        int32(pbcommon.ClusterState_CS_CREATED),
	...
	exec := act.sd.DB().
		...
		Where(&database.Cluster{Appid: act.req.Appid, Name: act.req.Name}).
		Where("Flabels = ?", act.req.Labels).
		Where("Fstate = ?", pbcommon.ClusterState_CS_DELETED).
		Updates(ups)
	...
	if exec.RowsAffected == 0 {
		return pbcommon.ErrCode_E_DM_DB_UPDATE_ERR, "recreate the cluster failed, there is no cluster that fit the conditions."
	}
	return pbcommon.ErrCode_E_OK, ""
}

func (act *CreateAction) createCluster() (pbcommon.ErrCode, string) {
	act.sd.AutoMigrate(&database.Cluster{})
	...
	act.resp.Clusterid = st.Clusterid

	if st.Clusterid != act.req.Clusterid {
		if st.State == int32(pbcommon.ClusterState_CS_CREATED) {
			return pbcommon.ErrCode_E_DM_ALREADY_EXISTS, "the cluster with target name already exist."
		}

		if errCode, errMsg := act.reCreateCluster(); errCode != pbcommon.ErrCode_E_OK {
			return errCode, errMsg
		}
		act.resp.Clusterid = act.req.Clusterid
	}
	return pbcommon.ErrCode_E_OK, ""
}
...
func (act *CreateAction) Do() error {
	...
	// create cluster.
	if errCode, errMsg := act.createCluster(); errCode != pbcommon.ErrCode_E_OK {
