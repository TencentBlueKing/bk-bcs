...
// UpdateAction is multi release update action object.
...
func (act *UpdateAction) updateMultiRelease() (pbcommon.ErrCode, string) {
	...
	if exec.RowsAffected == 0 {
		return pbcommon.ErrCode_E_DM_DB_UPDATE_ERR, "update the multi release failed(release no-exist or already published)."
	}
...
func (act *UpdateAction) Do() error {
	...
	// update multi release.
	if errCode, errMsg := act.updateMultiRelease(); errCode != pbcommon.ErrCode_E_OK {
