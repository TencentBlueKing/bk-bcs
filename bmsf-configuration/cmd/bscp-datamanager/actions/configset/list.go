...
// ListAction is configset list action object.
...
func (act *ListAction) Output() error {
	...
	act.resp.ConfigSets = configsets
	return nil
}
...
func (act *ListAction) queryApp() (pbcommon.ErrCode, string) {
	...
	if err == dbsharding.RECORDNOTFOUND {
		return pbcommon.ErrCode_E_DM_NOT_FOUND, "app with target name non-exist, can't query configset list under it."
	}
	if err != nil {
...
func (act *ListAction) queryConfigSetList() (pbcommon.ErrCode, string) {
...
func (act *ListAction) Do() error {
	...
	if errCode, errMsg := act.queryConfigSetList(); errCode != pbcommon.ErrCode_E_OK {
		return act.Err(errCode, errMsg)
	}
	return nil
}
