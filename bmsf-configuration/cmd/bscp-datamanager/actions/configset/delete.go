...
// DeleteAction is configset delete action object.
...
func (act *DeleteAction) deleteConfigSet() (pbcommon.ErrCode, string) {
	...
	// do not delete the metadata, just mark deleted soft state.
	ups := map[string]interface{}{
		"State":        int32(pbcommon.ConfigSetState_CSS_DELETED),
		"LastModifyBy": act.req.Operator,
	}

	err := act.sd.DB().
		Model(&database.ConfigSet{}).
		Where(&database.ConfigSet{Bid: act.req.Bid, Cfgsetid: act.req.Cfgsetid}).
		Updates(ups).Error
