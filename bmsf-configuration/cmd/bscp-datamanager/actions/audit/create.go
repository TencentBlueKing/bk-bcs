...
// CreateAction is audit create action object.
type CreateAction struct {
	viper *viper.Viper
	smgr  *dbsharding.ShardingManager

	req  *pb.CreateAuditReq
	resp *pb.CreateAuditResp
...
func NewCreateAction(viper *viper.Viper, smgr *dbsharding.ShardingManager,
	req *pb.CreateAuditReq, resp *pb.CreateAuditResp) *CreateAction {
...
func (act *CreateAction) verify() error {
	...
	length := len(act.req.Sourceid)
	if length == 0 {
		return errors.New("invalid params, sourceid missing")
	}
...
func (act *CreateAction) createAudit() (pbcommon.ErrCode, string) {
	...
	if err := act.sd.DB().Create(&st).Error; err != nil {
...
func (act *CreateAction) Do() error {
	...
	// create audit.
	if errCode, errMsg := act.createAudit(); errCode != pbcommon.ErrCode_E_OK {
