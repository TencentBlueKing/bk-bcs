...
func (as *AccessServer) PreviewRendering(ctx context.Context, req *pb.PreviewRenderingReq) (*pb.PreviewRenderingResp, error) {
	rtime := time.Now()
	...
	defer func() {
		cost := as.collector.StatRequest("PreviewRendering", response.ErrCode, rtime, time.Now())
	...
	action := tplaction.NewPreviewAction(as.viper, as.businessSvrCli, req, response)
	as.executor.Execute(action)
