...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.QueryHistoryMultiCommitsReq{
		Seq:       act.req.Seq,
		Bid:       act.req.Bid,
		Appid:     act.req.Appid,
	...
	logger.V(2).Infof("QueryHistoryMultiCommits[%d]| request to businessserver QueryHistoryMultiCommits, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.QueryHistoryMultiCommits(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver QueryHistoryMultiCommits, %+v", err)
	}
