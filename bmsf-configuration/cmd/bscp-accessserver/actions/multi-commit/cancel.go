...
func (act *CancelAction) cancel() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.CancelMultiCommitReq{
		Seq:           act.req.Seq,
		Bid:           act.req.Bid,
	...
	logger.V(2).Infof("CancelMultiCommit[%d]| request to businessserver CancelMultiCommit, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.CancelMultiCommit(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver CancelMultiCommit, %+v", err)
	}
