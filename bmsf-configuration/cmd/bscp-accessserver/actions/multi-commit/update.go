...
func (act *UpdateAction) update() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.UpdateMultiCommitReq{
		Seq:           act.req.Seq,
		Bid:           act.req.Bid,
	...
	resp, err := act.buSvrCli.UpdateMultiCommit(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver UpdateMultiCommit, %+v", err)
	}
