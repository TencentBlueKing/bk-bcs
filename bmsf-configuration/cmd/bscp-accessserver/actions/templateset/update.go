...
// UpdateAction create a template set object
...
func (act *UpdateAction) updateTemplateSet() (pbcommon.ErrCode, string) {
	...
	req := &pbtemplateserver.UpdateConfigTemplateSetReq{
		Seq:      act.req.Seq,
		Bid:      act.req.Bid,
		Setid:    act.req.Setid,
		Name:     act.req.Name,
	...
	logger.V(2).Infof("UpdateConfigTemplateSet[%d]| request to templateserver UpdateConfigTemplateSet, %+v", req.Seq, req)

	resp, err := act.templateClient.UpdateConfigTemplateSet(ctx, req)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to templateserver UpdateConfigTemplateSet, %+v", err)
	}
