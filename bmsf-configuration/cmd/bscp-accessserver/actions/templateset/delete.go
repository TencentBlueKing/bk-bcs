...
// DeleteAction delete a config template set object
...
func (act *DeleteAction) deleteTemplateSet() (pbcommon.ErrCode, string) {
	req := &pbtemplateserver.DeleteConfigTemplateSetReq{
		Seq:      act.req.Seq,
		Bid:      act.req.Bid,
		Setid:    act.req.Setid,
	...
	logger.V(2).Infof("DeleteConfigTemplateSetReq[%d]| request to templateserver DeleteConfigTemplateSetReq, %+v", req.Seq, req)

	resp, err := act.templateClient.DeleteConfigTemplateSet(ctx, req)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to templateserver DeleteConfigTemplateSet, %+v", err)
	}
