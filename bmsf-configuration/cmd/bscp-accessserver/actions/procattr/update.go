...
func (act *UpdateAction) update() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.UpdateProcAttrReq{
		Seq:       act.req.Seq,
	...
	logger.V(2).Infof("UpdateProcAttr[%d]| request to businessserver UpdateProcAttr, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.UpdateProcAttr(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver UpdateProcAttr, %+v", err)
	}
