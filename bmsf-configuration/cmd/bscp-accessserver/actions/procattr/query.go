...
func (act *QueryAction) query() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.QueryHostProcAttrReq{
		Seq:     act.req.Seq,
	...
	logger.V(2).Infof("QueryHostProcAttr[%d]| request to businessserver QueryHostProcAttr, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.QueryHostProcAttr(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver QueryHostProcAttr, %+v", err)
	}
