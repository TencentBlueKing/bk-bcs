...
func NewHostListAction(viper *viper.Viper, buSvrCli pbbusinessserver.BusinessClient,
	req *pb.QueryHostProcAttrListReq, resp *pb.QueryHostProcAttrListResp) *HostListAction {
	action := &HostListAction{viper: viper, buSvrCli: buSvrCli, req: req, resp: resp}
...
func (act *HostListAction) list() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.QueryHostProcAttrListReq{
		Seq:     act.req.Seq,
	...
	logger.V(2).Infof("QueryHostProcAttrList[%d]| request to businessserver QueryHostProcAttrList, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.QueryHostProcAttrList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver QueryHostProcAttrList, %+v", err)
	}
