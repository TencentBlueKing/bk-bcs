...
func (act *ReleaseAction) release() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.QueryAppInstanceReleaseReq{
		Seq:       act.req.Seq,
		Bid:       act.req.Bid,
		Appid:     act.req.Appid,
	...
	logger.V(2).Infof("QueryAppInstanceRelease[%d]| request to businessserver QueryAppInstanceRelease, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.QueryAppInstanceRelease(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver QueryAppInstanceRelease, %+v", err)
	}
