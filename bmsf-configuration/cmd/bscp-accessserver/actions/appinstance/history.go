...
func NewHistoryAction(viper *viper.Viper, buSvrCli pbbusinessserver.BusinessClient,
	req *pb.QueryHistoryAppInstancesReq, resp *pb.QueryHistoryAppInstancesResp) *HistoryAction {
	action := &HistoryAction{viper: viper, buSvrCli: buSvrCli, req: req, resp: resp}
...
func (act *HistoryAction) history() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.QueryHistoryAppInstancesReq{
		Seq:       act.req.Seq,
		Bid:       act.req.Bid,
		Appid:     act.req.Appid,
	...
	logger.V(2).Infof("QueryHistoryAppInstances[%d]| request to businessserver QueryHistoryAppInstances, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.QueryHistoryAppInstances(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver QueryHistoryAppInstances, %+v", err)
	}
