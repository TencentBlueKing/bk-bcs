...
func NewReachableAction(viper *viper.Viper, buSvrCli pbbusinessserver.BusinessClient,
	req *pb.QueryReachableAppInstancesReq, resp *pb.QueryReachableAppInstancesResp) *ReachableAction {
	action := &ReachableAction{viper: viper, buSvrCli: buSvrCli, req: req, resp: resp}
...
func (act *ReachableAction) reachable() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.QueryReachableAppInstancesReq{
		Seq:       act.req.Seq,
		Bid:       act.req.Bid,
		Appid:     act.req.Appid,
	...
	logger.V(2).Infof("QueryReachableAppInstances[%d]| request to businessserver QueryReachableAppInstances, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.QueryReachableAppInstances(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver QueryReachableAppInstances, %+v", err)
	}
