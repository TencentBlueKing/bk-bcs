...
func (act *QueryAction) verify() error {
	...
	// empty labels is fine in appid-name-labels query mode.
...
func (act *QueryAction) query() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("QueryCluster[%d]| request to businessserver QueryCluster, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.QueryCluster(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver QueryCluster, %+v", err)
	}
