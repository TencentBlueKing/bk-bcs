...
func (act *ListAction) Output() error {
	act.resp.Clusters = act.clusters
	return nil
}
...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.QueryClusterListReq{
		Seq:     act.req.Seq,
		Bid:     act.req.Bid,
		Appid:   act.req.Appid,
	...
	logger.V(2).Infof("QueryClusterList[%d]| request to businessserver QueryClusterList, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.QueryClusterList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver QueryClusterList, %+v", err)
	}
	act.clusters = resp.Clusters
