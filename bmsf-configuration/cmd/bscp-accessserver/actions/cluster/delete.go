...
func (act *DeleteAction) delete() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.DeleteClusterReq{
		Seq:       act.req.Seq,
		Bid:       act.req.Bid,
	...
	logger.V(2).Infof("DeleteCluster[%d]| request to businessserver DeleteCluster, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.DeleteCluster(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver DeleteCluster, %+v", err)
	}
