...
func (act *CreateAction) create() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("CreateCluster[%d]| request to businessserver CreateCluster, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.CreateCluster(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver CreateCluster, %+v", err)
	}
	// maybe already exist.
