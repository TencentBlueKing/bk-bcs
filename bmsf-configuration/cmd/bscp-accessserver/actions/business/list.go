...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.QueryBusinessListReq{
		Seq:   act.req.Seq,
		Index: act.req.Index,
		Limit: act.req.Limit,
	}
	...
	logger.V(2).Infof("QueryBusinessList[%d]| request to businessserver QueryBusinessList, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.QueryBusinessList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver QueryBusinessList, %+v", err)
	}
