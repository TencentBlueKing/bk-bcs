...
// QueryAction delete a config template object
...
func (act *QueryAction) queryTemplate() (pbcommon.ErrCode, string) {
	req := &pbtemplateserver.QueryConfigTemplateReq{
		Seq:        act.req.Seq,
		Bid:        act.req.Bid,
	...
	logger.V(2).Infof("QueryConfigTemplate[%d]| request to templateserver QueryConfigTemplate, %+v", req.Seq, req)

	resp, err := act.templateClient.QueryConfigTemplate(ctx, req)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to templateserver QueryConfigTemplate, %+v", err)
	}
	if resp.ErrCode != pbcommon.ErrCode_E_OK {
		return resp.ErrCode, resp.ErrMsg
	}

	act.resp.ConfigTemplate = resp.ConfigTemplate
