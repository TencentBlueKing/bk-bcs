...
func (act *PreviewAction) preview() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.PreviewRenderingReq{
		Seq:          act.req.Seq,
		Bid:          act.req.Bid,
		Appid:        act.req.Appid,
	...
	logger.V(2).Infof("PreviewRendering[%d]| request to businessserver PreviewRenderingReq, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.PreviewRendering(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver PreviewRendering, %+v", err)
	}
	act.content = resp.Content
