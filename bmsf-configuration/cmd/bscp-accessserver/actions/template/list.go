...
func (act *ListAction) listTemplate() (pbcommon.ErrCode, string) {
	req := &pbtemplateserver.QueryConfigTemplateListReq{
		Seq:   act.req.Seq,
		Bid:   act.req.Bid,
		Setid: act.req.Setid,
		Index: act.req.Index,
	...
	logger.V(2).Infof("QueryConfigTemplateList[%d]| request to templateserver QueryConfigTemplateList, %+v", req.Seq, req)

	resp, err := act.templateClient.QueryConfigTemplateList(ctx, req)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to templateserver QueryConfigTemplateList, %+v", err)
	}
	if resp.ErrCode != pbcommon.ErrCode_E_OK {
		return resp.ErrCode, resp.ErrMsg
	}

	act.resp.ConfigTemplates = resp.ConfigTemplates
...
func (act *ListAction) Do() error {
	...
	if errCode, errMsg := act.listTemplate(); errCode != pbcommon.ErrCode_E_OK {
