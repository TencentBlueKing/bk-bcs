...
func (act *DeleteAction) delete() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.DeleteZoneReq{
		Seq:      act.req.Seq,
		Bid:      act.req.Bid,
	...
	logger.V(2).Infof("DeleteZone[%d]| request to businessserver DeleteZone, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.DeleteZone(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver DeleteZone, %+v", err)
	}
