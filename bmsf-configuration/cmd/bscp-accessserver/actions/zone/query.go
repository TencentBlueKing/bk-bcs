...
// QueryAction query target zone object.
...
func (act *QueryAction) query() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("QueryZone[%d]| request to businessserver QueryZone, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.QueryZone(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver QueryZone, %+v", err)
	}
	act.zone = resp.Zone
