...
func (act *UpdateAction) update() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.UpdateZoneReq{
		Seq:      act.req.Seq,
		Bid:      act.req.Bid,
	...
	logger.V(2).Infof("UpdateZone[%d]| request to businessserver UpdateZone, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.UpdateZone(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver UpdateZone, %+v", err)
	}
