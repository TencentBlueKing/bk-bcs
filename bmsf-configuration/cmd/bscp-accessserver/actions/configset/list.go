...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.QueryConfigSetListReq{
		Seq:     act.req.Seq,
		Bid:     act.req.Bid,
		Appid:   act.req.Appid,
	...
	logger.V(2).Infof("QueryConfigSetList[%d]| request to businessserver QueryConfigSetList, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.QueryConfigSetList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver QueryConfigSetList, %+v", err)
	}
