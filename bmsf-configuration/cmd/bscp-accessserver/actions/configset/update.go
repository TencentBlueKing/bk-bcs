...
func (act *UpdateAction) update() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.UpdateConfigSetReq{
		Seq:      act.req.Seq,
		Bid:      act.req.Bid,
	...
	logger.V(2).Infof("UpdateConfigSet[%d]| request to businessserver UpdateConfigSet, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.UpdateConfigSet(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver UpdateConfigSet, %+v", err)
	}
