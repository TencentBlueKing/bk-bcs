...
func (act *QueryAction) query() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.QueryConfigSetReq{
		Seq:      act.req.Seq,
		Bid:      act.req.Bid,
		Appid:    act.req.Appid,
	...
	logger.V(2).Infof("QueryConfigSet[%d]| request to businessserver QueryConfigSet, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.QueryConfigSet(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver QueryConfigSet, %+v", err)
	}
