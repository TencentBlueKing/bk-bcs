...
func (act *UpdateAction) update() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.UpdateShardingReq{
		Seq:    act.req.Seq,
		Key:    act.req.Key,
		Dbid:   act.req.Dbid,
	...
	logger.V(2).Infof("UpdateSharding[%d]| request to businessserver UpdateSharding, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.UpdateSharding(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver UpdateSharding, %+v", err)
	}
