...
// QueryAction query target sharding object.
...
func (act *QueryAction) Output() error {
	act.resp.Sharding = act.sharding
	return nil
}
...
func (act *QueryAction) query() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.QueryShardingReq{
		Seq: act.req.Seq,
		Key: act.req.Key,
	}
	...
	logger.V(2).Infof("QuerySharding[%d]| request to businessserver QuerySharding, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.QuerySharding(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver QuerySharding, %+v", err)
	}
	act.sharding = resp.Sharding
