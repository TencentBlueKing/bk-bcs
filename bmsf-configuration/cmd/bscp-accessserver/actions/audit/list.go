...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.QueryAuditListReq{
		Seq:        act.req.Seq,
		Bid:        act.req.Bid,
	...
	logger.V(2).Infof("QueryAuditList[%d]| request to businessserver QueryAuditList, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.QueryAuditList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver QueryAuditList, %+v", err)
	}
