...
func (act *DeleteAction) deleteTemplateBinding() (pbcommon.ErrCode, string) {
	req := &pbtemplateserver.DeleteConfigTemplateBindingReq{
		Seq:        act.req.Seq,
		Bid:        act.req.Bid,
	...
	logger.V(2).Infof("DeleteConfigTemplateBindingReq[%d]| request to templateserver DeleteConfigTemplateBindingReq, %+v", req.Seq, req)

	resp, err := act.templateClient.DeleteConfigTemplateBinding(ctx, req)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to templateserver DeleteConfigTemplateBinding, %+v", err)
	}
...
func (act *DeleteAction) Do() error {
	...
	// delete config template binding
