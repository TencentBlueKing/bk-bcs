...
func (act *DeleteAction) delete() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.DeleteStrategyReq{
		Seq:        act.req.Seq,
		Bid:        act.req.Bid,
	...
	logger.V(2).Infof("DeleteStrategy[%d]| request to businessserver DeleteStrategy, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.DeleteStrategy(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver DeleteStrategy, %+v", err)
	}
