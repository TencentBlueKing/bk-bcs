...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.QueryStrategyListReq{
		Seq:     act.req.Seq,
		Bid:     act.req.Bid,
		Appid:   act.req.Appid,
	...
	logger.V(2).Infof("QueryStrategyList[%d]| request to businessserver QueryStrategyList, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.QueryStrategyList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver QueryStrategyList, %+v", err)
	}
