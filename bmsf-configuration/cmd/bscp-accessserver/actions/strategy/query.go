...
func (act *QueryAction) query() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("QueryStrategy[%d]| request to businessserver QueryStrategy, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.QueryStrategy(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver QueryStrategy, %+v", err)
	}
