...
// ListAction delete a config template object
...
func (act *ListAction) listTemplateVersion() (pbcommon.ErrCode, string) {
	req := &pbtemplateserver.QueryTemplateVersionListReq{
		Seq:        act.req.Seq,
		Bid:        act.req.Bid,
	...
	logger.V(2).Infof("QueryTemplateVersionList[%d]| request to templateserver QueryTemplateVersionList, %+v", req.Seq, req)

	resp, err := act.templateClient.QueryTemplateVersionList(ctx, req)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to templateserver QueryTemplateVersionList, %+v", err)
	}
...
func (act *ListAction) Do() error {
	...
	if errCode, errMsg := act.listTemplateVersion(); errCode != pbcommon.ErrCode_E_OK {
