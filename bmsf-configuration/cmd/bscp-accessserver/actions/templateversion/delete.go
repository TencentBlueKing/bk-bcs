...
func (act *DeleteAction) deleteTemplateVersion() (pbcommon.ErrCode, string) {
	req := &pbtemplateserver.DeleteTemplateVersionReq{
		Seq:       act.req.Seq,
		Bid:       act.req.Bid,
	...
	logger.V(2).Infof("DeleteTemplateVersionReq[%d]| request to templateserver DeleteTemplateVersionReq, %+v", req.Seq, req)

	resp, err := act.templateClient.DeleteTemplateVersion(ctx, req)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to templateserver DeleteTemplateVersion, %+v", err)
	}
