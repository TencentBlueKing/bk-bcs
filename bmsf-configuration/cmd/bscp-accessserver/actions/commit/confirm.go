...
func (act *ConfirmAction) confirm() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("ConfirmCommit[%d]| request to businessserver ConfirmCommit, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.ConfirmCommit(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver ConfirmCommit, %+v", err)
	}
