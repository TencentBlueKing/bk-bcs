...
func (act *DeleteAction) delete() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.DeleteAppReq{
		Seq:      act.req.Seq,
		Bid:      act.req.Bid,
		Appid:    act.req.Appid,
	...
	logger.V(2).Infof("DeleteApp[%d]| request to businessserver DeleteApp, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.DeleteApp(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver DeleteApp, %+v", err)
	}
