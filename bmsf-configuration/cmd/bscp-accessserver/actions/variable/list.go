...
// ListAction delete a variable object
...
func (act *ListAction) listVariable() (pbcommon.ErrCode, string) {
	req := &pbtemplateserver.QueryVariableListReq{
		Seq:           act.req.Seq,
		Bid:           act.req.Bid,
	...
	logger.V(2).Infof("QueryVariableList[%d]| request to templateserver QueryVariableList, %+v", req.Seq, req)

	resp, err := act.templateClient.QueryVariableList(ctx, req)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to templateserver QueryVariableList, %+v", err)
	}
	if resp.ErrCode != pbcommon.ErrCode_E_OK {
		return resp.ErrCode, resp.ErrMsg
	}

	act.resp.Vars = resp.Vars
...
func (act *ListAction) Do() error {
	...
	// list variable
	if errCode, errMsg := act.listVariable(); errCode != pbcommon.ErrCode_E_OK {
