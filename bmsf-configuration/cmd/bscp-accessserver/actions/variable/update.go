...
// UpdateAction create a variable object
...
func (act *UpdateAction) updateVersion() (pbcommon.ErrCode, string) {
	...
	req := &pbtemplateserver.UpdateVariableReq{
		Seq:      act.req.Seq,
		Bid:      act.req.Bid,
		Vid:      act.req.Vid,
		Type:     act.req.Type,
	...
	logger.V(2).Infof("UpdateVariable[%d]| request to templateserver UpdateVariable, %+v", req.Seq, req)

	resp, err := act.templateClient.UpdateVariable(ctx, req)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to templateserver UpdateVariable, %+v", err)
	}
