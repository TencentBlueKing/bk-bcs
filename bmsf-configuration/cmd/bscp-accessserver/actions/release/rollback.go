...
func (act *RollbackAction) rollback() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.RollbackReleaseReq{
		...
		NewReleaseid: act.req.NewReleaseid,
	...
	logger.V(2).Infof("RollbackRelease[%d]| request to businessserver RollbackRelease, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.RollbackRelease(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver RollbackRelease, %+v", err)
	}
