...
func (act *QueryAction) query() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.QueryReleaseReq{
		Seq:       act.req.Seq,
		Bid:       act.req.Bid,
	...
	logger.V(2).Infof("QueryRelease[%d]| request to businessserver QueryRelease, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.QueryRelease(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver QueryRelease, %+v", err)
	}
