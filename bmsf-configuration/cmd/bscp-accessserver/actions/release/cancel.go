...
func (act *CancelAction) cancel() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.CancelReleaseReq{
		Seq:       act.req.Seq,
		Bid:       act.req.Bid,
	...
	logger.V(2).Infof("CancelRelease[%d]| request to businessserver CancelRelease, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.CancelRelease(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver CancelRelease, %+v", err)
	}
