...
func (act *UpdateAction) update() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.UpdateReleaseReq{
		Seq:       act.req.Seq,
		Bid:       act.req.Bid,
	...
	logger.V(2).Infof("UpdateRelease[%d]| request to businessserver UpdateRelease, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.UpdateRelease(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver UpdateRelease, %+v", err)
	}
