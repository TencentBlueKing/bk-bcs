...
func (act *PublishAction) publish() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("PublishRelease[%d]| request to businessserver PublishRelease, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.PublishRelease(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver PublishRelease, %+v", err)
	}
