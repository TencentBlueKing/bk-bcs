...
func (act *ListAction) list() (pbcommon.ErrCode, string) {
	r := &pbbusinessserver.QueryShardingDBListReq{
		Seq: act.req.Seq,
	}
	...
	logger.V(2).Infof("QueryShardingDBList[%d]| request to businessserver QueryShardingDBList, %+v", act.req.Seq, r)

	resp, err := act.buSvrCli.QueryShardingDBList(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_AS_SYSTEM_UNKONW, fmt.Sprintf("request to businessserver QueryShardingDBList, %+v", err)
	}
