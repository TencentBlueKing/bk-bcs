...
func NewPublishAction(viper *viper.Viper, dataMgrCli pbdatamanager.DataManagerClient, publisher *mq.Publisher, pubTopic string,
	req *pb.PublishReleaseReq, resp *pb.PublishReleaseResp) *PublishAction {
	action := &PublishAction{viper: viper, dataMgrCli: dataMgrCli, publisher: publisher, pubTopic: pubTopic, req: req, resp: resp}
...
func (act *PublishAction) queryRelease() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("PublishRelease[%d]| request to datamanager QueryRelease, %+v", act.req.Seq, r)
...
func (act *PublishAction) publish() (pbcommon.ErrCode, string) {
	...
	signalling := &structs.Signalling{
		Type:       structs.SignallingTypePublish,
	...
	logger.V(2).Infof("PublishRelease[%d]| send publishing message now, %+v", act.req.Seq, signalling)
