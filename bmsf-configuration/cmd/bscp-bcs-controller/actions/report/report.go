...
func NewReportAction(viper *viper.Viper, dataMgrCli pbdatamanager.DataManagerClient,
	...
	action := &ReportAction{viper: viper, dataMgrCli: dataMgrCli, req: req, resp: resp}
...
func (act *ReportAction) report() (pbcommon.ErrCode, string) {
	...
	logger.V(2).Infof("Report[%d]| request to datamanager CreateAppInstanceRelease, %+v", act.req.Seq, r)

	resp, err := act.dataMgrCli.CreateAppInstanceRelease(ctx, r)
	if err != nil {
		return pbcommon.ErrCode_E_BCS_SYSTEM_UNKONW, fmt.Sprintf("request to datamanager CreateAppInstanceRelease, %+v", err)
	}
...
func (act *ReportAction) Do() error {
	// report app instance informations.
