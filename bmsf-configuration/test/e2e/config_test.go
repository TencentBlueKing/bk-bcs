...
func TestConfigSet(t *testing.T) {
	...
	{
		t.Logf("Case: create repeated config")
		api := testHost(fmt.Sprintf(createConfigAPIV2, e2eTestBizID, newAppID))
		resp, err := httpRequest("POST", api, "application/json", strings.NewReader(data))
		assert.Nil(err)
	...
	fpath := gjson.Get(data, "fpath").String()
	{
		t.Logf("Case: query config by name + fpath")
		api := testHost(fmt.Sprintf(queryConfigAPIV2, e2eTestBizID, newAppID))
		resp, err := httpRequest("GET", api+"?"+"name="+name+"&fpath="+fpath, "application/json", nil)
		assert.Nil(err)
		...
		assert.Equal(fpath, gjson.Get(body, "data.fpath").String(), body)
	...
	{
		t.Logf("Case: query config by cfg_id")
		api := testHost(fmt.Sprintf(queryConfigAPIV2, e2eTestBizID, newAppID))
		resp, err := httpRequest("GET", api+"?"+"cfg_id="+newCfgID, "application/json", nil)
		assert.Nil(err)
	...
	{
		t.Logf("Case: query config list")
		data, err := e2edata.QueryConfigListTestData(e2eTestBizID, newAppID, true, 0, 100)
		assert.Nil(err)

		api := testHost(fmt.Sprintf(listConfigAPIV2, e2eTestBizID, newAppID))
		resp, err := httpRequest("POST", api, "application/json", strings.NewReader(data))
		assert.Nil(err)
	...
	{
		t.Logf("Case: update config")
		data, err := e2edata.UpdateConfigTestData(e2eTestBizID, newAppID, newCfgID)
		assert.Nil(err)

		api := testHost(fmt.Sprintf(updateConfigAPIV2, e2eTestBizID, newAppID, newCfgID))
		...
		assert.Equal(gjson.Get(data, "fpath").String(), gjson.Get(queryBody, "data.fpath").String(), queryBody)
