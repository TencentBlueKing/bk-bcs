<template>
  <div
    ...
    <div class="bk-selector-wrapper">
      <input
        class="bk-selector-input" readonly="readonly"
      ...
      <i
        class="bcs-icon bcs-icon-close bk-selector-icon clear-icon"
      ...
      <div
        class="bk-spin-loading bk-spin-loading-mini bk-spin-loading-primary selector-loading-icon"
        v-show="isLoading">
          ...
          <template v-if="localList.length !== 0">
            <li
              :class="['bk-selector-list-item',
                       item.children && item.children.length ? 'bk-selector-group-list-item' : '']"
              ...
              <template v-if="item.children && item.children.length">
                ...
                <ul class="bk-selector-group-list">
                  <li v-for="(child, index) in item.children" :key="index" class="bk-selector-list-item">
                    <div
                      class="bk-selector-node bk-selector-sub-node"
                      ...
                      <div class="text" @click.stop="selectItem(child, $event)" :title="child[displayKey]">
                        <label
                          class="bk-form-checkbox bk-checkbox-small mr0 bk-selector-multi-label"
                          v-if="multiSelect" style="line-height: 42px;">
                          <input
                            type="checkbox"
                      ...
                      <div class="bk-selector-tools" v-if="tools !== false">
                        <i
                          class="bcs-icon bcs-icon-edit2 bk-selector-list-icon"
                          v-if="tools.edit !== false"
                          @click.stop="editFn(index)"></i>
                        <i
                          class="bcs-icon bcs-icon-close bk-selector-list-icon"
              ...
              <template v-else>
                <div
                  ...
                  :class="{ 'bk-selector-selected': !multiSelect && item[settingKey] === selected,
...
export default {
  ...
  methods: {
    ...
    toggleSelect(data, event) {
      ...
      let $selected;
      let $selectedList;
      const { settingKey } = this;
      const isMultiSelect = this.multiSelect;
      const index = (data?.[settingKey] !== undefined) ? data[settingKey] : undefined;
