    ...
    <template slot="content">
      <div style="margin: -20px;" v-bkloading="{ isLoading: ccHostLoading }">
        ...
        <div style="min-height: 443px;">
          <table
            class="bk-table has-table-hover biz-table biz-cluster-create-table"
            :style="{ borderBottomWidth: candidateHostList.length ? '1px' : 0 }">
            <thead>
              <tr>
                <th style="width: 60px; text-align: right;">
                  <label class="bk-form-checkbox mt5">
                    <input
                      type="checkbox" name="check-all-host"
              ...
              <template v-if="curPageData.length">
                <tr v-for="(host, index) in curPageData" @click.stop="rowClick" :key="index">
                  <td style="width: 60px; text-align: right; ">
                    <label class="bk-form-checkbox mt5">
                      <input
                        type="checkbox" name="check-host"
                        v-model="host.isChecked" @click.stop="selectHost(candidateHostList)"
  ...
  methods: {
    async handleGetNodeOverview() {
      ...
      for (let i = 0; i < this.curPageData.length; i++) {
        promiseList.push(this.$store.dispatch('cluster/getNodeOverview', {
    ...
    selectHost(hosts = this.candidateHostList) {
      ...
      setTimeout(() => {
        ...
        const canSelectedHosts = hosts.filter(host => host.status === 'RUNNING');
