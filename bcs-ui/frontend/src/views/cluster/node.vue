<template>
  <div class="biz-content">
    ...
    <div class="biz-content-wrapper">
      ...
      <div v-if="!exceptionCode" class="biz-cluster-node-wrapper">
        ...
        <div class="biz-cluster-tab-content">
          <bcs-alert
            type="info" class="biz-cluster-node-tip"
          ...
          <div class="biz-cluster-node-content">
            <div class="biz-cluster-node-header">
              ...
              <div class="biz-searcher-wrapper">
                <node-searcher
                  :cluster-id="clusterId" :project-id="projectId" ref="searcher"
                ...
                <tbody>
                  <template v-if="nodeList.length">
                    <tr v-for="(node, index) in curNodeList" :key="index">
                      ...
                      <td>
                        <ring-cell
                          :percent="node.cpuMetric" :fill-color="'#3ede78'"
                      ...
                      <td>
                        <ring-cell
                          :percent="node.memMetric" :fill-color="'#3a84ff'"
                      ...
                      <td>
                        <ring-cell
                          :percent="node.diskMetric" :fill-color="'#853cff'"
                      ...
                      <td>
                        <ring-cell
                          :percent="node.diskioMetric" :fill-color="'#853cff'"
    ...
    <bk-dialog
      ...
      <template slot="content">
        <div :class="{ 'stopDialog-content': true, 'font-content': !$INTERNAL }">{{stopDialogConf.content}}</div>
      </template>
      <div slot="footer">
        <div class="bk-dialog-outer">
          <template v-if="isUpdating">
...
export default {
  ...
  methods: {
    ...
    async handleGetTaskData(node) {
      ...
      this.taskData = steps.map(step => latestTask?.steps[step]);
