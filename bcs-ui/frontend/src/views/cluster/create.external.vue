      ...
      <div v-else class="biz-cluster-create-form-wrapper">
        <template v-if="clusterClassify !== 'public'">
          ...
          <div class="form-item bk-form-item" :class="isEn ? 'en' : ''">
            ...
            <div class="form-item-inner">
              <input
                maxlength="64" type="text" class="bk-form-input cluster-name" :placeholder="$t('请输入集群名称')"
  ...
  methods: {
    ...
    selectHost(hosts = this.candidateHostList) {
      ...
      setTimeout(() => {
        ...
        const selectedHosts = hosts.filter(host => host.isChecked === true && !host.is_used && String(host.agent) === '1');
