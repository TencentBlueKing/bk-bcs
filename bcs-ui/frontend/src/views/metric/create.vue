...
export default {
  ...
  methods: {
    ...
    changeSelectedService(displayName) {
      ...
      const curSelectedService = this.serviceList.find(item => item.displayName === this.createParams.displayName);
    ...
    async confirmCreateMetric() {
      ...
      const { port } = this.createParams;
      const serviceName = this.createParams.service_name;
      const { selector } = this.createParams;
      const { interval } = this.createParams;
