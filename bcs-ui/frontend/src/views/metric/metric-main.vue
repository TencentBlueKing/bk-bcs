  ...
  methods: {
    ...
    searchMetric(resetPage, notLoading = false) {
      ...
      if (this.searchKeyWord === '') {
        ...
        results = this.dataListTmp.filter(m => m.name.indexOf(this.searchKeyWord) > -1);
    ...
    async confirmCreateMetric() {
      ...
      const { timeout } = me.createParams;
      const { frequency } = me.createParams;
    ...
    async confirmEditMetric() {
      ...
      const { timeout } = me.editParams;
      const { frequency } = me.editParams;
