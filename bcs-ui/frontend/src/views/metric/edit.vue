...
export default {
    ...
    isShow: {
      async handler(newVal) {
        ...
        if (newVal) {
          ...
          const curSelectedService = this.serviceList.find(item => (item.resourceName === this.editParams.service_name)
  ...
  methods: {
    ...
    async confirmEditMetric() {
      ...
      const { port } = this.editParams;
      const { selector } = this.editParams;
      const { interval } = this.editParams;
