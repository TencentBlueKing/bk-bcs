    ...
    <div class="biz-content-wrapper pt0">
      <div class="biz-configuration-instantiation-wrapper">
        ...
        <transition name="fade">
          <div
            class="biz-configuration-instantiation-content" v-if="previewShow"
...
export default {
  ...
  methods: {
    ...
    clearNamespaceStatus() {
      const { selectedNamespaceList } = this;
      this.selectedNamespaceCluster = {};
      selectedNamespaceList.forEach((ns) => {
        ns.isSelected = false;
    ...
    async previewNamespaceByForm(ns, index) {
      ...
      try {
        if (this.lbServiceList.length) {
          ...
          const curTplsetVer = this.tplsetVerList.filter(tplsetVer => `${tplsetVer.id}-${tplsetVer.show_version_id}` === this.tplsetVerId)[0] || {};
    ...
    variableValChange() {
      ...
      previewList.forEach((preview, previewIndex) => {
        ...
        scrollTopList.push(this.editorConfig.editors[previewIndex]
      ...
      me.$bkInfo({
        ...
        async confirmFn() {
          ...
          try {
            ...
            const hasNoProd = !!me.selectedNamespaceList.filter(item => item.environment !== 'prod')[0];
      ...
      try {
        ...
        if (resCluster) {
          ...
          resNamespaces.forEach((ns) => {
            const inItemNamespaces = itemNamespaces.find(itemNs => itemNs.id === ns.id && itemNs.name === ns.name);
            if (inItemNamespaces) {
