      ...
      <div class="biz-tab-box" v-show="!isDataLoading">
        ...
        <div class="biz-tab-content" v-bkloading="{ isLoading: isTabChanging }">
          <bk-alert type="info" class="mb20">
            <div slot="title">
              {{$t('deploy.templateset.daemonSetExplanation')}}，
              <a class="bk-text-button" :href="PROJECT_CONFIG.k8sDaemonset" target="_blank">{{$t('plugin.tools.docs')}}</a>
            </div>
          </bk-alert>
          <template v-if="!daemonsets.length">
            <div class="biz-guide-box mt0">
              <bk-button icon="plus" type="primary" @click.stop.prevent="addLocalApplication">
                <span style="margin-left: 0;">{{$t('generic.button.add')}}DaemonSet</span>
              </bk-button>
            </div>
          </template>
          <template v-else>
            <div class="biz-configuration-topbar">
              <div class="biz-list-operation">
                ...
                <bcs-popover ref="applicationTooltip" :content="$t('deploy.templateset.addDaemonSet')" placement="top">
  ...
  methods: {
    ...
    removeApplication(application, index) {
      ...
      this.$bkInfo({
        ...
        content: this.$createElement('p', { style: { 'text-align': 'left' } }, `${this.$t('deploy.templateset.deleteDaemonSet')}：${application.config.metadata.name || this.$t('deploy.templateset.unnamed')}`),
        confirmFn() {
