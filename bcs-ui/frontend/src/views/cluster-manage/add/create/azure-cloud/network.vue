    ...
    <DescList size="middle" :title="$t('cluster.create.label.basicConfig')">
      ...
      <bk-form-item
        ...
        <bcs-select :clearable="false" searchable v-model="networkConfig.clusterAdvanceSettings.networkType">
          <bcs-option id="ACO" name="Azure CNI Overlay"></bcs-option>
          <bcs-option id="ACNS" name="Azure CNI Node Subnet"></bcs-option>
        </bcs-select>
        <div id="netPlugin">
...
import { cloudSecurityGroups } from '@/api/base';
...
//   const data = await cloudSecurityGroups({
