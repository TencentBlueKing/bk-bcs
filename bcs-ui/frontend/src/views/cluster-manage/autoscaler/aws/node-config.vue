          ...
          <bk-form-item :label="$t('cluster.ca.nodePool.create.loginType.text')">
            ...
            <div class="bg-[#F5F7FA] p-[16px] mt-[4px]">
              <template v-if="loginType === 'password'">
                ...
                <bk-form-item
                  :label="$t('awsCA.password.confirm')"
              ...
              <template v-else-if="loginType === 'ssh'">
                <bk-form-item
                  ...
                  property="nodePoolConfig.launchTemplate.initLoginUsername"
                  error-display-type="normal">
                  <bcs-input disabled v-model="nodePoolConfig.launchTemplate.initLoginUsername"></bcs-input>
                </bk-form-item>
                <bk-form-item
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const handleGetNodeRoles = async () => {
      ...
      nodeRoles.value = await $store.dispatch('clustermanager/cloudNoderoles', {
    ...
    // 登录方式
    const loginType = ref<'ssh'|'password'>('ssh');
