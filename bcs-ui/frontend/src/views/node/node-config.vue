<template>
    <bk-form class="node-config" :model="nodePoolConfig" :rules="nodePoolConfigRules" ref="formRef">
        ...
        <bk-form-item :label="$t('操作系统')"
            ...
            <bcs-select :loading="osImageLoading"
                v-model="nodePoolConfig.launchTemplate.imageInfo.imageID"
                searchable
                :disabled="isEdit"
                :clearable="false">
                <bcs-option v-for="imageItem in osImageList"
        ...
        <bk-form-item>
            <div class="mb15" style="display: flex;">
                <div class="prefix-select">
                    ...
                    <bcs-select v-model="nodePoolConfig.launchTemplate.CPU"
                        searchable
                        :clearable="false"
                        :disabled="isEdit">
                        <bcs-option v-for="cpuItem in getSchemaByProp('launchTemplate.CPU').enum"
                            :key="cpuItem"
                            :id="cpuItem"
                            :name="cpuItem">
                        </bcs-option>
                ...
                <div class="prefix-select ml30">
                    ...
                    <bcs-select v-model="nodePoolConfig.launchTemplate.Mem"
                        searchable
                        :clearable="false"
                        :disabled="isEdit">
                        <bcs-option v-for="memItem in getSchemaByProp('launchTemplate.Mem').enum"
                            :key="memItem"
                            :id="memItem"
                            :name="memItem">
                        </bcs-option>
            ...
            <bcs-table :data="instanceList"
                ...
                <bcs-table-column :label="$t('机型')" prop="typeName" show-overflow-tooltip>
                    <template #default="{ row }">
                        <bcs-radio :value="nodePoolConfig.launchTemplate.instanceType === row.nodeType"
            ...
            <div class="mt25" style="display:flex;align-items:center;">
                <div class="prefix-select">
                    ...
                    <bcs-select v-model="nodePoolConfig.launchTemplate.systemDisk.diskType"
                        :disabled="isEdit"
                        :clearable="false"
                        class="min-width-150">
                        <bcs-option v-for="diskItem in diskEnum"
                            :key="diskItem.id"
                            :id="diskItem.id"
                            :name="diskItem.name">
                        </bcs-option>
            ...
            <template v-if="showDataDisks">
                <div class="panel" v-for="(disk, index) in nodePoolConfig.launchTemplate.dataDisks" :key="index">
                    ...
                    <div class="panel-item mt10">
                        <bk-checkbox :disabled="isEdit"
                            v-model="disk.autoFormatAndMount">
                            {{$t('格式化并挂载于')}}
                        </bk-checkbox>
                        <template v-if="disk.autoFormatAndMount">
                            <bcs-select class="min-width-80 ml10"
                                :disabled="isEdit"
                                v-model="disk.fileSystem"
                                :clearable="false">
                                <bcs-option v-for="fileSystem in getSchemaByProp('launchTemplate.dataDisks.fileSystem').enum"
                                    :key="fileSystem"
                                    :id="fileSystem"
                                    :name="fileSystem">
                                </bcs-option>
        ...
        <bk-form-item :label="$t('安全组')"
            ...
            error-display-type="normal"
            required>
            <bcs-select :loading="securityGroupsLoading"
                v-model="nodePoolConfig.launchTemplate.securityGroupIDs"
                multiple
                searchable
                :disabled="isEdit">
                <bcs-option v-for="securityGroup in securityGroupsList"
        ...
        <bk-form-item :label="$t('支持子网')"
            ...
            <bcs-table
                ...
                <bcs-table-column :label="$t('子网ID')" min-width="150">
                    <template #default="{ row }">
                        <bk-checkbox :disabled="!(curInstanceItem.zones && curInstanceItem.zones.includes(row.zone)) || isEdit"
        ...
        <bk-form-item :label="$t('运行时版本')">
            <bcs-select v-model="nodePoolConfig.nodeTemplate.runtime.runtimeVersion"
                :clearable="false"
                :disabled="isEdit">
                <bcs-option v-for="version in versionList"
                    :key="version"
                    :id="version"
                    :name="version">
                </bcs-option>
...
<script lang="ts">
    ...
    export default defineComponent({
        ...
        setup (props, ctx) {
                ...
                'nodePoolConfig.launchTemplate.initLoginPassword': [
                    {
                        ...
                        trigger: 'blur',
                        validator: () => {
                            return isEdit.value || nodePoolConfig.value.launchTemplate.initLoginPassword.length > 0
                        }
                    },
                    {
                ...
                confirmPassword: [
                    {
                        ...
                        trigger: 'blur',
                        validator: () => {
                            return confirmPassword.value === nodePoolConfig.value.launchTemplate.initLoginPassword
                        }
                    }
                ],
                // 安全组校验
                'nodePoolConfig.launchTemplate.securityGroupIDs': [
                    {
                        ...
                        trigger: 'blur',
                        validator: () => {
                            return !!nodePoolConfig.value.launchTemplate.securityGroupIDs.length
                        }
                    }
                ],
                // 子网校验
                'nodePoolConfig.autoScaling.subnetIDs': [
                    {
                        ...
                        trigger: 'blur',
                        validator: () => {
                            return !!nodePoolConfig.value.autoScaling.subnetIDs.length
                        }
                    }
                ]
            })
            ...
            const curInstanceItem = computed(() => {
                return instanceTypesList.value.find(instance =>
                    instance.nodeType === nodePoolConfig.value.launchTemplate.instanceType) || {}
            })
            watch(() => [
            ...
            const versionList = computed(() => {
                return nodePoolConfig.value.nodeTemplate.runtime.containerRuntime === 'docker'
                    ? ['18.6', '19.3']
                    : ['1.3.4', '1.4.3']
            })
