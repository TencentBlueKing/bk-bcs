<template>
    <BcsContent>
        ...
        <!-- 标签 -->
        <bcs-dialog theme="primary"
            v-model="showLabels"
            :show-footer="false"
            title="Labels"
            header-position="left"
            width="600">
            <bcs-table :data="labels"
                :outer-border="false"
                :header-border="false"
    ...
    export default defineComponent({
        ...
        setup (props, ctx) {
            ...
            const labels = computed(() => {
                return Object.keys(nodePoolData.value?.nodeTemplate?.labels || {}).map(key => {
                    return {
                        key,
            ...
            const taints = computed(() => {
                return nodePoolData.value?.nodeTemplate?.taints || []
            })
