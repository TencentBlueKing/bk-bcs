...
export default {
  ...
  computed: {
    ...
    curLabelList() {
      const { customs } = this.curApp.release;
      const answers = {};
      const list = [];
  ...
  methods: {
    ...
    setAppDetail() {
      const { files } = this.originReleaseData.release.chartVersionSnapshot;
      const tplName = this.originReleaseData.release.chartVersionSnapshot.name;
      const { questions } = this.originReleaseData.release.chartVersionSnapshot;
      ...
      if (!valueFileName) {
        ...
        if (this.isLocked) {
          ...
          if (String(this.tplVersionId) === this.originReleaseVersion) {
            ...
            const matchName = fileNames.find(name => name.endsWith(this.instanceValueFileName));
            // 默认用原来的value文件
            if (matchName) {
