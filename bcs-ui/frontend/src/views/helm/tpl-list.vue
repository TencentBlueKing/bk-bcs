                            ...
                            <tr>
                              ...
                              <td class="action">
                                <span
                                  ...
                                  <router-link
                                    class="bk-button bk-primary mr5"
              ...
              <div class="bk-form-content">
                <div class="bk-dropdown-box" style="width: 300px;">
                  <bcs-select
                    ...
                    :placeholder="$t('请选择')">
                    <bk-option
                      v-for="item in delInstanceDialogConf.versions"
                      :key="item.id"
                      :name="item.version"
                      :id="item.id">
                    </bk-option>
      ...
      <div slot="footer">
        <div class="bk-dialog-outer">
          <template v-if="delInstanceDialogConf.versionIds.length && !delInstanceDialogConf.releases.length">
            <bk-button
              type="primary" :loading="isVersionDeleting" class="bk-dialog-btn bk-dialog-btn-confirm"
    ...
    <bcs-dialog
      ...
      <bcs-form form-type="vertical">
        <bcs-form-item :label="$t('选择要下载的版本')">
          <bcs-select
            v-model="downloadDialog.downloadVersion"
            :loading="isTplVersionLoading"
            :clearable="false"
            @change="handleSelectVersion">
            <bcs-option
              v-for="item in downloadDialog.versions"
              :key="item.version"
              :id="item.version"
              :name="item.version">
            </bcs-option>
  ...
  methods: {
    ...
    async getTplList() {
      ...
      try {
        ...
        // 进行分类，包括项目仓库和私有仓库
        const tplList = res.data.filter(item => item.defaultChartVersion);
