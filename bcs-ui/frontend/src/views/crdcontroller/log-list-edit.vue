        ...
        <section class="log-wrapper">
          ...
          <section v-if="detailData.log_source_type === 'selected_containers'">
            <div class="form-row">
              <BkFormItem
                ...
                error-display-type="normal">
                <bcs-select
                  v-model="detailData.workload.kind"
                  :placeholder="$t('应用类型')"
                  :disabled="isEdit"
              ...
              <BkFormItem
                ...
                error-display-type="normal">
                <bcs-input
                  v-model="detailData.workload.name"
                  :disabled="isEdit"
            ...
            <BkFormItem
              ...
              error-display-type="normal">
              <div
                v-for="(item, index) in detailData.workload.container_confs"
                :key="index"
          ...
          <section v-else-if="detailData.log_source_type === 'selected_labels'">
            ...
            <BkFormItem
              ...
              error-display-type="normal">
              <span
                class="add-express-btn" v-if="!detailData.selector.match_expressions.length"
              ...
              <template v-else>
                <div
                  v-for="(item, index) in detailData.selector.match_expressions"
                  :key="index"
  ...
  setup(props, ctx) {
    ...
    const rules = ref({
      config_name: [
        ...
        {
          validator: () => /^[A-Za-z0-9_]+$/.test(detailData.value.config_name)
      ...
      'workload.container_confs': [
        {
          validator: () => detailData.value.workload.container_confs.every(item => !!item.name),
          message: $i18n.t('输入容器名'),
          trigger: 'custom',
        },
      ],
      'selector.match_labels': [{
        validator: () => Object.keys(detailData.value.selector.match_labels).length
