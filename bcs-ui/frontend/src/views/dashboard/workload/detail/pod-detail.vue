                    ...
                    <bk-table :data="container">
                        ...
                        <bk-table-column :label="$t('操作')" width="200" :resizable="false" :show-overflow-tooltip="false">
                            <template #default="{ row }">
                                ...
                                <bk-popover placement="bottom" theme="light dropdown" :arrow="false" v-if="row.container_id && !isSharedCluster">
                                    <bk-button style="cursor: default;" text class="ml10">{{ $t('日志查询') }}</bk-button>
                                    <div slot="content">
                                        <ul>
    ...
    export default defineComponent({
        ...
        setup (props, ctx) {
            ...
            const handleShowTerminal = (row) => {
                const url = `${window.DEVOPS_BCS_API_URL}/web_console/projects/${projectId.value}/clusters/${clusterId.value}/?namespace=${props.namespace}&pod_name=${props.name}&container_id=${row.container_id}`
