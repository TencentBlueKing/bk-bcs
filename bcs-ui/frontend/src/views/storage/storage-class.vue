        ...
        <div class="biz-pv">
          <div class="biz-table-wrapper">
            <bk-table
              ...
              <bk-table-column :label="$t('来源')" prop="provisioner" min-width="130">
                <template slot-scope="{ row }">
                  <bcs-popover placement="top" :delay="500">
                    <p class="item-name">{{ row.provisioner || '--' }}</p>
                    <template slot="content">
                      <p style="text-align: left; white-space: normal;word-break: break-all;">{{ row.provisioner || '--' }}</p>
                    </template>
                  </bcs-popover>
                </template>
              </bk-table-column>
              <bk-table-column :label="$t('云盘类型')" prop="cloud_disk_type_name" min-width="130">
                <template slot-scope="{ row }">
                  {{ row.cloud_disk_type_name || '--' }}
                </template>
              </bk-table-column>
              <bk-table-column :label="$t('计费模式')" prop="pay_mode_name" min-width="130">
                <template slot-scope="{ row }">
                  {{ row.pay_mode_name || '--' }}
                </template>
              </bk-table-column>
              <bk-table-column :label="$t('回收策略')" prop="reclaim_policy" min-width="130">
                <template slot-scope="{ row }">
                  {{ row.reclaim_policy || '--' }}
                </template>
              </bk-table-column>
...
export default {
  ...
  methods: {
    ...
    /**
             ...
             */
    searchStorageByWord() {
      const search = String(this.searchKeyword || '').trim()
        .toLowerCase();
      let results = [];
      if (search === '') {
        this.dataList.splice(0, this.dataList.length, ...this.dataListTmp);
      } else {
        results = this.dataListTmp.filter(m => m.name.toLowerCase().indexOf(search) > -1);
