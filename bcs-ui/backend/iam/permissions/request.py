...
class ResourceRequest(ABC):
    ...
    def make_resources(self) -> List[Resource]:
        if isinstance(self.res, str):
            return [Resource(settings.APP_CODE, self.resource_type, self.res, self._make_attribute(self.res))]

        return [
            Resource(settings.APP_CODE, self.resource_type, res_id, self._make_attribute(res_id))
...
class ActionResourcesRequest:
    ...
    def to_action(self) -> Union[models.ActionWithResources, models.ActionWithoutResources]:
        ...
        if self.resources:
            ...
            related_resource_type = models.RelatedResourceType(settings.APP_CODE, self.resource_type, instances)
