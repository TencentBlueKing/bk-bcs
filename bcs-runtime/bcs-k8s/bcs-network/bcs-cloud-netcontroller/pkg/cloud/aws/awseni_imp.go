...
func (c *Client) queryInstanceInfo(instanceIP string) (*ec2.Instance, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("describe instance failed, err %s", err.Error())
	}

	blog.V(2).Infof("aws DescribeInstances response %s", resp.String())

	if len(resp.Reservations) == 0 {
		return nil, fmt.Errorf("no reservations info in DescribeInstances response")
	}

	reservation := resp.Reservations[0]
	if len(reservation.Instances) == 0 {
		return nil, fmt.Errorf("no instance in reservation %s", aws.StringValue(reservation.ReservationId))
	}
