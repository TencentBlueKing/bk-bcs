...
var _ = Describe("DRPlan Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			...
			By("creating the custom resource for the Kind DRPlan")
			err = k8sClient.Get(ctx, typeNamespacedName, drplan)
			if err != nil && errors.IsNotFound(err) {
				resource := &drv1alpha1.DRPlan{
					ObjectMeta: metav1.ObjectMeta{
						Name:      resourceName,
						Namespace: "default",
					},
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance DRPlan")
		...
		It("should successfully reconcile the resource", func() {
			...
			controllerReconciler := &DRPlanReconciler{
				Client: k8sClient,
