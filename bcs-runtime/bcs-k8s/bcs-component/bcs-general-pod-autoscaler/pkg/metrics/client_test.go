...
import (
	...
	autoscalingapi "github.com/Tencent/bk-bcs/bcs-runtime/bcs-k8s/bcs-component/bcs-general-pod-autoscaler/pkg/apis/autoscaling/v1alpha1"
)
	...
	if isResource {
		...
		fakeCMClient.AddReactor("get", "*", func(action core.Action) (handled bool, ret runtime.Object, err error) {
			...
			if getForAction.GetName() == "*" {
				...
				rmetricPoint := tc.reportedMetricPoints[0]
				timestamp := offsetTimestampBy(rmetricPoint.timestamp)
					...
					Items: []cmapi.MetricValue{
						{
							...
							Value: *resource.NewMilliQuantity(int64(rmetricPoint.level), resource.DecimalSI),
						},
					},
				}
