...
func validateHorizontalPodAutoscalerSpec(autoscaler autoscaling.GeneralPodAutoscalerSpec, fldPath *field.Path,
	...
	if autoscaler.MaxReplicas < 1 {
		allErrs = append(allErrs, field.Invalid(fldPath.Child("maxReplicas"), autoscaler.MaxReplicas,
	...
	if refErrs := ValidateCrossVersionObjectReference(autoscaler.ScaleTargetRef,
		fldPath.Child("scaleTargetRef")); len(refErrs) > 0 {
		allErrs = append(allErrs, refErrs...)
	}
	if autoscaler.AutoScalingDrivenMode.MetricMode != nil {
		if refErrs := validateMetrics(autoscaler.AutoScalingDrivenMode.MetricMode.Metrics, fldPath.Child("metrics"),
			autoscaler.MinReplicas); len(refErrs) > 0 {
			allErrs = append(allErrs, refErrs...)
		}
	}
	if autoscaler.AutoScalingDrivenMode.WebhookMode != nil {
		if refErrs := validateWebhook(autoscaler.AutoScalingDrivenMode.WebhookMode.WebhookClientConfig,
			fldPath.Child("webhook")); len(refErrs) > 0 {
			allErrs = append(allErrs, refErrs...)
		}
	}
	if autoscaler.AutoScalingDrivenMode.TimeMode != nil {
		if refErrs := validateTime(autoscaler.AutoScalingDrivenMode.TimeMode.TimeRanges,
			fldPath.Child("time")); len(refErrs) > 0 {
			allErrs = append(allErrs, refErrs...)
		}
	}
	if autoscaler.AutoScalingDrivenMode.EventMode != nil {
		if refErrs := validateEvent(autoscaler.AutoScalingDrivenMode.EventMode.Triggers,
			fldPath.Child("event")); len(refErrs) > 0 {
			allErrs = append(allErrs, refErrs...)
		}
	}
...
// ErrorList with any errors.
func ValidateCrossVersionObjectReference(ref autoscaling.CrossVersionObjectReference,
...
func ValidateHorizontalPodASU(newAutoscaler, oldAutoscaler *autoscaling.GeneralPodAutoscaler) field.ErrorList {
	...
	allErrs = append(allErrs, apimachineryvalidation.ValidateNonnegativeField(int64(status.CurrentReplicas),
		field.NewPath("status", "currentReplicas"))...)
	allErrs = append(allErrs, apimachineryvalidation.ValidateNonnegativeField(int64(status.DesiredReplicas),
		field.NewPath("status", "desiredReplicas"))...)
	return allErrs
}
...
var validSelectPolicyTypes = sets.NewString(string(autoscaling.MaxPolicySelect), string(autoscaling.MinPolicySelect),
...
func validateScalingRules(rules *autoscaling.GPAScalingRules, fldPath *field.Path) field.ErrorList {
	...
	if rules != nil {
		if rules.StabilizationWindowSeconds != nil && *rules.StabilizationWindowSeconds < 0 {
			allErrs = append(allErrs, field.Invalid(fldPath.Child("stabilizationWindowSeconds"),
				rules.StabilizationWindowSeconds, "must be greater than or equal to zero"))
		}
		...
		if rules.SelectPolicy != nil && !validSelectPolicyTypes.Has(string(*rules.SelectPolicy)) {
			allErrs = append(allErrs, field.NotSupported(fldPath.Child("selectPolicy"), rules.SelectPolicy,
				validSelectPolicyTypesList))
		}
...
func validateMetricSpec(spec autoscaling.MetricSpec, fldPath *field.Path) field.ErrorList {
	...
	case autoscaling.ObjectMetricSourceType:
		if spec.Object == nil {
			allErrs = append(allErrs, field.Required(fldPath.Child("object"),
				"must populate information for the given metric source"))
		}
		expectedField = "object"
	case autoscaling.PodsMetricSourceType:
		if spec.Pods == nil {
			allErrs = append(allErrs, field.Required(fldPath.Child("pods"),
				"must populate information for the given metric source"))
		}
		expectedField = "pods"
	...
	case autoscaling.ContainerResourceMetricSourceType:
		if spec.ContainerResource == nil {
			allErrs = append(allErrs, field.Required(fldPath.Child("containerResource"),
				"must populate information for the given metric source"))
		}
		expectedField = "containerResource"
	case autoscaling.ExternalMetricSourceType:
		if spec.External == nil {
			allErrs = append(allErrs, field.Required(fldPath.Child("external"),
				"must populate information for the given metric source"))
		}
...
func validateObjectSource(src *autoscaling.ObjectMetricSource, fldPath *field.Path) field.ErrorList {
	...
	allErrs = append(allErrs, ValidateCrossVersionObjectReference(src.DescribedObject,
		fldPath.Child("describedObject"))...)
	...
	if src.Target.Value == nil && src.Target.AverageValue == nil {
		allErrs = append(allErrs, field.Required(fldPath.Child("target").Child("averageValue"),
...
func validateExternalSource(src *autoscaling.ExternalMetricSource, fldPath *field.Path) field.ErrorList {
	...
	if src.Target.Value != nil && src.Target.AverageValue != nil {
		allErrs = append(allErrs, field.Forbidden(fldPath.Child("target").Child("value"),
...
func validateMetricTarget(mt autoscaling.MetricTarget, fldPath *field.Path) field.ErrorList {
	...
	if mt.Type != autoscaling.UtilizationMetricType &&
		...
		allErrs = append(allErrs, field.Invalid(fldPath.Child("type"), mt.Type,
			"must be either Utilization, Value, or AverageValue"))
	}

	if mt.AverageUtilization == nil && mt.AverageValue == nil && mt.Value == nil {
		allErrs = append(allErrs, field.Invalid(fldPath.Child(" utilization, value and averageValue"), mt.Type,
	...
	if mt.AverageUtilization != nil && *mt.AverageUtilization < 1 {
		allErrs = append(allErrs, field.Invalid(fldPath.Child("averageUtilization"), mt.AverageUtilization,
			"must be greater than 0"))
	}
