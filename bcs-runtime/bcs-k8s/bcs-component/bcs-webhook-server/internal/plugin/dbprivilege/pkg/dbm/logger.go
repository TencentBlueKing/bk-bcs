...
const (
	...
	// NOCC:hardcode-password
	bkAppSecretPattern = `"bk_app_secret":[ ]*"[^"]*"` // nolint
	bkUsernamePattern  = `"bk_username":[ ]*"[^"]*"`
	hiddenAppCode      = `"bk_app_code":"***"`
	hiddenAppSecret    = `"bk_app_secret":"***"` // nolint
	hiddenUsername     = `"bk_username":"***"`
)

var (
	regexAppCode, _   = regexp.Compile(bkAppCodePattern)   // nolint
	regexAppSecret, _ = regexp.Compile(bkAppSecretPattern) // nolint
	regexUsername, _  = regexp.Compile(bkUsernamePattern)  // nolint
)
