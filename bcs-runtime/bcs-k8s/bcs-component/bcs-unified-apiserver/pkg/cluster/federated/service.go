...
// List 查询 Service 列表, Json格式返回
func (s *ServiceStor) List(ctx context.Context, namespace string, opts metav1.ListOptions) (*v1.ServiceList, error) {
	result, err := s.masterClient.CoreV1().Services(namespace).List(ctx, opts)
	if err != nil {
		return nil, err
	}
...
func (s *ServiceStor) Delete(ctx context.Context, namespace string, name string, opts metav1.DeleteOptions) (*metav1.Status, error) {
	...
	if err := s.masterClient.CoreV1().Services(namespace).Delete(ctx, name, opts); err != nil {
		return nil, err
	}
