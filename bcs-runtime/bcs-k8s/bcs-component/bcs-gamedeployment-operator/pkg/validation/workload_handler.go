...
func (whsvr *WebhookServer) handleWorkload(ar v1.AdmissionReview) *v1.AdmissionResponse {
	klog.V(2).Info("handlinging gamedeployment")
	...
	if len(ar.Request.OldObject.Raw) == 0 {
		klog.Warningf("Skip to validate GameDeployment %s deletion for no old object, "+
	...
	if err := whsvr.validateWorkloadDeletion(gd.Labels, gd.Spec.Replicas); err != nil {
		return toV1AdmissionResponse(err)
	}
