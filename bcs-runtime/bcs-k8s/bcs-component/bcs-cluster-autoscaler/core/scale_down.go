...
func computeScaleDownResourcesLeftLimits(nodes []*apiv1.Node, resourceLimiter *cloudprovider.ResourceLimiter,
...
func calculateScaleDownGpusTotal(nodes []*apiv1.Node, cp cloudprovider.CloudProvider,
	...
	for _, node := range nodes {
		...
		if !cacheHit {
			...
			if err != nil {
				return nil, errors.ToAutoscalerError(errors.CloudProviderError, err).AddPrefix(
					"can not get gpu count for node %v when calculating cluster gpu usage")
			}
			if nodeGroup != nil {
...
func computeScaleDownResourcesDelta(cp cloudprovider.CloudProvider, node *apiv1.Node, nodeGroup cloudprovider.NodeGroup,
	...
	if cloudprovider.ContainsGpuResources(resourcesWithLimits) {
		gpuType, gpuCount, err := gpu.GetNodeTargetGpus(cp.GPULabel(), node, nodeGroup)
		if err != nil {
			return scaleDownResourcesDelta{}, errors.ToAutoscalerError(errors.CloudProviderError, err).AddPrefix(
				"Failed to get node %v gpu: %v", node.Name)
		}
...
func NewScaleDown(context *context.Context, clusterStateRegistry *clusterstate.ClusterStateRegistry,
	...
	return &ScaleDown{
		...
		nodeUtilizationMap:   make(map[string]simulatorinternal.UtilizationInfo),
...
func (sd *ScaleDown) UpdateUnneededNodes(
	...
	utilizationMap := make(map[string]simulatorinternal.UtilizationInfo)
	...
	for _, node := range filteredNodesToCheck {
		...
		if isNodeBeingDeleted(node, timestamp) {
			...
			metrics.UpdateUnremovableNodes(node.Name, string(simulator.CurrentlyBeingDeleted), "", "")
			continue
		}
		...
		if hasNoScaleDownAnnotation(node) {
			...
			metrics.UpdateUnremovableNodes(node.Name, string(simulator.ScaleDownDisabledAnnotation), "", "")
			continue
		}
		...
		if !found {
			...
			metrics.UpdateUnremovableNodes(node.Name, string(simulator.UnexpectedError), "", "")
			continue
		}

		utilInfo, err := simulatorinternal.CalculateUtilization(node, nodeInfo, sd.context.IgnoreDaemonSetsUtilization,
		...
		if !sd.isNodeBelowUtilzationThreshold(node, utilInfo) {
			klog.V(4).Infof("Node %s is not suitable for removal - %s utilization too big (%f)", node.Name,
				utilInfo.ResourceName, utilInfo.Utilization)
			metrics.UpdateUnremovableNodes(node.Name, string(simulator.NotUnderutilized), "", "")
			continue
		}
	...
	if len(unremovable) > 0 {
		...
		for _, un := range unremovable {
			sd.unremovableNodes[un.Node.Name] = unremovableTimeout
			if un.BlockingPod != nil {
				metrics.UpdateUnremovableNodes(un.Node.Name, string(un.Reason),
					fmt.Sprintf("%s/%s", un.BlockingPod.Pod.Namespace, un.BlockingPod.Pod.Name), string(un.BlockingPod.Reason))
			} else {
				metrics.UpdateUnremovableNodes(un.Node.Name, string(un.Reason), "", "")
			}
		}
...
func (sd *ScaleDown) markSimulationError(simulatorErr errors.AutoscalerError,
	...
	sd.unneededNodesList = make([]*apiv1.Node, 0)
	sd.unneededNodes = make(map[string]time.Time)
	sd.nodeUtilizationMap = make(map[string]simulatorinternal.UtilizationInfo)
	sd.clusterStateRegistry.UpdateScaleDownCandidates(sd.unneededNodesList, timestamp)
...
func (sd *ScaleDown) mapNodesToStatusScaleDownNodes(nodes []*apiv1.Node, nodeGroups map[string]cloudprovider.NodeGroup,
...
func (sd *ScaleDown) TryToScaleDown(allNodes []*apiv1.Node, pods []*apiv1.Pod,
	...
	scaleDownResourcesLeft := computeScaleDownResourcesLeftLimits(nodesWithoutMaster, resourceLimiter,
		sd.context.CloudProvider, currentTime)
	...
	for _, node := range nodesWithoutMaster {
		if val, found := sd.unneededNodes[node.Name]; found {
			...
			if ready && !val.Add(sd.context.ScaleDownUnneededTime).Before(currentTime) {
				metrics.UpdateUnremovableNodes(node.Name, string(simulator.NotUnneededLongEnough), "", "")
				continue
			}
			...
			if !ready && !val.Add(sd.context.ScaleDownUnreadyTime).Before(currentTime) {
				metrics.UpdateUnremovableNodes(node.Name, string(simulator.NotUnreadyLongEnough), "", "")
				continue
			}
			...
			if nodeGroup == nil || reflect.ValueOf(nodeGroup).IsNil() {
				...
				metrics.UpdateUnremovableNodes(node.Name, string(simulator.NotAutoscaled), "", "")
				continue
			}
			...
			if size-deletionsInProgress-tempNodesForNg <= nodeGroup.MinSize() {
				...
				metrics.UpdateUnremovableNodes(node.Name, string(simulator.NodeGroupMinSizeReached), "", "")
				continue
			}

			scaleDownResourcesDeltaNow, err := computeScaleDownResourcesDelta(sd.context.CloudProvider, node,
				nodeGroup, resourcesWithLimits)
			if err != nil {
			...
			checkResult := scaleDownResourcesLeft.checkScaleDownDeltaWithinLimits(scaleDownResourcesDeltaNow)
			if checkResult.exceeded {
				klog.V(4).Infof("Skipping %s - minimal limit exceeded for %v", node.Name, checkResult.exceededResources)
				metrics.UpdateUnremovableNodes(node.Name, string(simulator.MinimalResourceLimitExceeded), "", "")
	...
	emptyNodes := sd.getEmptyNodes(candidates, pods, sd.context.MaxEmptyBulkDelete,
		scaleDownResourcesLeft, tempNodesPerNodeGroup)
	if len(emptyNodes) > 0 {

		emptyNodesAfterFilter := make([]*apiv1.Node, 0)
		nodeInfos := buildNodeInfos(nodesWithoutMaster, getPodsInfos(pods))
	...
	nodesToRemove, unremovable, _, err := simulator.FindNodesToRemove(candidates, nodesWithoutMaster, nonExpendablePods,
	...
	// 被移除的节点需要先有 soft taint 阻止新 Pod 调度，减少极端情况
	// 否则移到下一轮循环再判断
	...
	nodeInfos := buildNodeInfos(nodesWithoutMaster, getPodsInfos(pods))
	delete(nodeInfos, toRemove.Node.Name)
	...
	sd.context.LogRecorder.Eventf(apiv1.EventTypeNormal, "ScaleDown", "Scale-down: removing node %s,"+
		" utilization: %v, pods to reschedule: %s", toRemove.Node.Name, utilization, strings.Join(podNames, ","))

	// Nothing super-bad should happen if the node is removed from tracker prematurely.
	simulatorinternal.RemoveNodeFromTracker(sd.usageTracker, toRemove.Node.Name, sd.unneededNodes)
	nodeDeletionStart := time.Now()
	...
	go func() {
		...
		if readinessMap[toRemove.Node.Name] {
			metrics.RegisterScaleDown(1, gpu.GetGpuTypeForMetrics(gpuLabel, availableGPUTypes, toRemove.Node, nodeGroup),
...
func (sd *ScaleDown) getEmptyNodesNoResourceLimits(candidates []*apiv1.Node, pods []*apiv1.Pod,
	maxEmptyBulkDelete int, tempNodesPerNodeGroup map[string]int) []*apiv1.Node {
...
func (sd *ScaleDown) scheduleDeleteEmptyNodes(emptyNodes []*apiv1.Node, client kube_client.Interface,
	...
	for _, node := range emptyNodes {
		...
		simulatorinternal.RemoveNodeFromTracker(sd.usageTracker, node.Name, sd.unneededNodes)
		...
		go func(nodeToDelete *apiv1.Node, nodeGroupForDeletedNode cloudprovider.NodeGroup) {
			...
			defer func() {
				if deleteErr != nil {
					_, err := deletetaint.CleanToBeDeleted(nodeToDelete, client)
					if err != nil {
						klog.Errorf("CleanToBeDeleted failed. Error: %v", err)
					}
...
func (sd *ScaleDown) deleteNode(node *apiv1.Node, pods []*apiv1.Pod,
	...
	defer func() {
		if !deleteSuccessful {
			_, err := deletetaint.CleanToBeDeleted(node, sd.context.ClientSet)
			if err != nil {
				klog.Errorf("CleanToBeDeleted failed. Error: %v", err)
			}
			if !drainSuccessful {
	...
	// attempt drain
	evictionResults, err := drainNode(node, podsToDrain, sd.context.ClientSet, sd.context.Recorder,
		sd.context.MaxGracefulTerminationSec, MaxPodEvictionTime, EvictionRetryTime, PodEvictionHeadroom)
	if err != nil {
		return status.NodeDeleteResult{ResultType: status.NodeDeleteErrorFailedToEvictPods,
	...
	if typedErr := deleteNodeFromCloudProvider(node, sd.context.CloudProvider, sd.context.Recorder,
		sd.clusterStateRegistry); typedErr != nil {
		return status.NodeDeleteResult{ResultType: status.NodeDeleteErrorFailedToDelete, Err: typedErr}
	}
...
func drainNode(node *apiv1.Node, pods []*apiv1.Pod, client kube_client.Interface, recorder kube_record.EventRecorder,
	...
	for range pods {
		...
		case <-time.After(time.Until(retryUntil) + 5*time.Second):
			// All pods initially had results with TimedOut set to true, so the ones that didn't receive an actual result
			// are correctly marked as timed out.
			return evictionResults, errors.NewAutoscalerError(errors.ApiCallError,
				"Failed to drain node %s/%s: timeout when waiting for creating evictions", node.Namespace, node.Name)
		}
	}
	...
	if len(evictionErrs) != 0 {
		return evictionResults, errors.NewAutoscalerError(errors.ApiCallError,
			"Failed to drain node %s/%s, due to following errors: %v", node.Namespace, node.Name, evictionErrs)
	}

	// Evictions created successfully, wait maxGracefulTerminationSec + podEvictionHeadroom to see
	// if pods really disappeared.
	for start := time.Now(); time.Since(start) < time.Duration(maxGracefulTerminationSec)*time.Second+
		podEvictionHeadroom; time.Sleep(5 * time.Second) {
		allGone := true
		for _, pod := range pods {
			...
			if err == nil && (podreturned == nil || podreturned.Spec.NodeName == node.Name) {
				klog.Errorf("Not deleted yet %v/%v", podreturned.Namespace, podreturned.Name)
				allGone = false
				break
			}
...
func deleteNodeFromCloudProvider(node *apiv1.Node, cloudProvider cloudprovider.CloudProvider,
	...
	if nodeGroup == nil || reflect.ValueOf(nodeGroup).IsNil() {
		return errors.NewAutoscalerError(errors.InternalError, "picked node that doesn't belong to a node group: %s",
			node.Name)
	}
