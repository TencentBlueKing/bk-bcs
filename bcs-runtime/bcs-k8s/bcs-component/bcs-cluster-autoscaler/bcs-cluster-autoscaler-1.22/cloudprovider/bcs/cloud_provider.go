...
func BuildCloudProvider(opts scalingconfig.Options, do cloudprovider.NodeGroupDiscoveryOptions,
	...
	if opts.CloudConfig != "" {
		config, fileErr := os.Open(opts.AutoscalingOptions.CloudConfig)
		if fileErr != nil {
			klog.Fatalf("Couldn't open cloud Provider configuration %s: %#v", opts.AutoscalingOptions.CloudConfig, fileErr)
		}
		defer config.Close()
