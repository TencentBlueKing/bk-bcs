...
var (
	...
	statusConfigMapName = flag.String("status-config-map-name", "cluster-autoscaler-status",
	...
	balancingIgnoreLabelsFlag = multiStringFlag("balancing-ignore-label",
		"Specifies a label to ignore in addition to the basic and cloud-provider set of labels when comparing "+
			"if two node groups are similar")
	...
	concurrentGceRefreshes = flag.Int("gce-concurrent-refreshes", 1,
		"Maximum number of concurrent refreshes per cloud object type.")
	enableProfiling                    = flag.Bool("profiling", false, "Is debug/pprof endpoint enabled")
	clusterAPICloudConfigAuthoritative = flag.Bool("clusterapi-cloud-config-authoritative", false,
		"Treat the cloud-config flag authoritatively (do not fallback to using kubeconfig flag). ClusterAPI only")
	cordonNodeBeforeTerminate = flag.Bool("cordon-node-before-terminating", false,
		"Should CA cordon nodes before terminating during downscale process")
	...
	daemonSetEvictionForOccupiedNodes = flag.Bool("daemonset-eviction-for-occupied-nodes", true,
		"DaemonSet pods will be gracefully terminated from non-empty nodes")
	userAgent = flag.String("user-agent", "cluster-autoscaler",
		"User agent used for HTTP calls.")
	...
	batchScaleUpCount = flag.Int("batch-scale-up-count", 100,
		"The number of nodes to scale up at a batch in webhook mode.")

	evictLatest = flag.Bool("should-evict-latest", true, "If true, get latest pod lists when scale down node")
)

// createAutoscalingOptions TODO
// nolint
func createAutoscalingOptions() scalingconfig.Options {
	...
	return scalingconfig.Options{
		...
		BatchScaleUpCount:         *batchScaleUpCount,
	}
}

// getKubeConfig get config of kube
...
// createKubeClient create kube client
...
// defaultLeaderElectionConfiguration construct default configuration of leader election
...
// parseMinMaxFlag parse MinMax flag
...
// validateMinMaxFlag validates flag of MinMax
...
// parseMultipleGpuLimits parse multiple gpu limits
...
// parseSingleGpuLimit parse single gpu limits
