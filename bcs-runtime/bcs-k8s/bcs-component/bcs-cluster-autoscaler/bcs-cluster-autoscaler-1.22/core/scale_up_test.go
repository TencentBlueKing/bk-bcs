...
func TestScaleUpMaxMemoryLimitHit(t *testing.T) {
	...
	config := &scaleTestConfig{
		...
		extraPods: []podConfig{
			{"p-new-1", 2000, 100 * MiB, 0, "", false},
			{"p-new-2", 2000, 100 * MiB, 0, "", false},
			{"p-new-3", 2000, 100 * MiB, 0, "", false},
		},
...
func TestScaleUpCapToMaxTotalNodesLimit(t *testing.T) {
	...
	config := &scaleTestConfig{
		...
		extraPods: []podConfig{
			{"p-new-1", 4000, 100 * MiB, 0, "", false},
			{"p-new-2", 4000, 100 * MiB, 0, "", false},
			{"p-new-3", 4000, 100 * MiB, 0, "", false},
		},
...
func TestWillConsiderGpuAndStandardPoolForPodWhichDoesNotRequireGpu(t *testing.T) {
	...
	config := &scaleTestConfig{
		...
		pods: []podConfig{
			{"gpu-pod-1", 2000, 1000 * MiB, 1, "gpu-node-1", true},
			{"std-pod-1", 2000, 1000 * MiB, 0, "std-node-1", false},
		},
		extraPods: []podConfig{
			{"extra-std-pod", 2000, 1000 * MiB, 0, "", true},
		},
...
func TestWillConsiderOnlyGpuPoolForPodWhichDoesRequiresGpu(t *testing.T) {
	...
	config := &scaleTestConfig{
		...
		extraPods: []podConfig{
			{"extra-gpu-pod", 2000, 1000 * MiB, 1, "", true},
		},
...
func TestWillConsiderAllPoolsWhichFitTwoPodsRequiringGpus(t *testing.T) {
	...
	config := &scaleTestConfig{
		...
		pods: []podConfig{
			{"gpu-pod-1", 2000, 1000 * MiB, 1, "gpu-1-node-1", true},
			{"gpu-pod-2", 2000, 1000 * MiB, 2, "gpu-2-node-1", true},
			{"gpu-pod-3", 2000, 1000 * MiB, 4, "gpu-4-node-1", true},
			{"std-pod-1", 2000, 1000 * MiB, 0, "std-node-1", false},
		},
		extraPods: []podConfig{
			{"extra-gpu-pod-1", 1, 1 * MiB, 1, "", true}, // CPU and mem negligible
			{"extra-gpu-pod-2", 1, 1 * MiB, 1, "", true}, // CPU and mem negligible
			{"extra-gpu-pod-3", 1, 1 * MiB, 1, "", true}, // CPU and mem negligible
		},
...
func (r reportingStrategy) BestOption(options []expander.Option,
	nodeInfo map[string]*schedulerframework.NodeInfo) *expander.Option {
	r.results.inputOptions = expanderOptionsToGroupSizeChanges(options)
	for _, option := range options {
		tempgroupSizeChange := expanderOptionToGroupSizeChange(option)
		if tempgroupSizeChange == r.optionToChoose {
			return &option
		}
	}
...
func expanderOptionsToGroupSizeChanges(options []expander.Option) []groupSizeChange {
	...
	for _, option := range options {
		tempgroupSizeChange := expanderOptionToGroupSizeChange(option)
		groupSizeChanges = append(groupSizeChanges, tempgroupSizeChange)
	}
...
func runSimpleScaleUpTest(t *testing.T, config *scaleTestConfig) *scaleTestResults {
	...
	resourceLimiter := cloudprovider.NewResourceLimiter(
		map[string]int64{cloudprovider.ResourceNameCores: config.options.MinCoresTotal,
			cloudprovider.ResourceNameMemory: config.options.MinMemoryTotal},
		map[string]int64{cloudprovider.ResourceNameCores: config.options.MaxCoresTotal,
			cloudprovider.ResourceNameMemory: config.options.MaxMemoryTotal})
	provider.SetResourceLimiter(resourceLimiter)

	assert.NotNil(t, provider)

	// Create context with non-random expander strategy.
	context, _ := NewScaleTestAutoscalingContext(config.options, &fake.Clientset{}, listers, provider, nil)
...
func simpleScaleUpTest(t *testing.T, config *scaleTestConfig, expectedResults *scaleTestResults) {
	...
	if len(expectedResults.expansionOptions) > 0 {
		...
		assert.Contains(t, expectedResults.expansionOptions, config.expansionOptionToChoose,
			"final expected expansion option must be in expected expansion options")
		assert.Contains(t, results.expansionOptions, config.expansionOptionToChoose,
			"final expected expansion option must be in expected expansion options")
...
func TestScaleUpUnhealthy(t *testing.T) {
	...
	context, _ := NewScaleTestAutoscalingContext(options, &fake.Clientset{}, listers, provider, nil)

	nodes := []*apiv1.Node{n1, n2}
	nodeInfos, _ := getNodeInfosForGroups(nodes, nil, provider, listers, []*appsv1.DaemonSet{},
...
func TestScaleUpAutoprovisionedNodeGroup(t *testing.T) {
	...
	listers := kube_util.NewListerRegistry(nil, nil, podLister, nil, nil, nil, nil, nil, nil, nil)
	context, _ := NewScaleTestAutoscalingContext(options, fakeClient, listers, provider, nil)
	...
	nodes := []*apiv1.Node{}
	nodeInfos, _ := getNodeInfosForGroups(nodes, nil, provider, context.ListerRegistry, []*appsv1.DaemonSet{},
		context.PredicateChecker, nil)
