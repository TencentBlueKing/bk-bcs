...
func sortNodesWithCostAndUtilization(nodes []*corev1.Node, candidates []string,
	...
	for i := range nodes {
		...
		utilInfo, err := simulator.CalculateUtilization(node, nodeInfo, sd.context.IgnoreDaemonSetsUtilization,
			sd.context.IgnoreMirrorPodsUtilization, sd.context.CloudProvider.GPULabel(), time.Now())
		if err != nil {
...
func ExecuteScaleDown(context *contextinternal.Context, sd *ScaleDown,
	...
	for i := range nodes {
		...
		if err != nil {
			return fmt.Errorf("Failed to find node group info for %v: %v", node.Name, err)
		}
		...
		if err != nil {
			return fmt.Errorf("Failed to get target size of node group %v: %v", ng.Id(), err)
		}
		...
		podsToRemove, daemonsetPods := simpleGetPodsToMove(nodeNameToNodeInfo[node.Name])
		...
		go func() {
			...
			result = sd.deleteNode(node, podsToRemove, daemonsetPods, ng)
