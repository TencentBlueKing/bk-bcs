...
func TestFindUnneededNodes(t *testing.T) {
	...
	n8.Spec.Taints = []apiv1.Taint{{Key: deletetaint.ToBeDeletedTaint,
		Value: strconv.FormatInt(time.Now().Unix()-301, 10)}}
	// Nod being deleted recently.
	n9 := BuildTestNode("n9", 1000, 10)
	n9.Spec.Taints = []apiv1.Taint{{Key: deletetaint.ToBeDeletedTaint,
		Value: strconv.FormatInt(time.Now().Unix()-60, 10)}}
	...
	err = sd.UpdateUnneededNodes(allNodes, allNodes, allNodes, []*apiv1.Pod{p1, p2, p3, p4}, time.Now(), nil, nil)
	if err != nil {
		t.Logf("UpdateUnneededNodes failed")
	}
	sd.unremovableNodes = make(map[string]time.Time)
	...
	err = sd.UpdateUnneededNodes(allNodes, allNodes, scaleDownCandidates, []*apiv1.Pod{p1, p2, p3, p4},
		time.Now(), nil, nil)
	if err != nil {
		t.Logf("UpdateUnneededNodes failed")
	}
	...
	err = sd.UpdateUnneededNodes(allNodes, allNodes, allNodes, []*apiv1.Pod{}, time.Now(), nil, nil)
	if err != nil {
		t.Logf("UpdateUnneededNodes failed")
	}
	assert.Equal(t, 0, len(sd.unneededNodes))
	...
	err = sd.UpdateUnneededNodes(allNodes, allNodes, allNodes, []*apiv1.Pod{},
		time.Now().Add(context.UnremovableNodeRecheckTimeout+time.Second), nil, nil)
	if err != nil {
		t.Logf("UpdateUnneededNodes failed")
	}
	assert.Equal(t, 1, len(sd.unneededNodes))
		...
		t.Run(scenario.name, func(t *testing.T) {
			...
			fakeClient.Fake.AddReactor("update", "nodes",
				func(action core.Action) (bool, runtime.Object, error) {
					...
					obj, ok := update.GetObject().(*apiv1.Node)
					if !ok {
						t.Logf("GetObject returns bad values")
					}
...
func TestDrainNode(t *testing.T) {
	...
	_, err := drainNode(n1, []*apiv1.Pod{p1, p2}, fakeClient, kube_util.CreateEventRecorder(fakeClient), 20, 5*time.Second,
		0*time.Second, PodEvictionHeadroom)
...
func TestDrainNodeWithRetries(t *testing.T) {
	...
	_, err := drainNode(n1, []*apiv1.Pod{p1, p2, p3}, fakeClient, kube_util.CreateEventRecorder(fakeClient),
		20, 5*time.Second, 0*time.Second, PodEvictionHeadroom)
	assert.NoError(t, err)
	deleted := make([]string, 0)
	deleted = append(deleted, getStringFromChan(deletedPods))
	deleted = append(deleted, getStringFromChan(deletedPods))
...
func TestDrainNodeEvictionFailure(t *testing.T) {
	...
	evictionResults, err := drainNode(n1, []*apiv1.Pod{p1, p2, p3, p4}, fakeClient,
		kube_util.CreateEventRecorder(fakeClient), 20, 0*time.Second, 0*time.Second, PodEvictionHeadroom)
	assert.Error(t, err)
...
func TestDrainNodeDisappearanceFailure(t *testing.T) {
	...
	evictionResults, err := drainNode(n1, []*apiv1.Pod{p1, p2, p3, p4}, fakeClient,
		kube_util.CreateEventRecorder(fakeClient), 0, 0*time.Second, 0*time.Second, 0*time.Second)
	assert.Error(t, err)
	assert.Equal(t, 4, len(evictionResults))
	assert.Equal(t, *p1, *evictionResults["p1"].Pod)
	assert.Equal(t, *p2, *evictionResults["p2"].Pod)
...
func TestScaleDown(t *testing.T) {
	...
	if err != nil {
		t.Logf("UpdateUnneededNodes failed. Error: %v", err)
	}
...
func simpleScaleDownEmpty(t *testing.T, config *scaleTestConfig) {
	...
	err := scaleDown.UpdateUnneededNodes(nodes, nodes, nodes, []*apiv1.Pod{},
		time.Now().Add(-5*time.Minute), nil, tempNodesPerGroup)
	if err != nil {
		t.Logf("UpdateUnneededNodes failed. Error: %v", err)
	}
...
func TestScaleDownNoMove(t *testing.T) {
	...
	if err != nil {
		t.Logf("UpdateUneededNodes failed. Error: %v", err)
	}
...
func TestFilterOutMasters(t *testing.T) {
	...
	pods := []*apiv1.Pod{
		BuildTestPodWithExtra("kube-apiserver-kubernetes-master", "kube-system", "n2",
			map[string]string{}), // without label
		BuildTestPodWithExtra("kube-apiserver-kubernetes-master", "fake-kube-system", "n6",
			map[string]string{"component": "kube-apiserver"}), // wrong namespace
		BuildTestPodWithExtra("kube-apiserver-kubernetes-master", "kube-system", "n3",
			map[string]string{"component": "kube-apiserver"}), // real api server
		BuildTestPodWithExtra("hidden-name", "kube-system", "n7",
			map[string]string{"component": "kube-apiserver"}), // also a real api server
		BuildTestPodWithExtra("kube-apiserver-kubernetes-master", "kube-system", "n1",
			map[string]string{"component": "kube-apiserver-dev"}), // wrong label
		BuildTestPodWithExtra("custom-deployment", "custom", "n5",
			map[string]string{"component": "custom-component", "custom-key": "custom-value"}), // unrelated pod
	}
