...
func (executor *BcsExecutor) healthcheckSetting(containerTask *container.BcsContainerTask,
	taskInfo *mesos.TaskInfo) error {
	...
	if health != nil {
		...
		case mesos.HealthCheck_HTTP:
			...
			checker, checkErr = healthcheck.NewHTTPChecker(containerTask.Name, httpChecker.GetScheme(),
				int(httpChecker.GetPort()), httpChecker.GetPath(), tm, nil)
...
func (executor *BcsExecutor) portMappingSetting(containerTask *container.BcsContainerTask, taskInfo *mesos.TaskInfo) {
	...
	if portMaps != nil && len(portMaps) > 0 {
		for _, ports := range portMaps {
			...
			logs.Infof("portMapping info Protocol %s HostPort %d ContainerPort %d.", ports.GetProtocol(), ports.GetHostPort(),
...
// preContainerStartEventCallback xxx
