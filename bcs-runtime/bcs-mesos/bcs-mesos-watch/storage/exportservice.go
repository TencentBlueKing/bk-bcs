...
func (handler *ExpServiceHandler) Add(data interface{}) error {
	...
	dataNode := "/bcsstorage/v1/mesos/watch/clusters/" + handler.ClusterID + "/namespaces/" + dataType.Namespace + "/" +
		handler.dataType + "/" + dataType.ServiceName

	if err := handler.oper.CreateDCNode(dataNode, data, "PUT"); err != nil {
		util.ReportStorageMetrics(handler.ClusterID, dataTypeExpSVR, actionPut, handlerWatchClusterNamespaceTypeName,
	...
	started2 := time.Now()
	dataNode2 := "/bcsstorage/v1/mesos/dynamic/namespace_resources/clusters/" + handler.ClusterID + "/namespaces/" +
		dataType.Namespace + "/" + handler.dataType + "/" + dataType.ServiceName

	if err := handler.oper.CreateDCNode(dataNode2, data, "PUT"); err != nil {
		util.ReportStorageMetrics(handler.ClusterID, dataTypeExpSVR, actionPut, handlerClusterNamespaceTypeName,
			util.StatusFailure, started2)
	} else {
		util.ReportStorageMetrics(handler.ClusterID, dataTypeExpSVR, actionPut, handlerClusterNamespaceTypeName,
			util.StatusSuccess, started2)
	}
	return nil
}
...
func (handler *ExpServiceHandler) Delete(data interface{}) error {
	...
	blog.V(3).Infof("ExportService %s-%s.%s handle delete Event", handler.ClusterID, dataType.Namespace,
		dataType.ServiceName)
	dataNode := "/bcsstorage/v1/mesos/watch/clusters/" + handler.ClusterID + "/namespaces/" + dataType.Namespace + "/" +
		handler.dataType + "/" + dataType.ServiceName

	if err := handler.oper.DeleteDCNode(dataNode, "DELETE"); err != nil {
		util.ReportStorageMetrics(handler.ClusterID, dataTypeExpSVR, actionDelete, handlerWatchClusterNamespaceTypeName,
			util.StatusFailure, started)
	} else {
		util.ReportStorageMetrics(handler.ClusterID, dataTypeExpSVR, actionDelete, handlerWatchClusterNamespaceTypeName,
			util.StatusSuccess, started)
	...
	if err := handler.oper.DeleteDCNode(dataNode2, "DELETE"); err != nil {
		util.ReportStorageMetrics(handler.ClusterID, dataTypeExpSVR, actionDelete, handlerClusterNamespaceTypeName,
			util.StatusFailure, started2)
	} else {
		util.ReportStorageMetrics(handler.ClusterID, dataTypeExpSVR, actionDelete, handlerClusterNamespaceTypeName,
			util.StatusSuccess, started2)
...
func (handler *ExpServiceHandler) Update(data interface{}) error {
	...
	blog.V(3).Infof("ExportService %s-%s.%s handle update event", handler.ClusterID, dataType.Namespace,
		dataType.ServiceName)
	dataNode := "/bcsstorage/v1/mesos/watch/clusters/" + handler.ClusterID + "/namespaces/" + dataType.Namespace + "/" +
		handler.dataType + "/" + dataType.ServiceName
