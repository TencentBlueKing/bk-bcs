...
import (
	...
	"github.com/Tencent/bk-bcs/bcs-runtime/bcs-mesos/bmsf-mesh/bmsf-mesos-adapter/pkg/util/str"
...
func (s *taskGroupController) isValid(task *TaskGroup) bool {
	...
	if !(task.Status == schetypes.TASK_STATUS_RUNNING || task.Status == schetypes.TASK_STATUS_LOST) {
		blog.Errorf("bk-bcs TaskGroup %s/%s Status[%s] lost IPAddress info, wait for RUNNING", task.GetNamespace(),
			task.GetName(), task.Status)
		return false
	}
	return true
}
...
func (s *taskGroupController) convertTaskGroupToAppNode(taskGroup *TaskGroup) *v1.AppNode {
	...
	for _, task := range taskGroup.Taskgroup {
		...
		if err := json.Unmarshal([]byte(task.StatusData), info); err != nil {
			blog.Errorf("TaskGroup %s/%s decode Container %s Status data failed, %s", taskGroup.GetNamespace(),
				taskGroup.GetName(), task.ID, err)
			continue
		}
	...
	if len(node.Spec.NodeIP) == 0 {
		blog.Errorf("bk-bcs convert TaskGroup %s/%s to AppNode finnally failed. lost node ip address, detail: %s",
			taskGroup.GetNamespace(), taskGroup.GetName(), taskGroup.ID)
		return nil
	}
