...
const (
	...
	// TypeHashIPPortNet The hash:ip,port,net set type uses a hash to store IP address,
	// port number and IP network address triples. The port number is interpreted together with a protocol (default TCP)
	// and zero protocol number cannot be used. Network address with zero prefix size cannot be stored either.
	...
	// TypeHashIPNetPortNet The hash:net,port,net set type behaves similarly to hash:ip,port,net
	// but accepts a cidr value for both the first and last parameter.
	...
	// TypeHashNetIface The hash:net,iface set type uses a hash to store different sized IP network address
	// and interface name pairs.
	...
	// The value of the timeout parameter for the create command means the default timeout value (in seconds)
	// for new entries. If a set is created with timeout support, then the same timeout option can be used
	// to specify non-default timeout values when adding entries.
	// Zero timeout value means the entry is added permanent to the set.
	// The timeout value of already added elements can be changed by readding the element using the -exist option.
	// When listing the set, the number of entries printed in the header might be larger than
	// the listed number of entries for sets with the timeout extensions: the number of entries in the set is updated
	// when elements added/deleted to the set and periodically when the garbage colletor evicts the timed out entries.
	...
	// An example when an element is added to a set with non-zero counter values.
	...
	// Enabling this extension on an ipset enables you to annotate an ipset entry with an arbitrary string.
	// This string is completely ignored by both the kernel and ipset itself and is purely for
	// providing a convenient means to document the reason for an entry's existence.
	// Comments must not contain any quotation marks and the usual escape character (\) has no meaning
	OptionComment = "comment"
	// OptionSkbinfo All set types support the optional skbinfo extension. This extension allow to store
	// the metainfo (firewall mark, tc class and hardware queue) with every entry and map it to packets by
	...
	// skbprio option has tc class format: MAJOR:MINOR, where major and minor numbers are hex without 0x prefix.
	// skbqueue option is just decimal number.
	OptionSkbinfo = "skbinfo"
	// OptionSkbmark All set types support the optional skbinfo extension. This extension allow to store
	// the metainfo (firewall mark, tc class and hardware queue) with every entry and map it to packets by
	// usage of SET netfilter target with --map-set option. skbmark option format: MARK or MARK/MASK,
	// where MARK and MASK are 32bit hex numbers with 0x prefix. If only mark is specified mask 0xffffffff are used.
	// skbprio option has tc class format: MAJOR:MINOR, where major and minor numbers are hex without 0x prefix.
	...
	// OptionSkbprio All set types support the optional skbinfo extension. This extension allow to store
	// the metainfo (firewall mark, tc class and hardware queue) with every entry and map it to packets by
	// usage of SET netfilter target with --map-set option. skbmark option format: MARK or MARK/MASK,
	// where MARK and MASK are 32bit hex numbers with 0x prefix. If only mark is specified mask 0xffffffff are used.
	// skbprio option has tc class format: MAJOR:MINOR, where major and minor numbers are hex without 0x prefix.
	...
	// OptionSkbqueue All set types support the optional skbinfo extension. This extension allow to store
	// the metainfo (firewall mark, tc class and hardware queue) with every entry and map it to packets by
	// usage of SET netfilter target with --map-set option. skbmark option format: MARK or MARK/MASK,
	// where MARK and MASK are 32bit hex numbers with 0x prefix. If only mark is specified mask 0xffffffff are used.
	// skbprio option has tc class format: MAJOR:MINOR, where major and minor numbers are hex without 0x prefix.
	...
	// OptionHashSize This parameter is valid for the create command of all hash type sets. It defines
	// the initial hash size for the set, default is 1024. The hash size must be a power of two,
	// the kernel automatically rounds up non power of two hash sizes to the first correct value.
	OptionHashSize = "hashsize"
	// OptionMaxElem This parameter is valid for the create command of all hash type sets. It does define
	...
	// OptionNoMatch The hash set types which can store net type of data (i.e. hash:*net*) support
	// the optional nomatch option when adding entries. When matching elements in the set,
	...
	// When sets created with this option become full the next addition to the set may succeed
