...
func New(pid string) (*IPTables, error) {
	return NewWithProtocol(pid, ProtocolIPv4)
}

// New creates a new IPTables for the given proto.
// The proto will determine which command is used, either "iptables" or "ip6tables".
func NewWithProtocol(pid string, proto Protocol) (*IPTables, error) {
	//path, err := exec.LookPath(getIptablesCommand(proto))
	//if err != nil {
	//	return nil, err
	//}
	path := "/usr/sbin/iptables"
	vstring, err := getIptablesVersionString(pid, path)
	if err != nil {
...
func (ipt *IPTables) runWithOutput(args []string, stdout io.Writer) error {
	...
	newArgs := []string{nsEnterPath, "-t", ipt.pid, "-n"}
...
func getIptablesVersionString(pid, path string) (string, error) {
	cmd := exec.Command(nsEnterPath, "-t", pid, "-n", path, "--version")
