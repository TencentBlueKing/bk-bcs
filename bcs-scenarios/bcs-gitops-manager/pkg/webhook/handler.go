...
func (s *Server) TGitWebhook(ctx context.Context, req *pb.TGitWebhookRequest, resp *pb.TGitWebhookResponse) error {
	...
	blog.Infof("RequestID[%s] tgit received webhook", ctxutils.RequestID(ctx))
	result, err := s.tgitHandler.Transfer(ctx, req.Data)
	if err != nil {
		blog.Errorf("RequestID[%s] tagit handle transfer failed with body '%s': %s",
			ctxutils.RequestID(ctx), string(req.Data), err.Error())
		return err
	}
	var respBody []byte
	...
	if err != nil {
		blog.Errorf("RequestID[%s] tgit send to gitops with body '%s' failed: %s",
			ctxutils.RequestID(ctx), string(result), err.Error())
		return err
	}
	blog.V(5).Infof("RequestID[%s] tgit webhook response: %s", ctxutils.RequestID(ctx), string(respBody))
	return nil
}
...
func (s *Server) GeneralWebhook(
	...
	blog.Infof("RequestID[%s] general received webhook", ctxutils.RequestID(ctx))
	...
	blog.V(5).Infof("RequestID[%s] general webhook response: %s", ctxutils.RequestID(ctx), string(respBody))
	return nil
}
