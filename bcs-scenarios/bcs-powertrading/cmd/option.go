...
type Endpoints struct {
	BKJob  string `json:"bkJob"`
	BKSops string `json:"bksops"`
	BKCC   string `json:"bkcc"`
	BKCR   string `json:"bkcr"`
}
...
func (opt *Options) Complete() error {
	...
	// recover passwd
	realPwd, err := encrypt.DesDecryptFromBase([]byte(opt.Storage.Password))
	if err != nil {
		return fmt.Errorf("descrypt password failed, %s", err.Error())
	}
	opt.Storage.Password = string(realPwd)
	token, err := encrypt.DesDecryptFromBase([]byte(opt.Gateway.Token))
	if err != nil {
		return fmt.Errorf("descrypt password failed, %s", err.Error())
	}
	opt.Gateway.Token = string(token)
	return nil
}
