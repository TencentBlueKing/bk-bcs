...
func run() {
	...
	fmt.Println("Operator builds gamestatefulset client success...")
	...
	gstsInformerFactory := informers.NewSharedInformerFactory(gstsClient, resyncDuration)
	hookInformerFactory := hookinformers.NewSharedInformerFactory(hookClient, resyncDuration)

	gstsController := gamestatefulset.NewGameStatefulSetController(
	...
	go gstsInformerFactory.Start(stopCh)
	fmt.Println("Operator starting gamestatefulset Informer factory success...")
