...
import (
	...
	tkexinformers "github.com/Tencent/bk-bcs/bcs-k8s/bcs-gamedeployment-operator/pkg/client/informers/externalversions/tkex/v1alpha1"
	gamedeploylister "github.com/Tencent/bk-bcs/bcs-k8s/bcs-gamedeployment-operator/pkg/client/listers/tkex/v1alpha1"
	"github.com/Tencent/bk-bcs/bcs-k8s/bcs-gamedeployment-operator/pkg/providers"
...
func (hc *HookController) Run(workers int, stopCh <-chan struct{}) {
	...
	if !controller.WaitForCacheSync(constants.HookRunController, stopCh, hc.hookRunSynced) {
		klog.Fatalf("Error running HookRun controller: failed to wait for caches to sync")
	}
