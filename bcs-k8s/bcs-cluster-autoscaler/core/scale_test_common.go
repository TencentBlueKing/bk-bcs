...
import (
	...
	estimatorinternal "github.com/bk-bcs/bcs-k8s/bcs-cluster-autoscaler/estimator"
	"testing"
...
func NewScaleTestAutoscalingContext(
	...
	processorCallbacks processor_callbacks.ProcessorCallbacks) contextinternal.Context {
...
func (p *mockAutoprovisioningNodeGroupListProcessor) Process(context *context.AutoscalingContext,
	...
	for _, machineType := range machines {
		nodeGroup, err := context.CloudProvider.NewNodeGroup(machineType, bestLabels, map[string]string{},
			[]apiv1.Taint{}, map[string]resource.Quantity{})
		assert.NoError(p.t, err)
